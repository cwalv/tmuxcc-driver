 ...
 */
export function encodeFrame(id, seq, payload) {
    const idBytes = _encoder.encode(id);
...
export class FrameDecoder {
    ...
    push(chunk) {
        ...
        while (this._bufferedLen >= MIN_HEADER_BYTES) {
            ...
            if (magic !== FRAME_MAGIC) {
                throw new RangeError(`data-plane framing error: expected magic byte 0xCC, got 0x${magic.toString(16).padStart(2, "0")} — stream may be corrupted`);
            }
            ...
            if (payLen > MAX_FRAME) {
                throw new RangeError(`data-plane framing error: PAYLEN ${payLen} exceeds MAX_FRAME cap of ${MAX_FRAME} bytes — possible garbage or hostile frame`);
            }
    ...
    _consumeBytes(n) {
        ...
        while (written < n) {
            const chunk = this._chunks[0];
            if (chunk === undefined)
                break; // should not happen if bufferedLen is tracked correctly
            const needed = n - written;
            if (chunk.length <= needed) {
...
export function decodeFrame(buf) {
    ...
    if (buf[OFF_MAGIC] !== FRAME_MAGIC) {
        throw new RangeError(`data-plane framing error: expected magic byte 0xCC, got 0x${buf[OFF_MAGIC].toString(16).padStart(2, "0")}`);
    }
    ...
    if (buf.length < expectedLen) {
        throw new RangeError(`buffer too short: ${buf.length} bytes, frame requires ${expectedLen} bytes`);
    }
