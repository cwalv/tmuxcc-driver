...
describe("InMemoryTransportPair — data plane", () => {
    it("delivers raw bytes from session-proxy to client, byte-identical", () => {
        ...
        assert.deepStrictEqual(Array.from(receivedBytes), Array.from(sentBytes));
    });
    it("delivers non-UTF-8 bytes byte-identical (binary transparency)", () => {
        ...
        assert.deepStrictEqual(Array.from(receivedBytes), [0xff, 0x00, 0xfe], "non-UTF-8 bytes must survive the transport unchanged");
    });
    it("delivers raw bytes from client to session-proxy", () => {
        ...
        assert.ok(receivedBytes !== null);
        assert.deepStrictEqual(Array.from(receivedBytes), Array.from(bytes));
    });
...
describe("InMemoryTransportPair — lifecycle", () => {
    ...
    it("drops messages sent after close", () => {
        ...
        client.onControl((msg) => received.push(msg));
        sessionProxy.close();
        const msg = { type: "focus.changed", seq: 3, paneId: null, windowId: null };
    ...
    it("a late subscriber does not disarm earlier ones (tc-1a9d regression shape)", () => {
        ...
        sessionProxy.close();
        assert.ok(preHandshakeDisconnectFired, "pre-handshake disconnect handler must fire — late subscriber must not disarm it");
        assert.ok(postHandshakeDisconnectFired, "post-handshake disconnect handler must also fire");
    });
