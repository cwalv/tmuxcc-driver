...
import { WIRE_PROTOCOL_VERSION, paneId, windowId, sessionId, isControlMessage, isSessionProxyMessage, isClientMessage, } from "./index.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
const P0 = paneId("p0");
...
describe("PaneOpenedMessage", () => {
    it("constructs correctly", () => {
        ...
        assert.strictEqual(msg.paneId, "p0");
    });
});
...
describe("LayoutUpdatedMessage", () => {
    it("constructs with a structured layout tree", () => {
        ...
        assert.strictEqual(left.paneId, "p0");
    });
});
describe("FocusChangedMessage", () => {
    it("constructs with a paneId", () => {
        ...
        assert.strictEqual(msg.paneId, "p1");
    });
...
describe("isSessionProxyMessage / isClientMessage", () => {
    ...
    it("identifies client→session-proxy messages", () => {
        const clientTypes = ["input", "resize.request", "client.capabilities", "resync.request"];
...
describe("SnapshotMessage", () => {
    it("constructs a full-state snapshot with session, windows, panes, focus (v3 single-session)", () => {
        ...
        assert.strictEqual(msg.session.sessionId, "s0");
        assert.strictEqual(msg.windows.length, 1);
        assert.strictEqual(msg.panes.length, 2);
        assert.strictEqual(msg.focus.paneId, "p0");
        void W1;
    });
...
describe("WindowAddedMessage", () => {
    ...
    it("is a session-proxy message", () => {
        const msg = { type: "window.added", seq: 1 };
        assert.strictEqual(isSessionProxyMessage(msg), true);
    });
});
describe("WindowClosedMessage", () => {
    it("constructs correctly", () => {
        ...
        assert.strictEqual(msg.windowId, "w0");
    });
});
...
describe("SessionProxySessionRenamedMessage", () => {
    ...
    it("is a session-proxy message", () => {
        const msg = { type: "session.renamed", seq: 1 };
        assert.strictEqual(isSessionProxyMessage(msg), true);
    });
});
// ---------------------------------------------------------------------------
...
describe("PaneModeChangedMessage", () => {
    ...
    it("is a session-proxy message", () => {
        const msg = { type: "pane.mode-changed", seq: 1 };
        assert.strictEqual(isSessionProxyMessage(msg), true);
    });
});
// ---------------------------------------------------------------------------
...
describe("CommandRequestMessage", () => {
    ...
    it("constructs a close-pane command", () => {
        ...
        assert.strictEqual(msg.command.paneId, "p0");
    });
    ...
    it("is recognized as a client message", () => {
        const msg = { type: "command.request", seq: 1 };
...
describe("CommandResponseMessage — success", () => {
    ...
    it("is recognized as a session-proxy message", () => {
        const msg = { type: "command.response", seq: 1 };
        assert.strictEqual(isSessionProxyMessage(msg), true);
    });
});
...
describe("wire invariant", () => {
    it("InputMessage uses string data, not tmux send-keys syntax", () => {
        ...
        const msg = { type: "input", seq: 1, paneId: P0, data: "hello" };
