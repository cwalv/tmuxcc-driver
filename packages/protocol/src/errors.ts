...
// `keyof TmuxCapabilityMap` is a type-only reference; the circular
// type-only import is erased by tsc and does not create a runtime cycle.
import type { TmuxCapabilityMap } from "./server-proxy-control.js";

// ---------------------------------------------------------------------------
// CommandFailure — shared wire envelope for ok=false branches
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// CapabilityRequiredDetails — sole details contract in this bead
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// CommandError — thrown-Error sibling of CommandFailure
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export function isCommandError(err: unknown, code?: string): err is CommandError {
  if (
    typeof err !== "object" ||
    err === null ||
    (err as { name?: unknown }).name !== "CommandError" ||
 ...
 */
export function requiredCapability(err: CommandError): CapabilityRequiredDetails | undefined {
  const d = err.details;
  if (
    typeof d !== "object" ||
    d === null ||
    typeof (d as { capability?: unknown }).capability !== "string"
  ) {
    return undefined;
  }
  // Trusting cast: wire-sourced details are bare JSON.parse output.
  return d as CapabilityRequiredDetails;
}
 ...
 */
export function toCommandFailure(err: CommandError): CommandFailure {
  const base = { ok: false as const, code: err.code, message: err.message };
  if (err.details !== undefined) {
    return { ...base, details: err.details };
  }
  return base;
}
