 ...
 *
 * These are emitted in the `result.code` field of SessionProxyCommandResponseMessage
 * when a specific command request fails.  Distinct from WireErrorCode (which is
 * used in the unsolicited ErrorMessage for protocol-level / session-level errors).
 *
 * Emission audit (grep-verifiable against packages/driver/src/runtime/):
 *
 * "verb.failed"        — input-path.ts: the tmux verb returned a non-zero exit
 *                        or a known-error string (e.g. split-pane on a dead pane,
 *                        rename-window on a non-existent window).
 * "verb.internal"      — input-path.ts: unexpected driver-side error while
 *                        executing the verb (e.g. exception thrown by the
 *                        session-proxy runner).
 * "verb.no-effect-ids" — input-path.ts (driver): the verb succeeded but the
 *                        response payload was missing expected entity ids.
 *                        Also client-minted (clients/ts/src/input.ts:584) when
 *                        a command.response ok=true carries only one of paneId /
 *                        windowId on a create-pane verb — the client surfaces it
 *                        as a failure rather than a half-populated success.
 * "read-only"          — session-proxy.ts: mutating command rejected because the
 *                        client attached with the read-only flag.
 * "pane.not-found"     — session-proxy.ts / input-path.ts: the targeted pane is
 *                        not present in the session model (also used as the code
 *                        in pane.attach.failed, which has its own message type).
 * "internal"           — input-path.ts / session-proxy.ts: unexpected session-proxy
 *                        error not attributable to the verb arguments.
 *
 ...
 *
 * These are distinct from SessionProxyCommandErrorCode — they appear in the
 * unsolicited `ErrorMessage` (type: "error") for protocol-level failures and
 * session-lifecycle events where there is no in-flight command to correlate.
 *
 * Emission audit (grep-verifiable against packages/driver/src/runtime/):
 *
 * "protocol.unknown-message"   — session-proxy.ts: unknown message type received;
 *                                message dropped.
 ...
 * "session.unavailable"        — session-proxy.ts: the tmux session the connection
 *                                was bound to has gone away unexpectedly, or a
 *                                switch-client caused the bound session to disappear.
 * "session.not-found"          — server-proxy.ts: session.attach named an unknown
 *                                session; emitted as an unsolicited ErrorMessage
 *                                before the data connection is closed.
 * "internal"                   — session-proxy.ts: unexpected session-proxy-side
 *                                error not attributable to a specific command.
 *
 * The type is open-ended for forward compatibility.
 */
