...
function makePipeline(
  replies: Map<string, CommandResult | "reject">,
): { pipeline: HydrationPipeline; sent: string[] } {
  const sent: string[] = [];
  return {
    sent,
    pipeline: {
...
describe("captureText — unit (tc-295a.11)", () => {
  ...
  it("4. invalid PaneId format (no 'p' prefix): returns ok=false", async () => {
    const bad = "x42" as unknown as PaneId;
  ...
  it("4b. PaneId with non-numeric suffix: returns ok=false", async () => {
    const bad = "pXYZ" as unknown as PaneId;
    const { pipeline, sent } = makePipeline(new Map());
 ...
 */
async function simulatePaneCaptureHandler(
  ...
  if (!result.ok) {
    return {
      ok: false,
      code: "pane.not-found",
      message: `Pane ${pid as string} could not be captured (it may have closed mid-request).`,
    };
  }
