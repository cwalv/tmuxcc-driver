...
// ---------------------------------------------------------------------------
// Minimal stub pipeline — satisfies the RuntimePipeline interface surface
// used by ControlServer (getModel + onModelChange) without needing a real
// tmux process. Type-asserted to RuntimePipeline; unused methods (send,
// onNotification, etc.) are never called by ControlServer.
// ---------------------------------------------------------------------------

type ModelChangeHandler = (newModel: SessionModel, prevModel: SessionModel) => void;

function createStubPipeline(): RuntimePipeline {
  let model: SessionModel = emptyModel();
  const handlers = new Set<ModelChangeHandler>();
  const buffers = createPaneBufferStore();
  const stub = {
    getModel: () => model,
    onModelChange: (h: ModelChangeHandler) => {
      handlers.add(h);
    ...
    injectNotification: () => {},
    patchModel: () => {},
    send: () => Promise.resolve({ ok: true as const, output: "" }),
    sendBatch: () => [],
    refreshCorrelatorPendingGauge: () => {},
  };
  return stub as unknown as RuntimePipeline;
}
...
describe("tc-295a.21: raw/unhandshaked connection — per-connection rejection, server survives", () => {
  ...
  it(
    "U1: addClient rejects with HandshakeError when the client closes without handshaking",
    { timeout: 10_000 },
    async () => {
      ...
      assert.ok(
        caught instanceof HandshakeError,
        `expected HandshakeError, got: ${String(caught)}`,
      );
      assert.equal(
        (caught as HandshakeError).code,
        "transport.closed",
        `expected code "transport.closed", got: ${(caught as HandshakeError).code}`,
      );
    },
  );
  ...
  it(
    "U2: clientCount remains 0 after a bad connection's addClient rejection",
    { timeout: 10_000 },
    async () => {
      const pipeline = createStubPipeline();
      const server = createControlServer(pipeline);
      ...
      await addPromise.catch((_err: unknown) => { /* per-connection rejection */ });

      assert.equal(
        server.clientCount(),
        0,
        "bad connection must not be counted in clientCount after rejection",
      );
    },
  );
  ...
  it(
    "U3: a good client can still connect successfully after a bad connection rejects",
    { timeout: 10_000 },
    async () => {
      const pipeline = createStubPipeline();
      const server = createControlServer(pipeline);
      ...
      const badP = server.addClient(dtBad).catch((_err: unknown) => {
      ...
      assert.equal(
        server.clientCount(),
        1,
        "good client must be counted after successful handshake",
      );
  ...
  it(
    "U4: concurrent bad + good connections — bad rejects, good succeeds",
    { timeout: 10_000 },
    async () => {
      const pipeline = createStubPipeline();
      const server = createControlServer(pipeline);
      ...
      const badCaught: HandshakeError[] = [];
      const badP = server.addClient(dtBad).catch((err: unknown) => {
