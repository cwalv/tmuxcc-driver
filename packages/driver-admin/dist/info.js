 ...
 * @module info
 */
import { serverProxySocketPath, connectSocketTransport, } from "@tmuxcc/server-proxy";
 ...
 */
export async function runServerProxyCommand(transport, command, contextLabel) {
    ...
    const responsePromise = new Promise((resolve, reject) => {
        ...
        transport.onClose((err) => {
            reject(err ??
                new Error(`server-proxy transport closed before ${contextLabel} response`));
        });
    });
 ...
 */
export async function fetchServerProxyInfo(socketName) {
    ...
    try {
        await runClientHandshake(transport, ADMIN_SERVER_PROXY_CAPS, "server-proxy.capabilities");
        const payload = await runServerProxyCommand(transport, { kind: "server-proxy.info" }, "server-proxy.info");
        if (payload.info === undefined) {
            throw new Error("server-proxy.info succeeded but returned no info payload");
        }
        return payload.info;
    }
    finally {
 ...
 */
export async function fetchSessionProxyInfo(socketName, sessionName) {
    ...
    let endpoint;
    try {
        await runClientHandshake(serverProxyTransport, ADMIN_SERVER_PROXY_CAPS, "server-proxy.capabilities");
        const claim = await runServerProxyCommand(serverProxyTransport, { kind: "session.claim", name: sessionName }, "session.claim");
        if (claim.endpoint === undefined) {
            throw new Error("server-proxy session.claim succeeded but returned no endpoint");
        }
 ...
 * on the matching id.
 */
function sendSessionProxyInfo(conn) {
    const correlationId = mintCorrelationId();
    return new Promise((resolve, reject) => {
        conn.onStateChange((state) => {
            if (state === "closed" || state === "failed") {
                reject(new Error(`session-proxy connection ${state} before session-proxy.info response`));
            }
        });
        conn.onControl((msg) => {
            const m = msg;
            ...
            if (!m.result.ok) {
                reject(new Error(`session-proxy.info failed: [${m.result.code}] ${m.result.message}`));
                return;
            }
            const info = m.result.payload?.info;
            if (info === undefined) {
                reject(new Error("session-proxy.info succeeded but returned no info payload"));
                return;
            }
            resolve(info);
        });
