...
test("runServerProxyCommand resolves with the correlated ok payload", async () => {
    ...
    const payload = await runServerProxyCommand(client, { kind: "server-proxy.info" }, "server-proxy.info");
...
test("runServerProxyCommand rejects on an ok:false result", async () => {
    ...
    await assert.rejects(runServerProxyCommand(client, { kind: "server-proxy.info" }, "server-proxy.info"), /server-proxy server-proxy\.info failed: \[protocol\.unknown-message\] nope/);
});
...
test("runServerProxyCommand rejects when the transport closes before a response", async () => {
    ...
    await assert.rejects(runServerProxyCommand(client, { kind: "server-proxy.info" }, "server-proxy.info"), /closed before server-proxy\.info response/);
});
