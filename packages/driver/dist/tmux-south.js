 ...
 */
export const TMUX_SESSION_NAME_TAKEN_CODE = "tmux.duplicate-session";
/**
 ...
 */
export async function listSessionForFreeze(socketName, sessionName) {
    // list-windows: window_id, window_name, window_layout
    const WIN_FMT = "#{window_id}\t#{window_name}\t#{window_layout}";
    const winResult = await runTmux(["-L", socketName, "list-windows", "-t", sessionName, "-F", WIN_FMT], 5_000);
    if (winResult.status !== 0 || winResult.error)
        return null;
    const windows = [];
    for (const line of (winResult.stdout ?? "").trim().split("\n")) {
    ...
    const paneResult = await runTmux(["-L", socketName, "list-panes", "-s", "-t", sessionName, "-F", PANE_FMT], 5_000);
    if (paneResult.status !== 0 || paneResult.error)
        return null;
    const panes = [];
    for (const line of (paneResult.stdout ?? "").trim().split("\n")) {
...
export async function createSession(socketName, name, capabilities, env) {
    ...
    if (env !== undefined && Object.keys(env).length > 0) {
        if (capabilities !== undefined && !capabilities.newSessionEnvFlag) {
            throw new CommandError("tmux.capability-required", `new-session -e (capability newSessionEnvFlag) requires tmux >= 3.2. ` +
                `The probed tmux version does not support this flag. ` +
                `Upgrade tmux to >= 3.2 or omit env from session.create.`, { capability: "newSessionEnvFlag" });
        }
    }
    ...
    if (result.status !== 0 || result.error) {
        ...
        if (result.stderr && result.stderr.toLowerCase().includes("duplicate session")) {
            throw new CommandError(TMUX_SESSION_NAME_TAKEN_CODE, `tmux new-session: session name already in use (${reason})`);
        }
