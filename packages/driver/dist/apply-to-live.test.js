...
describe("apply-to-live (tc-gjdx.4, real tmux)", () => {
    ...
    it("T1: preview (dryRun) equals the subsequent apply's created set", async (t) => {
        ...
        try {
            ...
            const seedTemplate = { windows: [{ name: "existing" }] };
            ...
            assert.deepEqual(previewNames, ["alpha", "beta"], `preview would-create = [alpha,beta], got ${previewNames.join(",")}`);
            ...
            assert.deepEqual(appliedNames, previewNames, `did-create set must equal would-create set (preview-equals-apply AC)`);
            // Ground truth: exactly [existing, alpha, beta].
            const liveNames = tmuxQ(socketName, ["list-windows", "-t", "t1sess", "-F", "#{window_name}"])
                .split("\n").filter(Boolean).sort();
            assert.deepEqual(liveNames, ["alpha", "beta", "existing"], `live windows after apply = [alpha,beta,existing], got ${liveNames.join(",")}`);
        }
        finally {
            await sp.shutdown();
    ...
    it("T2: name-matching windows are left alone — only missing windows are created", async (t) => {
        ...
        try {
            ...
            assert.deepEqual(createdNames, ["terminal"], `only "terminal" should be created, got ${createdNames.join(",")}`);
    ...
    it("T4: mid-apply failure is loud (failed verb + created-so-far) with NO rollback; existing windows untouched", async (t) => {
        ...
        try {
            ...
            const seedTemplate = { windows: [{ name: "pre-existing" }] };
            const r1 = await sendCommand(mux, {
                kind: "session.create",
                name: "t4sess",
                template: seedTemplate,
            ...
            const template = {
                windows: [
                    { name: "good" },
                    {
                        ...
                        geometry: {
                            kind: "hsplit",
                            children: Array.from({ length: 16 }, () => ({ kind: "pane" })),
                        },
                    },
                ],
            };
            ...
            assert.ok(afterIds.some((w) => w.name === "good"), `partial session preserved: "good" window created before failure`);
        }
        finally {
            await sp.shutdown();
            killServer(socketName);
...
//# sourceMappingURL=apply-to-live.test.js.map