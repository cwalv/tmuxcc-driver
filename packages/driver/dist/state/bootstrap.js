...
import { emptyModel, addSession, addWindow, addPane, updateSession, setFocus, parsedLayoutToWindowLayout, paneId, windowId, sessionId, } from "./model.js";
// ---------------------------------------------------------------------------
// Pane user-options
// ---------------------------------------------------------------------------
/**
 ...
 */
export function paneBoundOptionName(clientId) {
    if (clientId === undefined)
        return TMUXCC_BOUND_OPTION;
 ...
 */
export const BOOTSTRAP_PANES_FORMAT = "#{pane_id}\t#{window_id}\t#{session_id}\t#{pane_index}\t" +
    "#{pane_width}\t#{pane_height}\t#{pane_top}\t#{pane_left}\t" +
    "#{?pane_active,1,0}\t#{pane_pid}\t#{pane_current_command}\t" +
    "#{?pane_dead,1,0}\t#{pane_dead_status}\t#{@tmuxcc_label}\t" +
 ...
 */
export function bootstrapCommands(target) {
...
// ---------------------------------------------------------------------------
function mintPaneId(n) {
    return paneId("p" + n);
}
function mintWindowId(n) {
    return windowId("w" + n);
}
function mintSessionId(n) {
...
export function parseWindowsReply(body) {
    ...
    for (const line of text.split("\n")) {
        ...
        if (tmuxSessionId === null || tmuxWindowId === null)
            continue;
 ...
 */
export function parsePanesReply(body) {
    const text = new TextDecoder().decode(body);
    const rows = [];
    for (const line of text.split("\n")) {
        const trimmed = line.trim();
        ...
        const parts = trimmed.split("\t");
        if (parts.length < 9)
            continue; // need at least through pane_active
        ...
        const detach = detachRaw === "detach" || detachRaw === "kill" ? detachRaw : undefined;
        const iconRaw = (parts[15] ?? "").trim();
        const icon = iconRaw === "" ? undefined : iconRaw;
        const tmuxPaneId = parseSigilId(paneIdStr, "%");
        const tmuxWindowId = parseSigilId(winIdStr, "@");
        const tmuxSessionId = parseSigilId(sessIdStr, "$");
        if (tmuxPaneId === null || tmuxWindowId === null || tmuxSessionId === null)
            continue;
        if (isNaN(width) || isNaN(height))
            continue;
        rows.push({
 ...
 */
export function buildInitialModel(windowRows, paneRows) {
    let model = emptyModel();
    // ---- Step 1: collect sessions ----------------------------------------
    const sessionNames = new Map();
    const sessionActiveWindowId = new Map(); // tmux ids
