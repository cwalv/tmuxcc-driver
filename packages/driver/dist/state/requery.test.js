...
import { emptyModel, addPane, addSession, addWindow, paneId, sessionId, windowId, setFocus, updateSession, updatePane, } from "./model.js";
// ---------------------------------------------------------------------------
...
const SINGLE_PANE_LAYOUT = (paneNum) => `0000,80x24,0,0,${paneNum}`;
const TWO_PANE_HSPLIT_LAYOUT = (a, b) => `0000,80x24,0,0{40x24,0,0,${a},39x24,41,0,${b}}`;
// ---------------------------------------------------------------------------
...
describe("requeryDiff: dead-pane semantics", () => {
    it("pane absent from new list-panes → pane.closed delta", () => {
        ...
        const paneBoth = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
        ...
        assert.equal(closed[0].paneId, paneId("p2"));
    });
...
describe("requeryDiff: dead-pane wire shape (tc-4bv2 / tc-295a.10)", () => {
    ...
    it("W2.5: an all-dead-pane session bootstraps into a NON-EMPTY snapshot (READY-independent-of-live-panes)", () => {
        ...
        assert.equal(opened[0].dead, true, "pane.opened carries dead");
        assert.equal(opened[0].exitCode, 0);
    ...
    it("live pane → dead corpse in place emits pane.dead-changed (NOT pane.closed)", () => {
        ...
        assert.equal(deadChanged[0].dead, true);
        assert.equal(deadChanged[0].exitCode, 137);
    ...
    it("dead corpse → reaped (gone from list-panes) emits exactly one pane.closed carrying the exit code", () => {
        ...
        assert.equal(closed[0].exitCode, 42, "exit code carried through from corpse");
    });
    it("respawn-in-place (dead → live) emits pane.dead-changed dead:false with no exitCode", () => {
        ...
        assert.equal(dc[0].dead, false);
        assert.ok(!("exitCode" in dc[0]), "no exitCode when alive");
    });
});
// ---------------------------------------------------------------------------
...
describe("requeryDiff: durable pane-name (@tmuxcc_label, tc-1a8z)", () => {
    ...
    it("bootstrap re-reads @tmuxcc_label into the model (survives a driver restart)", () => {
        ...
        const opened = deltas.filter((d) => d.type === "pane.opened");
        assert.equal(opened.length, 1);
        assert.equal(opened[0].label, "deploy", "pane.opened carries the durable name");
    });
...
describe("requeryDiff: durable policy (@tmuxcc-detach/-icon, tc-i9aq.1) + binding carry-forward (tc-4b6k.2)", () => {
    ...
    it("bootstrap re-reads @tmuxcc-detach/-icon; boundClients starts empty (per-client not bulk-read)", () => {
        ...
        const opened = deltas.filter((d) => d.type === "pane.opened");
        assert.equal(opened.length, 1);
        assert.ok(!("bound" in opened[0]), "pane.opened bound off (no client id)");
        assert.equal(opened[0].detach, "kill", "pane.opened carries detach");
        assert.equal(opened[0].icon, "rocket", "pane.opened carries icon");
    });
    ...
    it("carries per-client binding forward across a topology-only requery cycle", () => {
        ...
        const { next, deltas } = requeryDiff(prev, okResult(winSingle), okResult(bare));
        assert.deepEqual([...next.panes.get(paneId("p1")).boundClients], ["ws-alpha"], "surviving pane keeps its per-client binding across the requery");
    ...
    it("a changed detach on a later requery emits exactly one pane.policy-changed", () => {
        ...
        assert.equal(pc[0].detach, "detach");
    ...
    it("clearing detach on a later requery emits pane.policy-changed with detach ABSENT", () => {
        ...
        assert.ok(!("detach" in pc[0]), "detach omitted when cleared");
    });
});
// ---------------------------------------------------------------------------
...
describe("requeryDiff: pane.closed strong contract matrix (tc-295a.10 W2.4)", () => {
    it("shell-exit (no remain-on-exit): exactly one pane.closed, exitCode absent (no corpse phase)", () => {
        ...
        const closed = deltas.filter((d) => d.type === "pane.closed");
        assert.equal(closed.length, 1, "exactly one pane.closed");
        assert.equal(closed[0].paneId, paneId("p1"));
        assert.ok(!("exitCode" in closed[0]), "exitCode absent — no corpse to read");
    });
    it("external kill-pane: sibling removed from a multi-pane window → exactly one pane.closed, window survives", () => {
        ...
        const both = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
            paneLine({ paneNum: 2, winNum: 1, sessNum: 0, width: 39, left: 41 });
        ...
        assert.equal(closed.length, 1, "exactly one pane.closed");
        assert.equal(closed[0].paneId, paneId("p2"));
        assert.ok(!("exitCode" in closed[0]), "exitCode absent for external kill");
    ...
    it("kill-window: one pane.closed per pane + one window.closed", () => {
        ...
        const closedIds = new Set(closed.map((d) => d.paneId));
        assert.ok(closedIds.has(paneId("p1")) && closedIds.has(paneId("p2")));
        for (const d of closed) {
            assert.ok(!("exitCode" in d), "exitCode absent for kill-window");
        }
...
describe("requeryDiff: reparenting (break-pane)", () => {
    it("same pane id, new window → no pane.opened/pane.closed, layouts update", () => {
        ...
        const paneBefore = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
            paneLine({ paneNum: 2, winNum: 1, sessNum: 0, width: 39, left: 41 });
        ...
        const paneAfter = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, active: true }) +
        ...
        assert.equal(moved.paneId, paneId("p2"));
        assert.equal(moved.windowId, windowId("w2"));
...
describe("RequeryEngine: basic cycle", () => {
    ...
    it("getModel() reflects the last cycle's result", async () => {
        ...
        const pane2 = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
            paneLine({ paneNum: 2, winNum: 1, sessNum: 0, width: 39, left: 41 });
...
describe("RequeryEngine: dirty mid-flight → re-run", () => {
    ...
    it("commit-only-clean (tc-128.5): mid-flight dirty discards intermediate model — getModel() stays unchanged until clean", async () => {
        ...
        await new Promise((r) => setImmediate(r));
        assert.equal(engine.getModel(), preCallModel, "engine.getModel() must stay at pre-call model while cycle is dirty mid-flight");
...
describe("RequeryEngine: storm / convergence-budget (tc-128.5)", () => {
    it("storm during every cycle → loop exits within budget, returns failed:true, model unchanged", async () => {
        ...
        const replies = [];
        for (let i = 0; i < 100; i++)
            replies.push(okResult(win), okResult(pane));
        ...
        assert.equal(engine.getModel(), preCallModel, "engine.getModel() must NOT have advanced to any intermediate candidate");
        ...
        assert.ok(cyclesRun > 0 && cyclesRun <= 20, `expected bounded cycles (≤20), got ${cyclesRun}`);
    ...
    it("failure-after-progress (problem 2): success-then-error must NOT desync model — getModel() stays at pre-call snapshot and a subsequent successful requery emits the full cumulative deltas", async () => {
        ...
        const paneSplit = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
            paneLine({ paneNum: 2, winNum: 1, sessNum: 0, width: 39, left: 41 });
        const d = deferredSubmit();
        const engine = createRequeryEngine({ submit: d.submit });
        const preCallModel = engine.getModel();
        ...
        assert.equal(engine.getModel(), preCallModel, "PROBLEM 2 regression: getModel() must stay at pre-call M0 across mid-flight dirty");
        ...
        assert.equal(engine.getModel(), preCallModel, "after failed requery: getModel() still M0 (model never advanced)");
        ...
        assert.equal(opened.length, 2, `expected cumulative deltas to include both panes (no gap), got: ${cumulative.map((dlt) => dlt.type).join(", ")}`);
 ...
 */
function applyDeltaToModel(m, delta) {
    switch (delta.type) {
        case "window.added": {
            ...
            if (!sessEntry)
                throw new Error("window.added with no session in model");
            const win = {
        ...
        case "window.closed": {
            ...
            if (sess) {
                ...
                m.sessions.set(sess.sessionId, {
                    ...sess,
                    windowIds: remaining,
                    activeWindowId: sess.activeWindowId === delta.windowId
                        ? (remaining[0] ?? null)
                        : sess.activeWindowId,
                });
            }
        ...
        case "window.renamed": {
            const win = m.windows.get(delta.windowId);
            if (!win)
                throw new Error(`window.renamed: ${delta.windowId} not found`);
        ...
        case "layout.updated": {
            const win = m.windows.get(delta.windowId);
            if (!win)
                throw new Error(`layout.updated: ${delta.windowId} not found`);
        ...
        case "pane.opened": {
            const win = m.windows.get(delta.windowId);
            if (!win)
                throw new Error(`pane.opened: window ${delta.windowId} not found`);
            const pane = {
                paneId: delta.paneId,
                windowId: delta.windowId,
        ...
        case "pane.closed": {
            ...
            if (win) {
                ...
                m.windows.set(win.windowId, {
                    ...win,
                    paneIds: remaining,
                    activePaneId: win.activePaneId === delta.paneId
                        ? (remaining[0] ?? null)
                        : win.activePaneId,
                });
            }
        ...
        case "pane.resized": {
            const pane = m.panes.get(delta.paneId);
            if (!pane)
                throw new Error(`pane.resized: ${delta.paneId} not found`);
        ...
        case "pane.mode-changed": {
            const pane = m.panes.get(delta.paneId);
            if (!pane)
                throw new Error(`pane.mode-changed: ${delta.paneId} not found`);
        ...
        case "focus.changed": {
            ...
            const win = m.windows.get(delta.windowId);
            if (!win)
                throw new Error(`focus.changed: window ${delta.windowId} not found`);
        ...
        case "session.renamed": {
            ...
            if (!first)
                throw new Error("session.renamed with no session");
        ...
        default:
            throw new Error(`applyDeltaToModel: unexpected delta type ${delta.type}`);
    }
}
function applyDeltasToModel(prev, deltas) {
    const m = toMutable(prev);
    for (const d of deltas)
        applyDeltaToModel(m, d);
    return freeze(m);
}
/**
 ...
 */
function comparable(model, opts = {}) {
...
describe("requery: property-style round-trip", () => {
    it("for many random model pairs, applyDeltas(prev, diffModel(prev, next)) == next", () => {
        ...
        for (let i = 0; i < N; i++) {
            ...
            assert.deepEqual(comparable(replayed), comparable(next), `round-trip mismatch on iteration ${i}`);
        }
    });
    it("for many random model pairs, requeryDiff round-trips through reply bodies", () => {
        ...
        for (let i = 0; i < N; i++) {
            ...
            const { next: parsed } = requeryDiff(emptyModel(), okResult(winsBody), okResult(panesBody));
            ...
            assert.deepEqual(comparable(parsed, { includeLayout: false }), comparable(next, { includeLayout: false }), `serialize/parse round-trip mismatch on iteration ${i}`);
        }
    });
});
/**
 ...
 * always valid.
 */
function serializeWindowsBody(model) {
    let out = "";
    for (const sess of model.sessions.values()) {
        ...
        for (const wid of sess.windowIds) {
            ...
            const layoutPaneNum = firstPid !== undefined
...
function serializePanesBody(model) {
    let out = "";
...
function twoPaneModelBodies() {
    ...
    const paneBody = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, width: 40, active: true }) +
        paneLine({ paneNum: 2, winNum: 1, sessNum: 0, width: 39, left: 41 });
...
describe("RequeryEngine: teardown-confirmation guard (tc-3si.2)", () => {
    it("legit teardown (both cycles agree) served with exactly one extra cycle of latency", async () => {
        ...
        await new Promise((r) => setImmediate(r));
        assert.equal(d.callCount(), 4, "engine ran exactly one confirming cycle (4 submits = 2 cycles × 2 commands)");
        ...
        assert.ok(types.filter((t) => t === "pane.closed").length === 2, `expected 2 pane.closed deltas, got ${types.join(", ")}`);
        assert.ok(types.includes("window.closed"));
    ...
    it("garbage first cycle (refuted) → model untouched, refuted counter incremented, loud log emitted", async () => {
        ...
        assert.ok(logText.includes("TEARDOWN REFUTED"), `expected loud refuted log, got: ${logText}`);
    ...
    it("threshold is configurable (subtotal teardown trips when threshold lowered)", async () => {
        ...
        const paneBodyPartial = paneLine({ paneNum: 1, winNum: 1, sessNum: 0, active: true }) +
            paneLine({ paneNum: 3, winNum: 2, sessNum: 0, width: 30, active: true }) +
            paneLine({ paneNum: 4, winNum: 2, sessNum: 0, width: 30, left: 31 }) +
            paneLine({ paneNum: 5, winNum: 2, sessNum: 0, width: 19, left: 61 });
    ...
    it("invalid threshold values fall back to default (0.8)", async () => {
        ...
        assert.equal(outcomes.length, 1);
        assert.equal(outcomes[0].threshold, 0.8, "invalid threshold (0) clamped back to default (0.8)");
    });
    ...
    it("budget exhaustion during pending teardown returns failed without touching the counter", async () => {
        ...
        for (let i = 0; i < 100; i++)
            replies.push(okResult(""), okResult(""));
        ...
        assert.equal(outcomes.length, 0, "no confirmation outcome — budget exhausted before a decision");
        assert.deepEqual(counterCalls, [], "teardown counter NOT bumped on budget exhaustion (we never decided)");
...
describe("RequeryEngine: failure telemetry hooks (tc-3si.5)", () => {
    ...
    it("onBudgetExhausted + onCycleFailed(reason=budget) fire together on budget exhaustion", async () => {
        ...
        const logText = captured.join("");
        assert.ok(logText.includes("BUDGET EXHAUSTED"), `expected loud budget-exhausted log; got: ${logText}`);
    });
    ...
    it("refuted teardown does NOT bump requery_failed_cycles_total{reason} (the dedicated counter handles it)", async () => {
        ...
        assert.deepEqual(failedReasons, [], "refuted teardown must NOT bump the error|budget failed-cycles counter (it has its own surface)");
    });
});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function recordingSubmit(results) {
    const queue = [...results];
    const commands = [];
    return {
        submit: (cmd) => {
...
describe("RequeryEngine: id-based session targeting (tc-0v59)", () => {
    it("first cold cycle targets =<name>; every cycle AFTER capturing the id targets $<id>", async () => {
        ...
        assert.ok(winCmd1.includes("-t =main"), `cold cycle 1 list-windows should target =main, got: ${winCmd1}`);
        assert.ok(paneCmd1.includes("-t =main"), `cold cycle 1 list-panes should target =main, got: ${paneCmd1}`);
        // Cycle 2: id ($7) captured from cycle 1's reply → must target by id.
        engine.markDirty();
        await engine.requery();
        const [winCmd2, paneCmd2] = [commands[2], commands[3]];
        assert.ok(winCmd2.includes("-t $7"), `cycle 2 list-windows should target $7, got: ${winCmd2}`);
        assert.ok(!winCmd2.includes("=main"), `cycle 2 must NOT target the mutable name, got: ${winCmd2}`);
        assert.ok(paneCmd2.includes("-s -t $7"), `cycle 2 list-panes should be session-scoped to $7, got: ${paneCmd2}`);
    });
    it("requery SURVIVES a rename: id-targeted reply with the NEW name emits session.renamed", async () => {
        ...
        assert.ok(commands[2].includes("-t $4") && !commands[2].includes("=before"), `post-rename cycle must target $4 not =before, got: ${commands[2]}`);
        ...
        assert.ok(renamed !== undefined, `expected a session.renamed delta, got: ${deltas.map((d) => d.type).join(", ")}`);
        assert.equal(renamed.newName, "after", "session.renamed must carry the new name");
    });
    it("a requery %error emits a loud per-incident stderr line (no silent swallow)", async () => {
        ...
        const logText = captured.join("");
        assert.ok(logText.includes("REQUERY ERROR"), `expected a loud REQUERY ERROR stderr line; got: ${JSON.stringify(logText)}`);
        assert.ok(logText.includes("list-windows"), `loud line should name which reply failed; got: ${logText}`);
    });
});
// ---------------------------------------------------------------------------
...
describe("requeryDiff: durable window options — synchronizePanes/monitorActivity/monitorSilence (tc-pqb4)", () => {
    ...
    it("bootstrap re-reads synchronizePanes=true into the model (survives a driver restart)", () => {
        ...
        assert.equal(next.windows.get(windowId("w1")).synchronizePanes, true, "synchronizePanes=true re-read from tmux reply on bootstrap");
    });
    it("requery does NOT clobber synchronizePanes=true back to false (regression: EDH-reload loses sync state)", () => {
        ...
        assert.equal(next.windows.get(windowId("w1")).synchronizePanes, true, "synchronizePanes must still be true after requery — no clobbering");
        const syncDeltas = deltas.filter((d) => d.type === "window.sync.changed");
        assert.equal(syncDeltas.length, 0, "no spurious window.sync.changed delta when tmux and model agree");
    });
    it("requery emits window.sync.changed when tmux disagrees with optimistic model (toggle raced a requery)", () => {
        ...
        assert.equal(syncDeltas[0].on, false);
    });
...
//# sourceMappingURL=requery.test.js.map