...
import { emptyModel, addSession, addWindow, addPane, removePane, removeWindow, removeSession, updatePane, updateWindow, updateSession, setFocus, checkInvariants, parsedLayoutToWindowLayout, scrollbackHandle, } from "./model.js";
...
function makeWindow(id, sessId, paneIds, activePaneId, name = "test-window") {
    return {
        windowId: id,
        sessionId: sessId,
        name,
...
function validModel() {
    ...
    const win = makeWindow(W1, S1, [P1, P2], P1);
    const p1 = makePane(P1, W1, S1);
    const p2 = makePane(P2, W1, S1);
...
describe("checkInvariants — I1: pane's windowId must exist", () => {
    it("reports pane-missing-window when pane references a non-existent window", () => {
        ...
        const orphanPane = makePane(P3, windowId("nonexistent"), S1);
        const panes = new Map(model.panes);
        panes.set(P3, orphanPane);
        const bad = { ...model, panes };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "pane-missing-window" && v.paneId === P3), `expected pane-missing-window, got: ${JSON.stringify(violations)}`);
    });
});
describe("checkInvariants — I2: window's sessionId must exist", () => {
    it("reports window-missing-session when window references a non-existent session", () => {
        const model = validModel();
        const orphanWin = makeWindow(W2, sessionId("nonexistent"), [], null);
        const windows = new Map(model.windows);
        windows.set(W2, orphanWin);
        const bad = { ...model, windows };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "window-missing-session" && v.windowId === W2));
    });
});
describe("checkInvariants — I3: window.paneIds entries must exist in model.panes", () => {
    it("reports window-pane-not-in-map for a ghost paneId in window.paneIds", () => {
        ...
        const updatedWin = { ...win, paneIds: [...win.paneIds, ghost] };
        const windows = new Map(model.windows);
        windows.set(W1, updatedWin);
        const bad = { ...model, windows };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "window-pane-not-in-map" && v.windowId === W1 && v.paneId === ghost));
    });
});
describe("checkInvariants — I4: session.windowIds entries must exist in model.windows", () => {
    it("reports session-window-not-in-map for a ghost windowId in session.windowIds", () => {
        ...
        const updatedSess = { ...sess, windowIds: [...sess.windowIds, ghost] };
        const sessions = new Map(model.sessions);
        sessions.set(S1, updatedSess);
        const bad = { ...model, sessions };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "session-window-not-in-map" && v.sessionId === S1 && v.windowId === ghost));
    });
});
describe("checkInvariants — I5: active pane per window", () => {
    it("reports window-no-active-pane when non-empty window has activePaneId null", () => {
        const model = validModel();
        const win = model.windows.get(W1);
        const badWin = { ...win, activePaneId: null };
        const windows = new Map(model.windows);
        windows.set(W1, badWin);
        const bad = { ...model, windows };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "window-no-active-pane" && v.windowId === W1));
    });
    it("reports window-active-pane-missing when activePaneId is not in paneIds", () => {
        const model = validModel();
        const win = model.windows.get(W1);
        const badWin = { ...win, activePaneId: paneId("phantom") };
        const windows = new Map(model.windows);
        windows.set(W1, badWin);
        const bad = { ...model, windows };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "window-active-pane-missing" && v.windowId === W1));
    });
});
describe("checkInvariants — I6: active window per session", () => {
    it("reports session-no-active-window when non-empty session has activeWindowId null", () => {
        const model = validModel();
        const sess = model.sessions.get(S1);
        const badSess = { ...sess, activeWindowId: null };
        const sessions = new Map(model.sessions);
        sessions.set(S1, badSess);
        const bad = { ...model, sessions };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "session-no-active-window" && v.sessionId === S1));
    });
    it("reports session-active-window-missing when activeWindowId is not in windowIds", () => {
        const model = validModel();
        const sess = model.sessions.get(S1);
        const badSess = { ...sess, activeWindowId: windowId("phantom") };
        const sessions = new Map(model.sessions);
        sessions.set(S1, badSess);
        const bad = { ...model, sessions };
        const violations = checkInvariants(bad);
        assert.ok(violations.some((v) => v.kind === "session-active-window-missing" && v.sessionId === S1));
    });
});
describe("checkInvariants — I7: focus consistency", () => {
    ...
    it("reports focus-pane-wrong-window when focus pane is not in focus window", () => {
        ...
        const p3 = makePane(P3, W2, S1);
        const w2 = makeWindow(W2, S1, [P3], P3);
        const sess = model.sessions.get(S1);
        const updatedSess = { ...sess, windowIds: [W1, W2] };
        const windows = new Map(model.windows);
        windows.set(W2, w2);
        const panes = new Map(model.panes);
        panes.set(P3, p3);
...
describe("update helpers — invariant preservation", () => {
    it("addSession → valid model", () => {
        ...
        const sess = makeSession(S1, [], null);
    ...
    it("addWindow + addPane → valid model", () => {
        ...
        const win = makeWindow(W1, S1, [], null);
...
describe("parsedLayoutToWindowLayout", () => {
    ...
    it("converts a two-pane horizontal split layout", () => {
        ...
        if (result.root.kind === "hsplit") {
            ...
            if (left.kind === "pane")
                assert.equal(left.paneId, "p1");
            if (right.kind === "pane")
                assert.equal(right.paneId, "p2");
        }
    });
