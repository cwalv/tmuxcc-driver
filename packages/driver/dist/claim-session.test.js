...
// ---------------------------------------------------------------------------
describe("createSessionClaimer — lost-create-race path (tc-u4ny.2)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("throws CommandError internal when the session cannot be found after a name-collision", async () => {
        ...
        try {
            ...
            const seedResult = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", "lost-race"], { encoding: "utf8", timeout: 5_000 });
            assert.equal(seedResult.status, 0, `seed new-session failed: ${seedResult.stderr}`);
            ...
            const ctx = {
                ...
                lookupByName: (_name) => undefined,
                registerSession: (_tmuxId, name) => ({
                    sessionId: "s1",
                    name,
                }),
                ensureSessionProxy: async (_sessionId, _name) => { },
                onClaimComplete: () => { },
            };
            ...
            const err = await claimer.claim("lost-race").then(() => null, (e) => e);
            assert.ok(isCommandError(err, "internal"), `Expected CommandError("internal") for the lost-race path, got: ${JSON.stringify(err)}`);
            assert.ok(err.message.includes("session.create race"), `Expected "session.create race" in message, got: ${err.message}`);
            ...
            assert.equal(TMUX_SESSION_NAME_TAKEN_CODE, "tmux.duplicate-session", "TMUX_SESSION_NAME_TAKEN_CODE must match the adapter wire code");
        }
        finally {
...
//# sourceMappingURL=claim-session.test.js.map