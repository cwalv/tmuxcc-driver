 ...
 */
async function waitFor(predicate, timeoutMs, what, intervalMs = 30) {
 ...
 */
function spawnTmuxSession(socketName, sessionName) {
    const r = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", sessionName], {
        timeout: 8_000,
    });
    if (r.error || r.status !== 0) {
        throw new Error(`Failed to spawn tmux session "${sessionName}" on socket "${socketName}": ` +
 ...
 */
function triggerNewWindow(socketName, sessionName) {
...
describe("tc-2x3.4: per-session error boundary", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("EB1: fault in session A pipeline does not crash process; session B uninterrupted; trip counted", async () => {
        ...
        const aTopologyNotifyFn = (kind) => {
            if (!_aReadyToFault || _aFaultFired)
                return;
            _aFaultFired = true;
            ...
            throw new Error(`[tc-2x3.4 fault injection] poison notification in session A (kind: ${kind})`);
        };
        // Session B: topology notification tracking (proves B keeps operating).
        let bTopologyNotifyCount = 0;
        const bTopologyNotifyFn = (_kind) => {
        ...
        await waitFor(() => aBoundaryFired, 8_000, "session A boundary trip (onFatalError to fire after poison notification)");
        assert.ok(aBoundaryFired, "A1: session A onFatalError must have fired");
        assert.ok(aBoundaryError instanceof Error &&
            aBoundaryError.message.includes("poison notification in session A"), `A1: onFatalError must receive the thrown error; got: ${String(aBoundaryError)}`);
        ...
        await waitFor(() => bTopologyNotifyCount > bCountAtFaultArm, 8_000, "session B topology notification after session A fault (process must be alive)");
        assert.ok(bTopologyNotifyCount > bCountAtFaultArm, `A2/A4: session B must receive topology notifications after session A's fault; ` +
    ...
    it("EB2: boundary-trip counter increments and is visible in metricsText", async () => {
        ...
        const proxy = createSessionProxy({
            ...
            onFatalError(err) {
                ...
                process.stderr.write(`[EB2-test] boundary trip: ${err instanceof Error ? err.message : String(err)}\n`);
            },
            onTopologyNotify(_kind) {
                if (!_readyToFault || _faultFired)
                    return;
                _faultFired = true;
        ...
        await waitFor(() => boundaryFired, 8_000, "boundary trip to fire (EB2 counter test)");
        ...
        assert.ok(typeof metricsAfter === "string", "metricsText must be a string after the boundary trip");
        assert.ok(metricsAfter.includes("session_boundary_trips_total"), `EB2: session_boundary_trips_total must appear in metricsText after the trip; ` +
        ...
        assert.equal(counterValue, 1, `EB2: session_boundary_trips_total must equal 1 after one boundary trip; got: ${counterValue}`);
    });
    /**
    ...
    it("EB6: boundary trip farewells clients with code internal, not session.unavailable", { timeout: 25_000 }, async () => {
        ...
        assert.ok(fault !== undefined, `EB6: client must receive the fault farewell error{code:"internal"}; got: ${JSON.stringify(received)}`);
        const misattributed = received.find((m) => m.type === "error" && m.code === "session.unavailable");
        assert.equal(misattributed, undefined, `EB6: a boundary trip must NOT emit the designed session-death goodbye ` +
    ...
    it("LIVENESS: hasSessionProxy is true while in-flight AND ready, false before/after", { timeout: 25_000 }, async () => {
        ...
        const supervisor = createSessionProxySupervisor();
    ...
    it("EB4: repeated trip→reattach does not leak listening fds (GAP 1 fd reclamation)", { timeout: 40_000 }, async () => {
        ...
        if (fdsBefore === -1)
            return; // /proc not available — skip assertion
        ...
        assert.ok(leaked <= tolerance, `EB4: open fd count grew by ${leaked} across ${N_CYCLES} trip→reattach cycles ` +
    ...
    it("EB5: circuit breaker quarantines after N rapid trips; clearQuarantine allows reattach", { timeout: 60_000 }, async () => {
        ...
        const supervisorOpts = {
            onTopologyNotify: (kind) => {
                ...
                state.faultCount++;
                throw new Error(`[EB5 fault injection] poison onTopologyNotify (kind: ${kind}, ` +
                    `fault #${state.faultCount})`);
            },
        };
        const supervisor = createSessionProxySupervisor(supervisorOpts);
        ...
        for (let trip = 1; trip <= THRESHOLD; trip++) {
            ...
            assert.ok(!supervisor.quarantinedSessions().has(sessionId), `EB5 G1: session must NOT be quarantined before trip ${trip}`);
            assert.equal(supervisor.aliveCount(), 0, `EB5: aliveCount must be 0 before trip ${trip} (no stale entry)`);
            ...
            await waitFor(() => supervisor.aliveCount() === 0, 8_000, `supervisor to remove entry after trip ${trip} boundary fault`);
        ...
        assert.ok(supervisor.quarantinedSessions().has(sessionId), `EB5 G2: session must be quarantined after ${THRESHOLD} rapid trips`);
        ...
        assert.ok(caughtError instanceof SessionQuarantineError, `EB5 G3: ensureSessionProxy must throw SessionQuarantineError after quarantine; ` +
            `got: ${String(caughtError)}`);
        // ---- G4: aliveCount must be 0 after quarantine (no reattach attempted) ----
        assert.equal(supervisor.aliveCount(), 0, "EB5 G4: aliveCount must remain 0 after quarantine (no session-proxy created)");
        // ---- G5: clearQuarantine allows reattach ----
        supervisor.clearQuarantine(sessionId);
        assert.ok(!supervisor.quarantinedSessions().has(sessionId), "EB5 G5: quarantinedSessions must NOT contain sessionId after clearQuarantine");
        ...
        state.armed = false;
        const proxyAfterClear = await supervisor.ensureSessionProxy(sessionId, sessionName, socketName);
...
//# sourceMappingURL=session-error-boundary.test.js.map