...
describe("Storm flood integration (synthetic)", () => {
    it("8000 events/s sustained (tc-3y8.8 scenario) trips the alarm with per-kind breakdown", () => {
        ...
        const { createFakeClock } = (() => {
            ...
            return {
                createFakeClock: () => ({
                    ...
                    clearTimeout: (h) => { _timers.delete(h); },
                    advance,
                }),
            };
        })();
        ...
        assert.ok(hasLayoutChange || hasSessionsChanged, "breakdown should attribute events to one of the dominant kinds");
        alarm.stop();
    });
    it("metricsRegistry counter accumulates all notification kinds from flood", async () => {
        ...
        for (const kind of kinds) {
            ...
            assert.ok(pattern.test(text), `topology_events_total{kind="${kind}"} should be 1000 in:\n${text}`);
        }
        reg.stop();
    });
...
//# sourceMappingURL=storm-flood.test.js.map