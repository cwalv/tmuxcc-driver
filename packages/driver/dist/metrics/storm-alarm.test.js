...
describe("StormAlarm", () => {
    ...
    it("trip breakdown contains per-kind counts", () => {
        ...
        assert.ok((breakdown.get("layout-change") ?? 0) >= 30, `layout-change count should be >= 30; got ${breakdown.get("layout-change")}`);
        assert.ok((breakdown.get("window-renamed") ?? 0) >= 25, `window-renamed count should be >= 25; got ${breakdown.get("window-renamed")}`);
        alarm.stop();
    });
    // -------------------------------------------------------------------------
...
//# sourceMappingURL=storm-alarm.test.js.map