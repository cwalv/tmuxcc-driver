...
describe("SessionProxyRegistry", () => {
    it("incTopologyEvent increments the counter and appears in text exposition", async () => {
        ...
        assert.ok(text.includes("topology_events_total"), "text should contain topology_events_total");
        // The Prometheus text for a counter with label kind="layout-change" value 2.
        assert.ok(text.includes('kind="layout-change"') && text.match(/kind="layout-change"\} 2/), `text should show layout-change=2; got:\n${text}`);
        assert.ok(text.includes('kind="window-add"'), "text should contain window-add counter");
        reg.stop();
    });
    it("undefined kind is recorded as 'unknown'", async () => {
        ...
        reg.incTopologyEvent(undefined);
        const text = await reg.metrics();
        assert.ok(text.includes('kind="unknown"'), "undefined kind should appear as 'unknown' label");
        reg.stop();
    });
    it("incCommandsIssued increments counter", async () => {
        ...
        reg.incCommandsIssued();
        const text = await reg.metrics();
        assert.ok(text.includes("commands_issued_total"), "text should contain commands_issued_total");
        assert.ok(text.match(/commands_issued_total \d+/), "commands_issued_total should have a numeric value");
        reg.stop();
    });
    it("incDeltasFannedOut increments per-client counter", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("deltas_fanned_out_total"), "text should contain deltas_fanned_out_total");
        assert.ok(text.includes('client="c1"'), "text should contain c1 client label");
        reg.stop();
    });
    it("observeCommandRoundTrip adds to histogram and labels by kind", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("command_round_trip_seconds"), "text should contain command_round_trip_seconds histogram");
        assert.ok(text.includes("command_round_trip_seconds_count"), "text should contain histogram count");
        assert.ok(text.includes('kind="list-windows"'), "text should contain kind=list-windows label");
        assert.ok(text.includes('kind="list-panes"'), "text should contain kind=list-panes label");
        reg.stop();
    });
    it("observeNotifyToDelta records the edge label", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("topology_notify_to_delta_seconds"), "text should contain topology_notify_to_delta_seconds histogram");
        assert.ok(text.includes('edge="leading"'), "text should contain edge=leading label");
        assert.ok(text.includes('edge="trailing"'), "text should contain edge=trailing label");
        assert.ok(text.includes('edge="heartbeat"'), "text should contain edge=heartbeat label");
        reg.stop();
    });
    it("incDeltasEmitted increments the counter by the given amount", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("deltas_emitted_total"), "text should contain deltas_emitted_total");
        assert.ok(text.match(/deltas_emitted_total 5/), `deltas_emitted_total should be 5; got:\n${text}`);
        reg.stop();
    });
    it("incOutputBytes + observeOutputFrameSize: aggregate, no per-pane labels", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("output_bytes_total"), "text should contain output_bytes_total");
        assert.ok(text.match(/output_bytes_total 1088/), `output_bytes_total should be 1088; got:\n${text}`);
        assert.ok(text.includes("output_frame_size_bytes"), "text should contain output_frame_size_bytes histogram");
        ...
        assert.ok(!/output_bytes_total\{[^}]*pane=/.test(text) &&
            !/output_frame_size_bytes\{[^}]*pane=/.test(text), "aggregate metrics must not carry a `pane` label (cardinality rule)");
        reg.stop();
    });
    it("notePauseEntered/Exited refcount and accumulator are well-formed", async () => {
        ...
        reg.notePauseExited();
        const text = await reg.metrics();
        assert.ok(text.includes("session_paused_seconds_total"), "text should contain session_paused_seconds_total");
        reg.stop();
    });
    it("default metrics (event-loop lag, GC, heap) are present in exposition", async () => {
        ...
        assert.ok(text.includes("nodejs_eventloop_lag_seconds") ||
            text.includes("nodejs_eventloop_lag"), "default metrics should include eventloop lag (load-bearing for tc-2x3)");
        assert.ok(text.includes("process_cpu_user_seconds_total") ||
            text.includes("process_cpu_seconds_total"), "default metrics should include process CPU");
        assert.ok(text.includes("nodejs_heap_size_total_bytes") ||
            text.includes("nodejs_heap_size_used_bytes"), "default metrics should include heap size");
        reg.stop();
    });
    it("classifyCommand: bounded vocabulary; unknown commands collapse to 'unknown'", () => {
        ...
        assert.equal(classifyCommand("set-option -g monitor-bell on"), "set-option");
        assert.equal(classifyCommand("refresh-client -A '%1:pause'"), "refresh-client");
        assert.equal(classifyCommand("send-keys -t %1 hello"), "send-keys");
        assert.equal(classifyCommand("attach-session -t mysession"), "attach-session");
    ...
    it("incRequeryCycle records per-trigger attribution", async () => {
        ...
        assert.ok(text.match(/trigger="leading"\} 3/), `leading should be 3; got:\n${text}`);
    ...
    it("incRequeryHeartbeatChange increments the expected-zero tripwire", async () => {
        ...
        assert.ok(text.match(/requery_heartbeat_changes_total 2/), `expected 2 heartbeat-change incidents; got:\n${text}`);
        reg.stop();
    });
    it("observeRequeryRoundTrip records a unimodal-shaped histogram (no labels)", async () => {
        ...
        assert.ok(!/requery_round_trip_seconds_count\{[^}]*kind=/.test(text), "requery_round_trip_seconds must be unlabelled (cardinality rule)");
        reg.stop();
    });
    ...
    it("incPretopologyDroppedBytes records bytes by provenance (the F4 tripwire)", async () => {
        ...
        assert.ok(text.match(/provenance="owned"\} 1536/), `owned should be 1536; got:\n${text}`);
        assert.ok(text.match(/provenance="foreign"\} 8192/), `foreign should be 8192; got:\n${text}`);
        reg.stop();
    });
    ...
    it("observeDeltasPerCycle: zero-delta cycles are observed (the no-change rate)", async () => {
        ...
        assert.ok(text.match(/deltas_per_cycle_count 4/), `expected 4 observations after one defensive no-op; got:\n${text}`);
        reg.stop();
    });
    it("noteFlowDrainClamped increments the expected-zero tripwire (tc-d7i)", async () => {
        ...
        reg.noteFlowDrainClamped();
        const text = await reg.metrics();
        assert.ok(text.match(/flow_drain_clamped_total 2/), `flow_drain_clamped_total should be 2; got:\n${text}`);
        reg.stop();
    });
    it("noteFlowBytesWhilePaused accumulates bytes; non-positive is a no-op (tc-d7i)", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.match(/flow_bytes_while_paused_total 5460/), `flow_bytes_while_paused_total should be 5460; got:\n${text}`);
        reg.stop();
    });
    it("noteFlowCommandFailed attributes by kind (tc-d7i)", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.match(/flow_commands_failed_total\{kind="continue"\} 2/), `continue failures should be 2; got:\n${text}`);
        assert.ok(text.match(/flow_commands_failed_total\{kind="pause"\} 1/), `pause failures should be 1; got:\n${text}`);
        reg.stop();
    });
    it("incBoundaryQuarantine increments the expected-zero tripwire (tc-m2y8)", async () => {
        ...
        const text = await reg.metrics();
        assert.ok(text.includes("session_boundary_quarantined_total"), "text should contain session_boundary_quarantined_total");
        assert.ok(text.match(/session_boundary_quarantined_total 1/), `quarantine count should be 1; got:\n${text}`);
        // Companion counter is present and independent.
        assert.ok(text.match(/session_boundary_trips_total 2/), `trips should be 2 (independent of quarantine); got:\n${text}`);
        reg.stop();
    });
    it("multiple registries are isolated (no cross-contamination)", async () => {
        ...
        assert.ok(text1.match(/kind="layout-change"\} 2/), "reg1 should have layout-change=2");
        ...
        assert.ok(!text2.match(/kind="layout-change"\} 2/), "reg2 should not be contaminated by reg1");
        reg1.stop();
        reg2.stop();
    });
});
