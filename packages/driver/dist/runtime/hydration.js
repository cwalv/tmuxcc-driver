 ...
 * transport.
 */
export const CLEAR_AND_SCROLLBACK = new Uint8Array([
 ...
 */
export function hydrateTransport(pipeline, transport, paneIds, sentinels) {
    const tasks = [];
 ...
 */
export function hydratePane(pipeline, transport, pid, sentinels) {
 ...
 */
export async function captureText(pipeline, pid) {
    // PaneId convention (ids.ts): "p" + tmux pane number. Mirror of _hydrateOnePane.
    const s = pid;
    if (s.length < 2 || s.charCodeAt(0) !== 0x70 /* 'p' */)
        return { ok: false };
    const tmuxN = parseInt(s.slice(1), 10);
    if (!Number.isFinite(tmuxN))
        return { ok: false };
...
async function _hydrateOnePane(pipeline, transport, pid, sentinels) {
    ...
    try {
        ...
        try {
            ...
            if (factsResult.ok)
                facts = parsePaneGridFacts(factsResult.body);
        }
        catch {
...
async function _deliverReplay(transport, pid, body, facts) {
    ...
    const cursor = facts !== null
        ...
        : EMPTY_BYTES;
    const combined = new Uint8Array(CLEAR_AND_SCROLLBACK.length + replay.length + cursor.length);
 ...
 * Exported for direct unit testing.
 */
export function parsePaneGridFacts(body) {
 ...
 * Exported for direct unit testing.
 */
export function cursorPositionEscape(cursorX, cursorY) {
    const row = cursorY + 1;
    const col = cursorX + 1;
 ...
 * Exported for direct unit testing.
 */
export function stripOneTrailingLf(src) {
