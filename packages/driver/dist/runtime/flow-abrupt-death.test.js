...
async function drainClampedTotal(sessionProxy) {
...
describe("tc-76m8.27: FC-1 accounting survives abrupt client death (real tmux)", { skip: !tmuxAvailable ? "tmux not found on PATH" : false }, () => {
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    it("T1: SIGSTOP+flood+SIGKILL analogue over a real socket does not trip the clamp; session-proxy stays healthy", { timeout: 60_000 }, async () => {
        ...
        try {
            ...
            await waitFor(() => (backpressuredSends > 0 ? true : undefined), 30_000, () => `server-side transport never backpressured; ` +
            ...
            await waitFor(() => (sessionProxy.server.clientCount() === 0 ? true : undefined), 10_000, () => `dead client never removed; clientCount=${sessionProxy.server.clientCount()}`);
            ...
            assert.equal(clamped, 0, `flow_drain_clamped_total must stay 0 across an abrupt client death; got ${clamped} ` +
    ...
    // -----------------------------------------------------------------------
    it("T2: deferred drain credits released by the close path do not debit the discarded ledger", { timeout: 60_000 }, async () => {
        ...
        try {
            ...
            function die() {
                ...
                drainResolve = null;
                if (r !== null)
                    r(); // deferred credits become queued microtasks…
                for (const h of closeHandlers)
                    h(); // …close handlers run first, synchronously
                closeHandlers.clear();
            }
            ...
            await runClientHandshake(clientTransport, CLIENT_CAPS);
            clientTransport.onControl((m) => clientMsgs.push(m));
            await addP;
            await waitFor(() => (clientMsgs.some((m) => m.type === "pane.hydration.end") ? true : undefined), 10_000, () => `hydration never completed; saw: ${clientMsgs.map((m) => m.type).join(",")}`);
            const pid = firstPaneId(sessionProxy);
            ...
            const clamped = await drainClampedTotal(sessionProxy);
            assert.equal(clamped, 0, `flow_drain_clamped_total must stay 0 when close releases the deferred credits; got ${clamped}`);
            assert.equal(sessionProxy.server.clientCount(), 0, "dead client must be removed by the close handlers");
            assert.equal(fc.bufferedBytes(pid), 0, "removeClient must fully reconcile the dead client's ledger");
        }
        finally {
            sessionProxy.kill();
            killTmuxServer(sock);
...
//# sourceMappingURL=flow-abrupt-death.test.js.map