 ...
 */
function buildBootstrapReplies(opts = {}) {
...
describe("Pipeline: bootstrap (requery-on-attach)", () => {
    ...
    it("scopes list-* to the bound session when sessionName is supplied (tc-tfv.3, tc-3y8.9)", async () => {
        ...
        assert.ok(written[0].includes("-t =mysession"), `list-windows must be scoped to the bound session, got: ${written[0]}`);
        assert.ok(written[1].includes("list-panes"), `got: ${written[1]}`);
        assert.ok(written[1].includes("-t =mysession"), `list-panes must be scoped to the bound session, got: ${written[1]}`);
...
describe("Pipeline: live requery on topology events", () => {
    it("a topology notification fires a requery, and the new snapshot lands in the model", async () => {
        ...
        const written = host.popWritten();
        assert.ok(written.some((w) => w.startsWith("list-windows")), `requery must issue list-windows, got: ${JSON.stringify(written)}`);
        assert.ok(written.some((w) => w.startsWith("list-panes")), `requery must issue list-panes, got: ${JSON.stringify(written)}`);
...
describe("Pipeline: bootstrap-race — topology notification during bootstrap triggers healing requery", () => {
    it("topology notification injected before bootstrap replies → healing requery issued after drain → updated snapshot lands in the model", async () => {
        ...
        assert.ok(writtenAfterBootstrap.some((w) => w.startsWith("list-windows")), `healing requery must issue list-windows; got: ${JSON.stringify(writtenAfterBootstrap)}`);
        assert.ok(writtenAfterBootstrap.some((w) => w.startsWith("list-panes")), `healing requery must issue list-panes; got: ${JSON.stringify(writtenAfterBootstrap)}`);
        // Feed the updated snapshot: two windows now (the raced notification was real).
        host.pushData(buildTwoWindowReplies());
        await new Promise((r) => setImmediate(r));
        const model = pipeline.getModel();
        assert.equal(model.windows.size, 2, "healing requery must update model to reflect the raced topology change");
    ...
    it("topology notification during bootstrap increments the onTopologyNotify counter", async () => {
        ...
        host.pushData(buildTwoWindowReplies());
        await new Promise((r) => setImmediate(r));
        assert.ok(topologyKinds.includes("window-add"), `onTopologyNotify must have been called with "window-add"; got: ${JSON.stringify(topologyKinds)}`);
        pipeline.stop();
    });
});
// ---------------------------------------------------------------------------
...
describe("Pipeline: post-bootstrap setup commands", () => {
    it("writes set-option -wg monitor-activity on after bootstrap completes", async () => {
        ...
        assert.ok(postBootstrapWrites.some((w) => w === monitorActivityCmd), `expected ${JSON.stringify(monitorActivityCmd)} after bootstrap; got: ${JSON.stringify(postBootstrapWrites)}`);
        pipeline.stop();
    });
    it("registers refresh-client -B sync-watch subscription after bootstrap", async () => {
        ...
        assert.ok(postBootstrapWrites.some((w) => w === syncWatchCmd), `expected sync-watch subscription command; got: ${JSON.stringify(postBootstrapWrites)}`);
        pipeline.stop();
    });
    it("registers refresh-client -B title-watch (%* pane_title) subscription after bootstrap (tc-s6ov.4)", async () => {
        ...
        assert.ok(postBootstrapWrites.some((w) => w === titleWatchCmd), `expected title-watch subscription command; got: ${JSON.stringify(postBootstrapWrites)}`);
        pipeline.stop();
    });
    it("does NOT write monitor-activity / sync-watch before bootstrap resolves", async () => {
        ...
        const monitorActivityCmd = `set-option -wg monitor-activity on\n`;
        assert.ok(!preBootstrapWrites.some((w) => w === monitorActivityCmd), `monitor-activity must not be written pre-bootstrap; got: ${JSON.stringify(preBootstrapWrites)}`);
        host.pushData(buildBootstrapReplies());
        await startPromise;
        pipeline.stop();
    });
...
describe("Pipeline: sync-watch subscription (tc-7xv.28)", () => {
    function syncWatchLine(tmuxWindowNum, value) {
...
describe("Pipeline: title-watch subscription (tc-s6ov.4)", () => {
    ...
    function titleWatchLine(tmuxPaneNum, value) {
    ...
    it("an out-of-band title change (no %output) still updates paneTitle", async () => {
        ...
        assert.equal(pipeline.getModel().panes.get(paneId("p1")).paneTitle, "set-by-other-client");
        pipeline.stop();
    });
    it("preserves leading/trailing spaces and does not trim the value", async () => {
        ...
        assert.equal(pipeline.getModel().panes.get(paneId("p1")).paneTitle, " spaced title ");
        pipeline.stop();
    });
