...
import { paneId, windowId, WIRE_PROTOCOL_VERSION } from "@tmuxcc/protocol";
...
function makeInput(paneSuffix, data) {
    return {
        type: "input",
        seq: nextSeq(),
...
function makeResize(paneSuffix, cols, rows) {
    return {
        type: "resize.request",
        seq: nextSeq(),
        paneId: paneId("p" + paneSuffix),
...
describe("defaultPaneIdToTmux", () => {
    ...
    it("throws TypeError for non-'p' prefix ids", () => {
        assert.throws(() => defaultPaneIdToTmux(paneId("x5")), (err) => err instanceof TypeError && /p<N>/.test(err.message));
        assert.throws(() => defaultPaneIdToTmux(paneId("")), TypeError);
    });
    it("throws TypeError for tmux-format pane id '%1' (wrong format)", () => {
        ...
        assert.throws(() => defaultPaneIdToTmux(paneId("%1")), (err) => err instanceof TypeError &&
            /%1/.test(err.message));
    });
});
describe("defaultWindowIdToTmux", () => {
    ...
    it("throws TypeError for non-'w' prefix ids", () => {
        assert.throws(() => defaultWindowIdToTmux(windowId("p3")), (err) => err instanceof TypeError && /w<N>/.test(err.message));
    });
    it("throws TypeError for tmux-format window id '@9' (wrong format)", () => {
        ...
        // Passing a tmux-format id is a programming error that should surface loudly.
        assert.throws(() => defaultWindowIdToTmux(windowId("@9")), (err) => err instanceof TypeError &&
            /@9/.test(err.message));
    });
});
// ---------------------------------------------------------------------------
...
describe("createInputPath — InputMessage", () => {
    ...
    it("invalid pane id throws TypeError", () => {
        ...
        assert.throws(() => path.handleClientMessage(badMsg), (err) => err instanceof TypeError && /p<N>/.test(err.message));
...
describe("createInputPath — custom paneIdToTmux option", () => {
    it("uses the provided mapping function instead of the default", () => {
        ...
        const path = createInputPath(host, {
            paneIdToTmux: (id) => {
                if (id === paneId("p1"))
                    return 100;
                throw new TypeError(`custom mapper: unrecognized pane id "${id}"`);
            },
        });
    ...
    it("throws when custom mapping throws for unrecognized id", () => {
        ...
        assert.throws(() => path.handleClientMessage(makeInput("1", "x")), TypeError);
        assert.equal(host.writes.length, 0, "no partial write on bad pane id");
    });
});
// ---------------------------------------------------------------------------
...
describe("createInputPath — break-pane command (tc-7xv.9)", () => {
    ...
    it("invalid pane id throws TypeError", () => {
        ...
        assert.throws(() => path.handleClientMessage(msg), (err) => err instanceof TypeError && /p<N>/.test(err.message));
        assert.equal(host.writes.length, 0, "no partial write on bad pane id");
    });
});
// ---------------------------------------------------------------------------
...
describe("createInputPath — creating verbs return effect ids (tc-ozk.1)", () => {
    ...
    it("tc-yudx: %error with a body → respond ok=false carrying the VERBATIM tmux refusal text", async () => {
        ...
        const path = createInputPath(host);
        const results = [];
        host.enqueueSendResult(Promise.resolve({ ok: false, body: enc("create pane failed: pane too small") }));
        path.handleClientMessage({
            type: "command.request",
            seq: nextSeq(),
            correlationId: "v4b",
...
describe("createInputPath — break-pane empty -P body no-op (tc-0c30.20)", () => {
    ...
    function makeSolePaneModel(paneSuffix, winSuffix) {
        const sid = makeSessionId("s0");
        const wid = makeWindowId("w" + winSuffix);
        const pid = makePaneId("p" + paneSuffix);
        const session = { sessionId: sid, name: "sess", windowIds: [wid], activeWindowId: wid };
        const window = {
            windowId: wid,
            sessionId: sid,
            name: "win",
...
describe("createInputPath — set-monitor-activity (tc-7xv.15)", () => {
    ...
    it("set-monitor-activity with invalid window id throws TypeError", () => {
        ...
        assert.throws(() => path.handleClientMessage(msg), (err) => err instanceof TypeError && /w<N>/.test(err.message));
...
// ---------------------------------------------------------------------------
import { emptyModel, addSession, addWindow, addPane, paneId as makePaneId, windowId as makeWindowId, sessionId as makeSessionId, } from "../state/model.js";
/**
...
function makeReversalModel(opts) {
    const sid = makeSessionId("s0");
    const wid = makeWindowId("w" + opts.windowSuffix);
    const pid = makePaneId("p" + opts.windowSuffix + "00");
    const session = { sessionId: sid, name: "sess", windowIds: [wid], activeWindowId: wid };
    const window = {
        windowId: wid,
        sessionId: sid,
 ...
 */
function makeResolvingFakeDeps() {
...
describe("createInputPath — tc-n4ct send-keys -H chunking", () => {
    it("payload of exactly 5000 bytes (boundary) emits exactly 1 send-keys -H chunk", async () => {
        ...
        assert.ok(host.writes[0]?.startsWith("send-keys -H -t %1 "), `expected send-keys -H -t %1 …, got: ${host.writes[0]?.slice(0, 40)}`);
    ...
    it("51200-byte payload (original bug: 50KB paste) emits 11 ordered chunks", async () => {
        ...
        assert.equal(host.writes.length, expectedChunks, `51200-byte input must produce ${expectedChunks} chunks, got ${host.writes.length}`);
    ...
    it("byte order is preserved across chunk boundaries (sequential hex content)", async () => {
        ...
        for (let i = 0; i < PAYLOAD_BYTES; i++) {
            ...
            assert.equal(allTokens[i], expected, `token at position ${i}: expected ${expected}, got ${allTokens[i]}`);
        }
    });
});
// ---------------------------------------------------------------------------
...
describe("createInputPath — tc-n4ct cross-message ordering", () => {
    ...
    async function drainUntil(host, expected) {
    ...
    it("input arriving during a multi-chunk send queues behind it (no interleave)", async () => {
        ...
        const tokens = (w) => w.replace(/^send-keys -H -t %\d+ /, "").trim().split(" ");
...
describe("createInputPath — runAckVerb ACK path (tc-u7cu.8)", () => {
    ...
    it("close-pane + %error → respond ok=false with verbatim tmux error body", async () => {
        ...
        const path = createInputPath(host);
        const results = [];
        host.enqueueSendResult(Promise.resolve({ ok: false, body: enc("can't find pane: %99") }));
        path.handleClientMessage({
            type: "command.request",
            seq: nextSeq(),
    ...
    it("close-pane + %error with no body → respond ok=false with generic message", async () => {
        ...
        if (results[0].result.ok === false) {
            assert.equal(results[0].result.code, "verb.failed");
            assert.ok(results[0].result.message.includes("close-pane"), `generic fallback message should mention the verb kind; got: ${results[0].result.message}`);
        }
    });
    ...
    it("kill-session + %error → respond ok=false with verbatim body", async () => {
        ...
        const path = createInputPath(host);
        const results = [];
        host.enqueueSendResult(Promise.resolve({ ok: false, body: enc("can't find session: =nosuchsession") }));
        path.handleClientMessage({
            type: "command.request",
            seq: nextSeq(),
...
//# sourceMappingURL=input-path.test.js.map