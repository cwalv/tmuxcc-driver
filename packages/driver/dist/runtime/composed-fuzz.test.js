...
import { emptyModel, paneId, sessionId, windowId, } from "../state/model.js";
...
function makeFuzzClock() {
    ...
    const clock = {
        now: () => _now,
        setTimeout: (fn, ms) => {
            const t = { id: _nextId++, fireAt: _now + Math.max(0, ms), fn };
            timers.push(t);
            return t.id;
        },
        clearTimeout: (handle) => {
    ...
    async function advance(ms, hook) {
 ...
 * vocabulary).
 */
function mutateWorld(w, rng) {
    const MAX_WINDOWS = 5;
    const choices = ["window-renamed", "window-pane-changed", "layout-change"];
    if (w.windows.length < MAX_WINDOWS)
        choices.push("window-add");
    if (w.windows.length > 1)
        choices.push("window-close");
    ...
    switch (kind) {
        ...
        case "window-close": {
            ...
            if (removed.active && w.windows.length > 0)
                w.windows[0].active = true;
            break;
        }
        case "window-renamed": {
            const idx = rng.int(w.windows.length);
...
class FuzzTmuxHost {
    ...
    stats = { totalWrites: 0, windowWrites: 0, paneWrites: 0 };
    /**
     ...
     */
    cycleStartStamps = [];
    constructor(_clock, _world, _rng, _opts) {
        this._clock = _clock;
        this._world = _world;
        this._rng = _rng;
        this._opts = _opts;
    ...
    pushNotification(kind, windowNum) {
        if (this._exited)
            return;
    ...
    stop() { this._exited = true; return Promise.resolve(); }
    kill() { this._exited = true; }
    pendingReplies() { return this._replyQueue.length; }
}
function makeCommandBlock(cmdNum, body) {
    const ts = 1000000;
    return `%begin ${ts} ${cmdNum} 1\r\n${body}%end ${ts} ${cmdNum} 1\r\n`;
}
function makeErrorBlock(cmdNum) {
    const ts = 1000000;
    return `%begin ${ts} ${cmdNum} 1\r\n%error ${ts} ${cmdNum} 1\r\n`;
}
...
async function runOneSeed(seed) {
...
async function runOneSeedInner(seed) {
...
async function raceWithBudget(p, step, maxIter) {
    let settled = false;
 ...
 * catches it.
 */
function assertCycleRateBounded(r) {
    const ALLOWED_PER_WINDOW = 1 + 5; // 1 + CYCLE_BUDGET
    const stamps = r.cycleStartStamps;
    for (let i = 0; i < stamps.length; i++) {
        const windowEnd = stamps[i] + r.ceilingMs;
        let count = 0;
        for (let j = i; j < stamps.length && stamps[j] <= windowEnd; j++)
            count++;
        assert.ok(count <= ALLOWED_PER_WINDOW, `seed ${r.seed}: cycle rate invariant violated — ${count} cycles starting in [${stamps[i]}, ${windowEnd}] ` +
...
function assertConvergence(r) {
    ...
    assert.deepEqual(servedSnap, worldSnap, `seed ${r.seed}: convergence — served model does not match synthetic world after quiet.\n` +
    ...
    if (r.finalWorld.windows.length > 0) {
        assert.ok(servedSessEntry !== undefined, `seed ${r.seed}: served model has no session even though world has ${r.finalWorld.windows.length} windows`);
        assert.equal(servedSessEntry.name, r.finalWorld.sessionName, `seed ${r.seed}: served session name "${servedSessEntry.name}" does not match world "${r.finalWorld.sessionName}"`);
    }
    ...
    if (r.observations.length === 0)
        return; // empty run — nothing to replay
    ...
    assert.deepEqual(replayed, finalSnap, `seed ${r.seed}: cumulative delta replay does not match final served snapshot.\n` +
 ...
 */
function assertNoStarvation(r) {
    assert.equal(r.pendingRepliesAtEnd, 0, `seed ${r.seed}: starvation — ${r.pendingRepliesAtEnd} list-* replies still pending in host queue`);
    assert.ok(r.pendingTimersAtEnd <= 1, `seed ${r.seed}: starvation — ${r.pendingTimersAtEnd} timers still armed after quiet ` +
...
// ===========================================================================
function applyDeltasToSnapshot(snap, deltas) {
    ...
    for (const delta of deltas) {
        switch (delta.type) {
            ...
            case "pane.closed":
                ...
                break;
            case "pane.resized":
                panes = panes.map((p) => p.paneId === delta.paneId ? { ...p, cols: delta.cols, rows: delta.rows } : p);
                break;
...
function worldToSnapshotMessage(w) {
    ...
    return {
        ...
        panes: paneList,
        focus: {
            paneId: activeWin ? paneId("p" + activeWin.paneNum) : "",
            windowId: activeWin ? windowId("w" + activeWin.windowNum) : "",
        },
    };
}
// ===========================================================================
    ...
    it(`drives ${N_SEEDS} seeded schedules through the composed engine + coalescer + drain`, async () => {
        ...
        if (FIXED_SEED !== undefined) {
            ...
            for (let i = 0; i < N_SEEDS; i++)
                seeds.push((RUN_SALT ^ (i * 0x9E37_79B1)) >>> 0);
        }
        ...
        if (failures.length > 0) {
            const f = failures[0];
            const msg = `Fuzz failure at seed ${f.seed}. Replay with:\n` +
                `  SP_FUZZ_SEED=${f.seed} SP_FUZZ_N=1 node --import tsx --test src/runtime/composed-fuzz.test.ts\n` +
                `Original error: ${f.error?.message ?? String(f.error)}`;
...
void emptyModel;
//# sourceMappingURL=composed-fuzz.test.js.map