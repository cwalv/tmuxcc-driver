 ...
 */
export function killTmuxServer(sock) {
 ...
 */
export function trackSocket(sock) {
    if (!SOCKET_REGEX.test(sock)) {
        throw new Error(`trackSocket: socket name must match /^tmuxcc-test-\\d+-/ ` +
 ...
 */
export function forgetSocket(sock) {
    tracked.delete(sock);
}
/**
 ...
 */
export function getTrackedSockets() {
    return [...tracked];
}
/**
 ...
 */
export function flushAllTracked() {
 ...
 */
export function sweepOrphanedSockets() {
    ...
    for (const entry of entries) {
        if (!entry.startsWith(SOCKET_PREFIX))
            continue;
...
function isPidAlive(pid) {
    try {
        ...
        if (code === "EPERM")
            return true; // Alive but unpermissioned.
...
//# sourceMappingURL=test-tmux-cleanup.js.map