...
import { sendKeysHex, refreshClientSize, splitWindow, newWindow, breakPane, parseEffectIds, setOptionForWindow, setOptionForPane, unsetOptionForPane, unsetOptionForWindow, setOptionForSession, unsetOptionForSession, setWindowSizeManual, setWindowSizeDefault, resizeWindow, resizePane as resizePaneCmd, } from "../parser/commands.js";
import { PHASE_TIMING_ENABLED, phaseLog } from "./phase-timing.js";
import { TMUXCC_LABEL_OPTION, paneBoundOptionName, TMUXCC_DETACH_OPTION, TMUXCC_ICON_OPTION, } from "../state/bootstrap.js";
// ---------------------------------------------------------------------------
// Id-mapping helpers
// ---------------------------------------------------------------------------
/**
 ...
 * so "%1" becomes "p1".
 */
function defaultPaneIdToTmux(id) {
    const s = id;
    if (!s.startsWith("p")) {
        throw new TypeError(`defaultPaneIdToTmux: expected internal model PaneId format "p<N>" (e.g. "p1"); ` +
    ...
    if (Number.isNaN(n)) {
        throw new TypeError(`defaultPaneIdToTmux: could not parse numeric suffix from PaneId "${s}".`);
    }
    return n;
}
/**
 ...
 * so "@9" becomes "w9".
 */
function defaultWindowIdToTmux(id) {
    const s = id;
    if (!s.startsWith("w")) {
        throw new TypeError(`defaultWindowIdToTmux: expected internal model WindowId format "w<N>" (e.g. "w3"); ` +
    ...
    const n = parseInt(s.slice(1), 10);
    if (Number.isNaN(n)) {
        throw new TypeError(`defaultWindowIdToTmux: could not parse numeric suffix from WindowId "${s}".`);
    }
    return n;
}
// ---------------------------------------------------------------------------
 ...
 */
function buildPanePolicyEvent(paneId, option, value, clientId) {
    switch (option) {
        case "bound":
 ...
 */
function buildPanePolicyRevert(paneId, option, prev, clientId) {
    switch (option) {
        case "bound":
            return {
                kind: "internal:set-pane-policy",
 ...
 */
function firstSessionName(model) {
 ...
 * ```
 */
export function createInputPath(deps, opts = {}) {
     ...
     */
    async function sendInputChunked(tmuxPaneNum, bytes) {
     ...
     */
    let inputTail = null;
    function enqueueInput(tmuxPaneNum, bytes) {
        const prev = inputTail;
        const chained = (prev === null
     ...
     */
    function sendCommandWithReversal(cmd, optimistic, reverse, ackOpts) {
     ...
     */
    function runCreatingVerb(respond, correlationId, verbKind, cmd, emptyBodyFallback) {
        ...
        void resultPromise.then((result) => {
            if (!result.ok) {
                ...
                const errorBody = result.body !== undefined
            ...
            if (ids === null) {
                ...
                if (bodyText.trim() === "" && emptyBodyFallback !== undefined) {
                    ...
                    if (fallback !== null) {
                        console.info(`[input-path] ${verbKind} returned %end with an empty -P -F body — ` +
     ...
     * `sendCommand` path.
     */
    function runAckVerb(respond, correlationId, verbKind, cmd) {
        ...
        const resultPromise = sendCommand(cmd);
        if (respond === undefined)
            return; // no return path wired — fire-and-forget.
        void resultPromise.then((result) => {
            if (!result.ok) {
                const errorBody = result.body !== undefined
                    ? new TextDecoder().decode(result.body).trim()
    ...
    function handleClientMessage(msg, respond, clientId) {
        switch (msg.type) {
            // -----------------------------------------------------------------------
            // input → send-keys -H
            //
            ...
            case "command.request": {
                ...
                switch (command.kind) {
                    ...
                    case "set-synchronize-panes": {
                        ...
                        const on = command.on;
                        sendCommandWithReversal(setOptionForWindow(tmuxWinNum, "synchronize-panes", on ? "on" : "off"), { kind: "internal:set-window-sync", windowId: wid, on }, (before) => {
                            const prev = before.windows.get(wid);
                            if (prev === undefined)
                                return null; // window gone — nothing to revert.
                    ...
                    case "set-monitor-activity": {
                        ...
                        const wid = command.windowId;
                        const on = command.on;
                        sendCommandWithReversal(setOptionForWindow(tmuxWinNum, "monitor-activity", on ? "on" : "off"), { kind: "internal:set-window-monitor-activity", windowId: wid, on }, (before) => {
                            const prev = before.windows.get(wid);
                            if (prev === undefined)
                                return null;
                    ...
                    case "set-monitor-silence": {
                        ...
                        sendCommandWithReversal(setOptionForWindow(tmuxWinNum, "monitor-silence", String(secondsVal)), { kind: "internal:set-window-monitor-silence", windowId: wid, seconds: secondsVal }, (before) => {
                            const prev = before.windows.get(wid);
                            if (prev === undefined)
                                return null;
                    ...
                    case "rename-pane": {
                        ...
                        sendCommandWithReversal(setOptionForPane(tmuxPaneNum, TMUXCC_LABEL_OPTION, command.title), { kind: "internal:set-pane-label", paneId: pid, label: newLabel }, (before) => {
                            const prev = before.panes.get(pid);
                            if (prev === undefined)
                                return null; // pane gone — nothing to revert.
                    ...
                    case "set-object-policy": {
                        ...
                        const optionName = command.option === "bound" ? paneBoundOptionName(clientId)
                        ...
                        if (command.scope === "pane") {
                            ...
                            const cmd = clear
                                ...
                                : setOptionForPane(tmuxPaneNum, optionName, command.value);
                            ...
                            sendCommandWithReversal(cmd, optimistic, (before) => {
                                const prev = before.panes.get(pid);
                                if (prev === undefined)
                                    return null; // pane gone — nothing to revert.
                        ...
                        if (command.scope === "window") {
                            ...
                            const cmd = clear
                                ...
                                : setOptionForWindow(tmuxWinNum, optionName, command.value);
                        ...
                        const cmd = clear
                            ...
                            : setOptionForSession(sessionName, optionName, command.value);
                        runAckVerb(respond, correlationId, "set-object-policy", cmd);
                        break;
                    }
...
//# sourceMappingURL=input-path.js.map