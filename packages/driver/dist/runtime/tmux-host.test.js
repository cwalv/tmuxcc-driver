...
describe("TmuxHost — real tmux 3.4", { skip: !tmuxAvailable ? "tmux not found on PATH" : false }, () => {
    ...
    it("a stale write stream is retired on pty death — no EBADF queue drop (tc-76m8.20)", async () => {
        ...
        assert.deepEqual(ptyWriteErrors, [], "a write against a dead pty must be dropped by the retired write stream, not dispatched against the closed fd");
    });
    it("a stale write stream cannot inject into a recycled pty fd (tc-76m8.20)", async () => {
        ...
        await hostB.start();
        for (let attempt = 0; hostB._pty.fd !== fdA && attempt < 3; attempt++) {
            hostB.kill();
            await new Promise((r) => { hostB.onExit(() => r()); });
        ...
        assert.ok(!bStream.includes("TC76M820-INJECTED"), "a dead pty's write stream must never inject bytes into a recycled fd");
        hostB.kill();
        await new Promise((r) => { hostB.onExit(() => r()); });
    });
});
