...
import { CLEAR_AND_SCROLLBACK, hydrateTransport, lfToCrlf, stripOneTrailingLf, parsePaneGridFacts, cursorPositionEscape, } from "./hydration.js";
...
describe("tc-5quo hydrateTransport — single pane", () => {
    it("sends capture-pane then display-message (grid facts) per pane", async () => {
        ...
        assert.equal(sentCommands[0], "capture-pane -t %1 -p -e -J -S - -E -", "must use -p -e -J -S - -E - for full-history rehydration (tc-0ghi: -J joins wrapped rows + preserves trailing spaces)");
        assert.equal(sentCommands[1], "display-message -p -t %1 -F '#{cursor_x},#{cursor_y},#{history_size},#{pane_height}'", "must read cursor_x/cursor_y/history_size/pane_height for the structured reconstruction (tc-w3ir.2)");
    });
    it("delivers CLEAR_AND_SCROLLBACK + replayed body in a single sendData frame", async () => {
        ...
        const expected = new Uint8Array(CLEAR_AND_SCROLLBACK.length + "hello\r\nworld".length);
    ...
    it("CLEAR_AND_SCROLLBACK prefix appears BEFORE the replayed body in the frame", async () => {
        ...
        for (let i = 0; i < CLEAR_AND_SCROLLBACK.length; i++) {
            assert.equal(bytes[i], CLEAR_AND_SCROLLBACK[i], `byte ${i} of frame must be CLEAR_AND_SCROLLBACK[${i}]`);
        }
...
describe("tc-5quo hydrateTransport — multi-pane + error handling", () => {
    it("hydrates each pane independently with its own capture-pane round-trip", async () => {
        ...
        assert.ok(sentCommands.some((c) => c.startsWith("capture-pane -t %1")), "must capture pane 1");
        assert.ok(sentCommands.some((c) => c.startsWith("capture-pane -t %2")), "must capture pane 2");
    ...
    it("hydrates concurrently across panes (Promise.all, not serial)", async () => {
        ...
        assert.equal(sentCommands.filter((c) => c.startsWith("capture-pane")).length, 2, "both capture-pane sends must have been dispatched before any reply (parallel)");
    ...
    it("skips malformed PaneId silently (defensive guard, does not throw)", async () => {
        ...
        const bad1 = "not-a-pane";
        const bad2 = "p-not-a-number";
        const bad3 = "p"; // length < 2
        const empty = "";
...
describe("tc-5quo hydration — bind-path invariant", () => {
    ...
    it("produces byte-identical hydration frames for the SAME pane regardless of how addClient was reached", async () => {
        ...
        assert.deepEqual(recX.frames[0].bytes, recY.frames[0].bytes, "hydration frame must be byte-identical across bind paths for the same model state");
    });
    it("acceptance: marker text from before disconnect AND output from during disconnect both appear in the replay", async () => {
        ...
        const captureBody = new TextEncoder().encode("MARKER-pre-disconnect\nLINE-DURING-DISCONNECT-1\nLINE-DURING-DISCONNECT-2");
        const { pipeline, setReply } = makeFakePipeline();
        setReply("capture-pane", ok(captureBody));
        const { transport, frames } = makeRecordingTransport();
        await hydrateTransport(pipeline, transport, [P1]);
        ...
        assert.ok(replayStr.includes("MARKER-pre-disconnect"), "pre-disconnect marker must appear in the hydrated replay");
        assert.ok(replayStr.includes("LINE-DURING-DISCONNECT-1"), "first line produced during disconnect must appear in the hydrated replay");
        assert.ok(replayStr.includes("LINE-DURING-DISCONNECT-2"), "second line produced during disconnect must appear in the hydrated replay");
    });
});
// ---------------------------------------------------------------------------
...
describe("tc-w3ir.2 structured grid reconstruction frame", () => {
    ...
    function replayString(frameBytes) {
    ...
    it("fresh no-scrollback pane: full grid + cursor restore to the prompt row (top-anchored)", async () => {
        ...
        assert.equal(replay.split("\r\n").length, ROWS, "must write the full pane-height grid (blank tail preserved, not trimmed)");
    });
