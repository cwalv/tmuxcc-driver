...
import { emptyModel, paneId, windowId, sessionId, } from "../state/model.js";
...
function createFakePipeline(initialModel) {
    ...
    return {
        ...
        async applyClientBinding(_clientId) {
        ...
        sendBatch(_commands) {
            ...
            return _commands.map(() => new Promise(() => { }));
        },
...
async function connectClient(server) {
    ...
    sessionProxyTransport.sendControl = function (msg) {
...
const P1 = paneId("p1");
const P2 = paneId("p2");
function makePane(id, winId, sessId, cols = 80, rows = 24) {
...
function makeModel1() {
    const sess = { sessionId: S1, name: "main", windowIds: [W1], activeWindowId: W1 };
    const win = { windowId: W1, sessionId: S1, name: "editor", paneIds: [P1], activePaneId: P1, layout: null, synchronizePanes: false, monitorActivity: true, monitorSilence: 0 }; // ── tc-7xv.15 ──
...
function makeModel2() {
    ...
    const sess = { sessionId: S1, name: "main", windowIds: [W1], activeWindowId: W1 };
    const win = { windowId: W1, sessionId: S1, name: "editor", paneIds: [P1, P2], activePaneId: P1, layout: null, synchronizePanes: false, monitorActivity: true, monitorSilence: 0 }; // ── tc-7xv.15 ──
    const p1 = makePane(P1, W1, S1);
...
describe("createControlServer", () => {
    ...
    describe("single client", () => {
        ...
        it("client receives deltas with contiguous seq after snapshot", async () => {
            ...
            const delta = received.find((m) => m.type === "pane.opened");
    ...
    describe("multi-client", () => {
        ...
        it("client connecting after a model change gets snapshot with updated model", async () => {
            ...
            const { received: recv1 } = await connectClient(server);
            assert.equal(recv1[0].panes.length, 1);
        ...
        it("two clients have independent seq counters", async () => {
            ...
            for (const [label, received] of [["client1", recv1], ["client2", recv2]]) {
                const seqs = received.map((m) => m.seq);
                for (let i = 0; i < seqs.length; i++) {
        ...
        it("a later client does not see deltas that occurred before it connected", async () => {
            ...
            assert.equal(recv2[0].type, "snapshot");
            // Any messages beyond the snapshot must all be client-count.changed (not model deltas).
            for (let i = 1; i < recv2.length; i++) {
                assert.equal(recv2[i].type, "client-count.changed", "client2 must not receive model deltas that occurred before it connected");
            }
    ...
    describe("client disconnect", () => {
        ...
        it("model changes after one client disconnects do not affect remaining clients", async () => {
            ...
            const client1ModelDeltas = recv1.filter((m) => m.type !== "snapshot" && m.type !== "client-count.changed");
            assert.equal(client1ModelDeltas.length, 0, "client1 should have only snapshot + count-changes (no model deltas post-disconnect)");
    ...
    describe("origin attribution (tc-ozk.2)", () => {
        ...
        function connIdOf(received) {
            const snap = received[0];
            assert.equal(snap.type, "snapshot");
            assert.ok(snap.connectionId !== undefined, "snapshot must carry connectionId");
            return String(snap.connectionId);
        }
        function firstPaneOpened(received) {
        ...
        it("two-connection: A's verb-caused pane carries origin.connectionId=A; B sees it as not-its-own", async () => {
            ...
            assert.notEqual(String(openedB.origin.connectionId), connB, "B must see origin.connectionId != its own → foreign-to-B");
        });
...
describe("multi-client snapshot fan-out (tc-j9c.7)", () => {
    ...
    it("model change fans out to both clients with independent seq counters", async () => {
        ...
        if (delta1?.type === "pane.opened")
            assert.equal(delta1.paneId, P2);
        if (delta2?.type === "pane.opened")
            assert.equal(delta2.paneId, P2);
    });
 ...
 */
function createAsyncTransportPair() {
    ...
    let sessionProxyControlHandler = () => { };
    const sessionProxyCloseHandlers = new Set();
    let clientControlHandler = () => { };
...
describe("tc-3eh.2: addClient snapshot timing — async transport regression", () => {
    ...
    it("client receives snapshot on async transport after microtask-deferred send", async () => {
        ...
        const snapshot = received.find((m) => m.type === "snapshot");
        assert.ok(snapshot !== undefined, `snapshot must arrive on async transport; received ${received.length} message(s): ${received.map((m) => m.type).join(", ")}`);
    ...
    it("snapshot arrives before any deltas on async transport", async () => {
        ...
        for (let i = 1; i < appMsgs.length; i++) {
            assert.ok((appMsgs[i].seq ?? 0) > 1, `delta at index ${i} must have seq > 1; got seq=${appMsgs[i].seq}`);
        }
        sessionProxyTransport.close();
    });
