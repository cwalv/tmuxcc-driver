...
import { createFlowController, DEFAULT_HIGH_WATER_BYTES, DEFAULT_LOW_WATER_BYTES, } from "./flow-control.js";
...
function makeBytes(len, fill = 0xAB) {
...
describe("createFlowController — multi-client per-client ledgers (tc-0wtb)", () => {
    ...
    it("RED→GREEN: 2 clients + one 6-byte append does NOT over-credit (no clamp, no pin)", () => {
        ...
        assert.equal(clamps.length, 0, `expected NO drain-clamp (per-client ledgers), got ${clamps.length}: ${JSON.stringify(clamps)}`);
    ...
    it("resumes only when ALL clients fall to/below low-water (slowest gates resume)", () => {
        ...
        assert.equal(fc.isPanePaused(P3), true, "must STILL be paused: client B is still at 1100 > low-water");
    ...
    it("removeClient re-evaluates the max: detaching the slowest client resumes", () => {
        ...
        assert.equal(fc.isPanePaused(P3), false, "detaching the slowest client must re-evaluate the max and resume");
        const cmds = commands(writes);
...
describe("createFlowController — defaults", () => {
    ...
    it("throws if lowWaterBytes >= highWaterBytes", () => {
        const { send } = makeFakeSend();
        const demux = createOutputDemux();
        assert.throws(() => createFlowController(send, demux, { highWaterBytes: 100, lowWaterBytes: 100 }), /lowWaterBytes.*must be less than/);
    });
});
// ---------------------------------------------------------------------------
...
describe("createFlowController — metrics hooks (tc-d7i)", () => {
    ...
    it("onCommandFailed fires on %error replies, not on success or rejection", async () => {
        ...
        assert.deepEqual(failures, ["pause"], "only the %error reply counts: ok replies and teardown rejections are ignored");
    });
});
//# sourceMappingURL=flow-control.test.js.map