...
describe("OutputDemux: foreign-pane containment", () => {
    ...
    it("A2: overflow drops excess bytes with a warning (does not accumulate unboundedly)", () => {
        ...
        try {
            ...
            assert.equal(demux.pendingBytes(FOREIGN), cap, "overflow byte must not increase pendingBytes");
            assert.ok(warnings.length >= 1, "overflow warning must be emitted");
            assert.ok(warnings[0].includes("overflow") || warnings[0].includes("pre-topology"), `warning must mention overflow: ${warnings[0]}`);
 ...
 */
function makeBootstrapStream(paneNum) {
    const ts = 1_000_000;
    const sid = "$0";
    const wid = "@1";
    const pid_ = `%${paneNum}`;
...
describe("OutputDemux: integration with pipeline and pane tracking", () => {
    ...
    it("A1 (integration): bytes for a new pane arriving before the model knows it are staged then flushed", async () => {
        ...
        assert.ok(all.includes(Buffer.from([0x41, 0x42])), `expected bytes 0x41 0x42 ("AB") after bind, got: ${all.toString("hex")}`);
...
//# sourceMappingURL=output-before-topology.test.js.map