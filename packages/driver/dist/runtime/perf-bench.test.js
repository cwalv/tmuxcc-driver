 ...
 */
function buildOutputLine(lineBytes) {
...
describe("tc-4sg: hot-path throughput benchmark", () => {
    ...
    it("stage 1 — tokenizer: raw byte stream → notification tokens", () => {
        ...
        console.log(`[perf] tokenizer: ${INPUT_BYTES.toLocaleString()} bytes, ` +
    ...
    it("stage 2 — decodeOutputPayload: octal-escaped payload → raw bytes", () => {
        ...
        console.log(`[perf] decodeOutputPayload: ${inputBytes.toLocaleString()} encoded bytes, ` +
    ...
    it("stage 3 — scrollback append: push decoded chunks into PaneBufferStore", () => {
        ...
        console.log(`[perf] scrollback.append: ${appendedTotal.toLocaleString()} bytes, ` +
    ...
    it("stage 4 — full pipeline: tokenize → decode → scrollback (no transport)", () => {
        ...
        for (const token of tokens) {
            if (token.kind !== "notification" || token.keyword !== "output")
                continue;
        ...
        const mbps = fmtMbps(INPUT_BYTES, secs);
        console.log(`[perf] full pipeline (no transport): ${INPUT_BYTES.toLocaleString()} encoded bytes in, ` +
    ...
    it("stage 5 — full pipeline with transport tap (sendData overhead)", () => {
        ...
        const nullTransport = {
            ...
            sendControl(_msg) { },
            onControl(_cb) { return () => { }; },
            onData(_cb) { return () => { }; },
            onClose(_cb) { return () => { }; },
            close(_err) { },
        };
        const demux = createOutputDemux();
        demux.attachTransport(nullTransport);
        const pid = paneId("p0");
        ...
        const mbps = fmtMbps(INPUT_BYTES, secs);
        console.log(`[perf] full pipeline (with transport tap): ${INPUT_BYTES.toLocaleString()} encoded bytes, ` +
...
//# sourceMappingURL=perf-bench.test.js.map