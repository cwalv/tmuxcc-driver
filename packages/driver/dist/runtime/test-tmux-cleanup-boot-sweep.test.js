...
import { sweepOrphanedSockets, trackSocket, } from "./test-tmux-cleanup.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("tc-bpn: sweepOrphanedSockets() — boot-sweep logic", () => {
    ...
    it("B1: sweep does NOT remove socket whose pid segment matches process.pid", () => {
        ...
        try {
            ...
            assert.ok(existsSync(`${TMUX_DIR}/${name}`), `B1: own-pid socket must survive sweep; file was deleted`);
        }
        finally {
            rmSocket(name);
        }
    ...
    // -------------------------------------------------------------------------
    it("B2: sweep does NOT remove socket whose pid belongs to an alive process", { timeout: 10_000 }, async () => {
        // Spawn a child process that stays alive until we kill it.
        const child = spawn(process.execPath, ["-e", "setInterval(() => {}, 60_000)"], { detached: false, stdio: "ignore" });
        ...
        try {
            sweepOrphanedSockets();
            assert.ok(existsSync(`${TMUX_DIR}/${name}`), `B2: socket for alive pid ${childPid} must survive sweep`);
        }
        finally {
            child.kill("SIGKILL");
            rmSocket(name);
    ...
    // -------------------------------------------------------------------------
    it("B3: sweep removes socket whose pid is dead", { timeout: 10_000 }, async () => {
        ...
        try {
            ...
            assert.ok(!existsSync(`${TMUX_DIR}/${name}`), `B3: socket for dead pid ${deadPid} must be removed by sweep`);
        }
        finally {
    ...
    it("B4: trackSocket('tmuxcc-foo') throws (no pid segment)", () => {
        assert.throws(() => trackSocket("tmuxcc-foo"), /must match.*tmuxcc-test/, "B4: name without pid segment must be rejected");
    });
    // -------------------------------------------------------------------------
    // B5. trackSocket rejects name with non-numeric pid segment
    // -------------------------------------------------------------------------
    it("B5: trackSocket('tmuxcc-test-foo') throws (non-numeric pid segment)", () => {
        assert.throws(() => trackSocket("tmuxcc-test-foo"), /must match.*tmuxcc-test/, "B5: non-numeric pid segment must be rejected");
        // Also test with a dash-terminated non-numeric pid.
        assert.throws(() => trackSocket("tmuxcc-test-abc-suffix"), /must match.*tmuxcc-test/, "B5: non-numeric pid segment with suffix must be rejected");
    });
    // -------------------------------------------------------------------------
    ...
    it("B6: sweep ignores entries not starting with 'tmuxcc-test-'", () => {
        ...
        try {
            sweepOrphanedSockets();
            assert.ok(existsSync(`${TMUX_DIR}/${prodName}`), "B6: production-named socket must NOT be touched by sweep");
            assert.ok(existsSync(`${TMUX_DIR}/${unrelatedName}`), "B6: unrelated entry must NOT be touched by sweep");
        }
        finally {
            rmSocket(prodName);
    ...
    // -------------------------------------------------------------------------
    it("B7: two concurrent test agents do not kill each other's sockets at boot", { timeout: 10_000 }, async () => {
        ...
        try {
            sweepOrphanedSockets();
            assert.ok(existsSync(`${TMUX_DIR}/${ownName}`), "B7: our own socket must survive sweep");
            assert.ok(existsSync(`${TMUX_DIR}/${otherName}`), "B7: the other live agent's socket must survive sweep");
        }
        finally {
            child.kill("SIGKILL");
            rmSocket(ownName);
...
//# sourceMappingURL=test-tmux-cleanup-boot-sweep.test.js.map