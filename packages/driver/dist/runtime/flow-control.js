 ...
 */
const DEFAULT_CLIENT = { default: true };
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
class FlowControllerImpl {
    ...
    constructor(send, demux, opts = {}) {
        ...
        if (this._lowWater >= this._highWater) {
            throw new Error(`[flow-control] lowWaterBytes (${this._lowWater}) must be less than highWaterBytes (${this._highWater})`);
        }
    }
    // -------------------------------------------------------------------------
    // Public interface
    ...
    removeClient(clientKey) {
        if (!this._clients.delete(clientKey))
            return;
    ...
    onPaneBytes(paneId, byteCount) {
        ...
        if (!this._paused.has(paneId) && this._maxBuffered(paneId) > this._highWater) {
            ...
            for (const clientKey of this._effectiveClients()) {
                ...
                if (cur > this._highWater)
                    inner.set(clientKey, this._highWater);
            }
        }
    }
    noteDrained(paneId, byteCount, clientKey) {
        if (byteCount <= 0)
            return;
    ...
    onExtendedOutput(paneId, byteCount) {
    ...
    _sendFlowCommand(tmuxN, state) {
        this._send(refreshClientFlow(tmuxN, state)).then((result) => {
            if (!result.ok)
                this._metrics.onCommandFailed?.(state);
        }, () => {
 ...
 */
export function createFlowController(send, demux, opts) {
