...
describe("tc-295a.8 attach-to-vanished-pane", () => {
    it("model-absent pane: emits pane.attach.failed{pane.not-found}, no hydration", async () => {
        ...
        const modelHas = (_pid) => false;
        if (!modelHas(P9)) {
            h.sendDirected({ type: "pane.attach.failed", paneId: P9 });
        }
        const controls = h.log.filter((e) => e.kind === "control");
    ...
    it("pane closes mid-capture (ok=false): hydratePane returns false → pane.not-found", async () => {
        const h = makeHarness();
        const { pipeline } = makePipeline(new Map([["capture-pane -t %9", { ok: false, commandNumber: 0, body: new Uint8Array(0) }]]));
...
describe("tc-295a.8 attach-to-live-pane hydrate-then-stream", () => {
    it("emits begin → clear+replay data frame → end, in that order", async () => {
        ...
        const dataEntry = h.log.find((e) => e.kind === "data");
        assert.deepEqual(dataEntry.bytes.subarray(0, CLEAR_AND_SCROLLBACK.length), CLEAR_AND_SCROLLBACK);
        assert.equal(new TextDecoder().decode(dataEntry.bytes.subarray(CLEAR_AND_SCROLLBACK.length)), "history-line");
    });
    it("live deltas after end stream normally (live pass-through restored)", async () => {
        ...
        assert.deepEqual(last.bytes, new Uint8Array([0x7a]));
    });
});
// ---------------------------------------------------------------------------
...
describe("tc-295a.9 live-bytes-during-hydration queueing", () => {
    it("queues live bytes arriving during the capture RTT; replays them after the clear+replay frame, before end takes effect", async () => {
        ...
        const pipeline = {
            send(command) {
                ...
                return new Promise((res) => { resolveCapture = res; });
            },
        };
        ...
        assert.equal(h.log.filter((e) => e.kind === "data").length, 0, "live byte during hydration must NOT have been delivered yet");
        ...
        const dataFrames = h.log.filter((e) => e.kind === "data");
        ...
        assert.equal(new TextDecoder().decode(dataFrames[0].bytes.subarray(CLEAR_AND_SCROLLBACK.length)), "REPLAY");
    ...
    it("bulk addClient-style hydrate of multiple panes frames each with its own begin/end", async () => {
        ...
        const { pipeline } = makePipeline(new Map([
...
function makeFcHarness() {
    ...
    const clamps = [];
    const fc = createFlowController(() => new Promise(() => { }), // fire-and-forget send
    demux, {
    ...
    const drainingTransport = {
        ...
        sendData(pid, bytes) {
            ...
            if (result !== undefined && typeof result.then === "function") {
                return result.then(() => fc.noteDrained(pid, bytes.length, rawTransport));
            }
...
describe("tc-t4k1 FC-1 drain over-credit on hydration", () => {
    it("hydrating a pane with scrollback history does NOT trip the DRAIN CLAMPED tripwire", async () => {
        ...
        assert.deepEqual(h.clamps, [], "replay bytes were never counted by onPaneBytes — crediting them via " +
...
//# sourceMappingURL=attach-hydration.test.js.map