...
describe("tc-7ml.1: flow-control resume wiring — notification routing + noteDrained auto-call", () => {
    ...
    it("W3: drainingTransport pattern automatically calls fc.noteDrained on sendData", async () => {
        ...
        assert.equal(pausedCounter, HIGH, `W3: bufferedBytes (${pausedCounter}) clamps to HIGH (${HIGH}) at the pause edge`);
        ...
        assert.equal(fcW3.isPanePaused(P7), false, "W3: pane must be resumed after manual noteDrained below low-water");
        ...
        assert.ok(postCounter <= preCounter + newChunk.length, `W3: bufferedBytes (${postCounter}) must not grow unboundedly; ` +
