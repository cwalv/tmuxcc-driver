...
import { trackSocket, forgetSocket, killTmuxServer, flushAllTracked, getTrackedSockets, } from "./test-tmux-cleanup.js";
// ---------------------------------------------------------------------------
...
describe("tc-blk: test-tmux-cleanup safety net", () => {
    ...
    it("S1: trackSocket throws on names outside the `tmuxcc-test-<pid>-` convention", () => {
        ...
        assert.throws(() => trackSocket("gc-chost.session"), /must match.*tmuxcc-test/, "must refuse user-owned socket names");
        assert.throws(() => trackSocket("default"), /must match.*tmuxcc-test/, "must refuse the literal default tmux socket name");
        assert.throws(() => trackSocket(""), /must match.*tmuxcc-test/, "must refuse the empty socket name");
        // Names that start with `tmuxcc-` but don't include pid segment also rejected.
        assert.throws(() => trackSocket("tmuxcc-foo"), /must match.*tmuxcc-test/, "must refuse tmuxcc- names without pid segment");
        // Names with `tmuxcc-test-` prefix but non-numeric pid segment are rejected.
        assert.throws(() => trackSocket("tmuxcc-test-foo"), /must match.*tmuxcc-test/, "must refuse tmuxcc-test- names with non-numeric pid segment");
    });
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    it("S3: a tracked socket whose test 'throws' is still reaped by flushAllTracked", { skip: tmuxAvailable ? false : "tmux not found on PATH" }, () => {
        ...
        try {
            execFileSync("tmux", ["-L", sock, "new-session", "-d", "-s", "throw-regression"], { timeout: 5000, stdio: ["ignore", "ignore", "ignore"] });
        }
        catch (err) {
            assert.fail(`tmux new-session failed: ${err.message}`);
        }
        // Verify the server is alive (would be observable as a leak otherwise).
        const checkAlive = spawnSync("tmux", ["-L", sock, "list-sessions"], { timeout: 3000 });
        assert.equal(checkAlive.status, 0, "S3: tmux server must be alive before flush (sanity check)");
        ...
        const checkDead = spawnSync("tmux", ["-L", sock, "list-sessions"], { timeout: 3000 });
        assert.notEqual(checkDead.status, 0, "S3: tmux server must be REAPED by flushAllTracked() — bead acceptance criterion");
        // And the tracked set must no longer contain it.
        assert.equal(getTrackedSockets().includes(sock), false, "S3: flushAllTracked() must clear the tracked set entry");
    });
    // -------------------------------------------------------------------------
    ...
    it("S4: forgetSocket removes the socket from the tracked set", () => {
        ...
        trackSocket(sock);
        assert.ok(getTrackedSockets().includes(sock), "S4: socket must be in tracked set after trackSocket");
        forgetSocket(sock);
        assert.equal(getTrackedSockets().includes(sock), false, "S4: socket must be absent after forgetSocket");
    });
});
//# sourceMappingURL=test-tmux-cleanup.test.js.map