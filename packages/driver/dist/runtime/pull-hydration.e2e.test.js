...
async function attachRecording(sessionProxy, identityId, opts) {
    const { sessionProxy: dt, client: ct } = createInMemoryTransportPair();
    const rec = { ct, seq: 1, control: [], data: [] };
    ...
    ct.onControl((msg) => rec.control.push(msg.type));
    ct.onData((pid, bytes) => rec.data.push({ paneId: pid, bytes }));
    await addP;
    return rec;
}
/** Does `bytes` start with the hydration clear escape (ESC[H ESC[2J ESC[3J)? */
function isReplayFrame(bytes) {
    if (bytes.length < CLEAR_AND_SCROLLBACK.length)
        return false;
    for (let i = 0; i < CLEAR_AND_SCROLLBACK.length; i++) {
        if (bytes[i] !== CLEAR_AND_SCROLLBACK[i])
            return false;
    }
    return true;
}
function hydrationBegins(rec) {
...
async function waitUntil(cond, timeoutMs, msg) {
    const deadline = Date.now() + timeoutMs;
    while (!cond()) {
        if (Date.now() > deadline)
            throw new Error(`Timeout (${timeoutMs}ms): ${msg}`);
        await sleep(25);
    }
}
/**
...
describe("tc-76m8.28: pullHydration suppresses the addClient-time unsolicited replay (real tmux)", { skip: !tmuxAvailable ? "tmux not found on PATH" : false }, () => {
    it("T1: pullHydration client gets NO unsolicited replay; explicit pane.attach still hydrates", { timeout: 40_000 }, async () => {
        ...
        try {
            ...
            assert.strictEqual(hydrationBegins(rec), 0, `unsolicited hydration pushed to a pullHydration client (control=${JSON.stringify(rec.control)})`);
            ...
            await waitUntil(() => hydrationBegins(rec) === 1 &&
                replayFrames(rec) === 1 &&
                rec.control.includes("pane.hydration.end"), 15_000, "explicit pane.attach must hydrate (begin → clear+replay → end)");
            assert.ok(!rec.control.includes("pane.attach.failed"), "pane.attach for a live pane must not fail");
        }
        finally {
            sessionProxy.kill();
            killTmuxServer(sock);
    ...
    it("T2: flag-less client keeps the tc-5quo bulk push (control)", { timeout: 40_000 }, async () => {
        ...
        try {
            ...
            await waitUntil(() => hydrationBegins(rec) >= 1 && replayFrames(rec) >= 1, 15_000, "flag-less client must receive the unsolicited bulk replay");
        }
        finally {
            sessionProxy.kill();
            killTmuxServer(sock);
    ...
    it("T3: pullHydration suppresses the targeted-primary push too", { timeout: 40_000 }, async () => {
        ...
        try {
            ...
            await sleep(NO_PUSH_WINDOW_MS);
            assert.strictEqual(hydrationBegins(rec), 0, `targeted-primary hydration pushed to a pullHydration client (control=${JSON.stringify(rec.control)})`);
            assert.strictEqual(replayFrames(rec), 0, "unsolicited clear+replay frame delivered");
            // The primary's pane.not-found validation moves to the client's own
            // pane.attach — no unsolicited failure either.
            assert.ok(!rec.control.includes("pane.attach.failed"), "no unsolicited pane.attach.failed for a live primary");
        }
        finally {
            sessionProxy.kill();
            killTmuxServer(sock);
...
//# sourceMappingURL=pull-hydration.e2e.test.js.map