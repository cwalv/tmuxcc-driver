...
let _cmdNum = 500;
const nextCmdNum = () => _cmdNum++;
const cmdBlock = (n, body) => `%begin 1000000 ${n} 1\r\n${body}%end 1000000 ${n} 1\r\n`;
class FakeHost {
    _data = new Set();
    _exit = new Set();
    _err = new Set();
    ...
    write(data) {
        ...
        if (t.startsWith("set-option") || t.startsWith("refresh-client")) {
            ...
            process.nextTick(() => { if (!this._exited)
                this.push(b(block)); });
        }
    }
    onData(h) { this._data.add(h); return () => this._data.delete(h); }
    onExit(h) { this._exit.add(h); return () => this._exit.delete(h); }
    onError(h) { this._err.add(h); return () => this._err.delete(h); }
    onStderr(h) { this._stderr.add(h); return () => this._stderr.delete(h); }
    stop() { this._exited = true; return Promise.resolve(); }
    kill() { this._exited = true; }
    push(bytes) { for (const h of this._data)
        h(bytes); }
}
/** One-session, one-window, one-pane (%1) bootstrap reply pair. */
function bootstrapReplies() {
...
function outputFrame(paneNum, raw) {
...
describe("pane.notify pipeline: storm rate-limiting", () => {
    it("rate-limits a bell storm and fires the Tier-1 expected-zero drop tripwire", async () => {
        ...
        process.stderr.write = (s) => {
            if (typeof s === "string" && s.includes("PANE-NOTIFY RATE LIMIT"))
                tripwireLines++;
            return true;
        };
        try {
...
//# sourceMappingURL=notify-pipeline.test.js.map