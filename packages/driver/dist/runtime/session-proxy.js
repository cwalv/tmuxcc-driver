 ...
 */
export function createSessionProxy(opts) {
    ...
    const demux = createOutputDemux({
        onPretopologyDropped: (bytes, provenance) => {
            ...
            if (provenance === "owned") {
                process.stderr.write(`[output-demux] OWNED PRETOPOLOGY DROP: ${bytes} bytes dropped before the pane was bound to the model. ` +
    ...
    const fc = createFlowController(fcSend, demux, {
        ...
        metrics: {
            onDrainClamped: (paneId, excessBytes) => {
                ...
                process.stderr.write(`[flow-control] DRAIN CLAMPED: pane ${paneId} drain credit exceeded buffered bytes by ${excessBytes}. ` +
            ...
            onCommandFailed: (kind) => {
                ...
                process.stderr.write(`[flow-control] COMMAND FAILED: refresh-client -A ${kind} replied %error. ` +
                    (kind === "continue"
     ...
     */
    function makeSentinels(controlTransport, dataTransport) {
        return {
            begin(pid) {
     ...
     */
    async function attachAndHydratePane(controlTransport, replayTransport, sentinels, pid) {
        ...
        if (!found) {
            server.sendDirected(controlTransport, {
                ...
                paneId: pid,
                code: "pane.not-found",
                message: `Pane ${pid} could not be captured (it may have closed mid-attach).`,
            });
        }
    }
    return {
        ...
        async addClient(transport, opts = {}) {
            ...
            transport.onControl((msg) => {
                ...
                if (msg.type === "command.request" && msg.command.kind === "pane.capture") {
                    const req = msg;
                    const captureCmd = req.command;
                    const pid = captureCmd.paneId;
                    // Model check: fail-loud if the pane is not present.
                    if (!pipeline.getModel().panes.has(pid)) {
                        server.sendCommandError(transport, req.correlationId, "pane.not-found", `Pane ${pid} is not present in the session model.`);
                        return;
                    }
                    ...
                    void captureText(pipeline, pid).then((result) => {
                        if (!result.ok) {
                            ...
                            server.sendCommandError(transport, req.correlationId, "pane.not-found", `Pane ${pid} could not be captured (it may have closed mid-request).`);
                            return;
                        }
                ...
                if (msg.type === "command.request" && clientFlags?.readOnly === true) {
                    const req = msg;
                    server.sendCommandError(transport, req.correlationId, "read-only", "Client is read-only: mutating commands are not allowed.");
                    return;
                }
                ...
                inputPath.handleClientMessage(msg, (correlationId, result) => {
                    if (result.ok) {
