...
// ---------------------------------------------------------------------------
function queryTmuxSize(sock, session) {
    const r = spawnSync("tmux", ["-L", sock, "list-panes", "-t", session, "-F", "#{pane_width}x#{pane_height}"], { encoding: "utf8" });
    if (r.status !== 0)
        return null;
...
async function waitForSize(sock, session, cols, rows, timeoutMs, msg) {
    const deadline = Date.now() + timeoutMs;
    for (;;) {
        const s = queryTmuxSize(sock, session);
        if (s !== null && s.cols === cols && s.rows === rows)
            return;
...
async function assertStaysSize(sock, session, cols, rows, windowMs, msg) {
    const deadline = Date.now() + windowMs;
    while (Date.now() < deadline) {
        const s = queryTmuxSize(sock, session);
        assert.ok(s !== null && s.cols === cols && s.rows === rows, `${msg}: geometry became ${JSON.stringify(s)}, expected ${cols}x${rows}`);
        await new Promise((r) => setTimeout(r, 40));
    }
}
async function attach(sessionProxy, identityId) {
...
describe("tc-76m8.3: size ownership follows activity (real tmux)", { skip: !tmuxAvailable ? "tmux not found on PATH" : false }, () => {
    it("T1: alternation moves ownership after debounce; the new owner's viewport is re-applied (AC1)", { timeout: 40_000 }, async () => {
        ...
        try {
            ...
            advance(1);
            await waitForSize(sock, session, 100, 30, 15_000, "after debounce, laptop owns and its viewport is re-applied");
    ...
    it("T2: rapid interleaved input does not oscillate ownership (AC2)", { timeout: 40_000 }, async () => {
    ...
    it("T3: single-client D4 mechanics unchanged (sole full client drives; sole ignore-size client does not)", { timeout: 40_000 }, async () => {
...
//# sourceMappingURL=size-ownership.e2e.test.js.map