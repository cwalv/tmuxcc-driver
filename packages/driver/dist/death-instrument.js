 ...
 */
const DEATH_INSTRUMENT_MODE = process.env["TMUXCC_DEATH_INSTRUMENT"] ?? "";
/** True when the instrument is enabled (any non-empty value). Read once. */
export const DEATH_INSTRUMENT_ENABLED = DEATH_INSTRUMENT_MODE !== "";
/** True when the bounded tmux-server liveness/RSS probe should run. */
const DEATH_INSTRUMENT_PROBE = DEATH_INSTRUMENT_ENABLED && DEATH_INSTRUMENT_MODE !== "noprobe";
/** Read a single `Key:   <n> kB` field from /proc/meminfo. -1 on miss/error. */
function meminfoKB(meminfo, key) {
 ...
 */
function probeTmuxServer(socketName) {
    if (!DEATH_INSTRUMENT_PROBE)
        return { state: "skipped", pid: -1, rssKB: -1 };
    try {
        const r = spawnSync("tmux", ["-L", socketName, "display-message", "-p", "#{pid}"], { encoding: "utf8", timeout: 1_500 });
        if (r.error)
 ...
 */
export function recordHostDeath(rec) {
    if (!DEATH_INSTRUMENT_ENABLED)
        return;
    try {
        let meminfo = "";
        try {
        ...
        if (rec.path === "onError" && rec.error?.stack) {
            ...
            process.stderr.write(`[tc-jlyi.17] hostDeath-errstack session=${rec.sessionName} stack=${stack}\n`);
        }
    }
    catch {
...
//# sourceMappingURL=death-instrument.js.map