...
describe("ServerProxyMetrics", () => {
    it("incCommand increments the per-kind counter and appears in exposition", async () => {
        ...
        assert.ok(text.match(/kind="session\.claim"\} 2/), `session.claim should be 2; got:\n${text}`);
    ...
    it("observeRpcRoundTrip records the application-leg histogram, by kind (tc-bn7d)", async () => {
        ...
        const text = await m.metricsText();
        assert.ok(text.includes("rpc_round_trip_seconds"), "text should contain rpc_round_trip_seconds histogram");
        assert.ok(text.includes("rpc_round_trip_seconds_count"), "text should contain histogram count");
        assert.ok(text.includes('kind="session.claim"'), "text should contain kind=session.claim label");
        assert.ok(text.includes('kind="server-proxy.info"'), "text should contain kind=server-proxy.info label");
        // Two session.claim observations landed on that kind's count.
        assert.ok(text.match(/rpc_round_trip_seconds_count\{kind="session\.claim"\} 2/), `session.claim count should be 2; got:\n${text}`);
        ...
        assert.ok(text.match(/rpc_round_trip_seconds_bucket\{le="5",kind="session\.claim"\} 1/), `only the 50 ms sample should be <=5 s; got:\n${text}`);
        m.stop();
    });
    ...
    it("incSelfExit attributes the designed self-exit by reason (tc-m2y8)", async () => {
        ...
        assert.ok(text.match(/server_proxy_self_exit_total\{reason="idle"\} 2/), `idle self-exits should be 2; got:\n${text}`);
        assert.ok(text.match(/server_proxy_self_exit_total\{reason="tmux-gone"\} 1/), `tmux-gone self-exits should be 1; got:\n${text}`);
        m.stop();
    });
    ...
    it("incWatcherRespawn counts respawns (no label) (tc-m2y8)", async () => {
        ...
        m.incWatcherRespawn();
        const text = await m.metricsText();
        assert.ok(text.match(/server_proxy_watcher_respawns_total 3/), `respawns should be 3; got:\n${text}`);
        // No label on this counter (one event type).
        assert.ok(!/server_proxy_watcher_respawns_total\{/.test(text), "watcher respawns counter must be unlabelled");
        m.stop();
    });
    ...
    it("addSocketFeedQueueDepth aggregates and returns to zero (tc-edf8)", async () => {
        ...
        assert.ok(text.match(/socketfeed_sendcontrol_queue_depth 2/), `queue depth should be 2 while backpressured; got:\n${text}`);
        // Drain resolves both waiters: gauge returns to 0.
        m.addSocketFeedQueueDepth(-2);
        text = await m.metricsText();
        assert.ok(text.match(/socketfeed_sendcontrol_queue_depth 0/), `queue depth should return to 0 after drain; got:\n${text}`);
    ...
    it("observeSocketFeedTimeInQueue records the per-send wait histogram (tc-edf8)", async () => {
        ...
        assert.ok(text.match(/socketfeed_time_in_queue_seconds_count 3/), `should have 3 observations; got:\n${text}`);
        // No labels (aggregate across all transports).
        assert.ok(!/socketfeed_time_in_queue_seconds_count\{/.test(text), "time-in-queue histogram must be unlabelled (aggregate)");
        m.stop();
    });
    it("default metrics (event-loop lag, CPU, heap) are present (tc-3si.6)", async () => {
        ...
        assert.ok(text.includes("nodejs_eventloop_lag_seconds") ||
            text.includes("nodejs_eventloop_lag"), "default metrics should include eventloop lag");
