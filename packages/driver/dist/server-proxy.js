...
import { runServerHandshake, WIRE_PROTOCOL_VERSION, sessionId as mintSessionId, describeClientIdentity, CommandError, isCommandError, toCommandFailure, } from "@tmuxcc/protocol";
...
export class ServerProxyAlreadyRunningError extends Error {
    ...
    code = "server-proxy.already-running";
...
// ---------------------------------------------------------------------------
const SERVER_PROXY_CAPABILITIES = {
...
class ServerProxyImpl {
    ...
    _metrics = createServerProxyMetrics();
     ...
     */
    _metricsHttpOp = Promise.resolve();
    constructor(opts) {
        this._opts = opts;
    ...
    endpoint() {
        if (!this._started)
            throw new Error("ServerProxy not started");
    ...
    setMetricsHttp(enabled, bind) {
        ...
        this._metricsHttpOp = run.then(() => undefined, () => undefined);
        return run;
    }
    toggleMetricsHttp() {
     ...
     */
    async _applyMetricsHttp(enabled, bind) {
        if (!enabled) {
    ...
    async start() {
        if (this._started)
            throw new Error("ServerProxy already started");
        ...
        if (this._tmuxCapabilityState?.belowFloor) {
            ...
            process.stderr.write(`serverProxy: tmuxcc requires tmux ${MINIMUM_TMUX_VERSION} or later` +
        ...
        this._supervisor.onCrash((sessionId, info) => {
            this._onSessionProxyCrash(sessionId, info);
        });
        this._started = true;
     ...
     */
    _removeOwnedSocket() {
        if (this._boundSocketIno === null)
            return; // never bound — own nothing
    ...
    _spawnWatcher() {
        ...
        return createTmuxWatcher(this._opts.socketName, () => {
     ...
     */
    _onWatcherEof() {
        if (!this._started || this._selfExited || this._probeInFlight)
            return;
        ...
        void probeTmuxLiveness(this._opts.socketName, TMUX_PROBE_TIMEOUT_MS).then((liveness) => {
            ...
            if (!this._started || this._selfExited)
                return;
            ...
            if (aliveMs >= WATCHER_HEALTHY_MS) {
                ...
                this._watcherRespawnDelayMs = Math.min(Math.max(this._watcherRespawnDelayMs * 2, WATCHER_RESPAWN_BACKOFF_INIT_MS), WATCHER_RESPAWN_BACKOFF_MAX_MS);
            }
            this._scheduleWatcherRespawn(this._watcherRespawnDelayMs);
        });
    }
    _scheduleWatcherRespawn(delayMs) {
        if (this._respawnTimer !== null)
    ...
    _onConnectionCountChange(count) {
     ...
     */
    async _selfExit(reason) {
        if (this._selfExited || !this._started)
            return;
     ...
     */
    _onSessionProxyCrash(sessionId, info) {
        process.stderr.write(`serverProxy: session-proxy for session '${info.sessionName}' (${sessionId}) exited ` +
    ...
    async _doRefreshSessions() {
        ...
        const availability = { binaryMissing: false };
        ...
        for (const [sid, entry] of this._sessions) {
            ...
            if (this._supervisor.hasSessionProxy(sid))
                continue;
            // Authoritative: only a positive "absent" from a reachable server removes.
            if ((await checkSessionPresence(this._opts.socketName, entry.tmuxId)) !== "absent")
                continue;
    ...
    // Connection handler
    // ---------------------------------------------------------------------------
    async _handleConnection(transport) {
        ...
        const state = { transport, nextSeq: 2, identity: session.clientIdentity, session };
        this._clients.set(transport, state);
        process.stderr.write(`serverProxy: client connected identity=` +
        ...
        transport.onControl((msg) => {
            if (msg.type === "command.request") {
                void this._handleCommand(state, msg);
            }
                ...
                void this._handleAttach(state, msg);
            }
    ...
    async _handleAttach(state, msg) {
        ...
        try {
            ...
            const sessionProxy = await this._supervisor.ensureSessionProxy(entry.sessionId, entry.name, this._opts.socketName);
            ...
            process.stderr.write(`serverProxy: session.attach for '${msg.sessionId}' failed: ${String(err)}\n`);
            try {
                transport.close();
            }
    ...
    async _buildInfo() {
        ...
        return {
            ...
            clients: Array.from(this._clients.values(), (c) => c.identity !== undefined ? { identity: c.identity } : {}),
    ...
    // Command dispatch
    // ---------------------------------------------------------------------------
    async _handleCommand(state, req) {
        ...
        try {
            ...
            const result = isCommandError(err)
                ...
                : { ok: false, code: "internal", message: err instanceof Error ? err.message : String(err) };
    ...
    _sendResponse(state, partial) {
        ...
        state.nextSeq++;
        try {
            state.transport.sendControl(msg);
        }
        catch {
     ...
     */
    async _querySessionTopology(sessionId, clientId) {
     ...
     */
    _registerNewSession(tmuxId, name) {
    ...
    _broadcastRenamed(sessionId, newName) {
        const delta = {
 ...
 */
export function createServerProxy(opts) {
