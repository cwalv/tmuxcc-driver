...
describe("npm-root-guard discriminator (tc-rs99)", () => {
    it("BLOCKS a direct subdir invocation (INIT_CWD === cwd, no root sentinel)", () => {
        ...
        assert.equal(isBlockedDirectSubdir({ fromRoot: false, initCwd: MEMBER, cwd: MEMBER }), true);
    });
    it("ALLOWS a nested root-initiated build (sentinel set by the root flow)", () => {
        ...
        assert.equal(isBlockedDirectSubdir({ fromRoot: true, initCwd: MEMBER, cwd: MEMBER }), false);
    });
    it("ALLOWS the classic root `-w` fan-out (npm runs the member from the root cwd)", () => {
        ...
        assert.equal(isBlockedDirectSubdir({ fromRoot: false, initCwd: ROOT, cwd: MEMBER }), false);
    });
    it("ALLOWS when INIT_CWD is unset (npm not the invoker)", () => {
        assert.equal(isBlockedDirectSubdir({ fromRoot: false, initCwd: undefined, cwd: MEMBER }), false);
    });
});
//# sourceMappingURL=npm-root-guard.test.js.map