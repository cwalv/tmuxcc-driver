...
describe("server-proxy-log (tc-k6v)", () => {
    ...
    it("L5: tee survives a synchronously-throwing (detached) stderr — logs anyway, never throws (tc-9xf1)", () => {
        ...
        try {
            ...
            assert.doesNotThrow(() => process.stderr.write("post-detach diagnostics\n"), "tee must swallow the detached-pipe EPIPE");
        ...
        const content = fs.readFileSync(logPath, "utf8");
        assert.ok(content.includes("post-detach diagnostics"), `log must capture the line even when the stderr forward EPIPEs, got: ${content}`);
    });
});
//# sourceMappingURL=server-proxy-log.test.js.map