...
import { createSession, killSession, listSessions, setGlobalScrollOnClear, setSessionMarker, setSessionWorkspace, probeTmuxLiveness, probeTmuxAlive, checkSessionPresence, } from "./tmux-south.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
let testCounter = 0;
...
function killServer(socketName) {
...
describe("tmux-south createSession (tc-zcqr)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("returns the authoritative tmux session id of the newly-created session", async () => {
        ...
        try {
            ...
            assert.match(result.tmuxId, /^\$\d+$/, `Expected session id like "$N", got: ${JSON.stringify(result.tmuxId)}`);
            // The returned id must round-trip through `list-sessions` — i.e. it
            // really is the live tmux session id, not a fabrication.
            const ls = spawnSync("tmux", ["-L", socketName, "list-sessions", "-F", "#{session_id} #{session_name}"], { encoding: "utf8", timeout: 5_000 });
            ...
            assert.ok(row.startsWith(`${result.tmuxId} `), `Expected returned id ${result.tmuxId} to match list-sessions row: ${row}`);
        }
        finally {
    ...
    it("creates the session with the @tmuxcc marker set", async () => {
        ...
        try {
            ...
            const show = spawnSync("tmux", ["-L", socketName, "show-options", "-t", "marked", "-v", "@tmuxcc"], { encoding: "utf8", timeout: 3_000 });
    ...
    it("sets the server-global scroll-on-clear off default (tc-w3ir.1)", async () => {
        ...
        try {
            ...
            const showGlobal = spawnSync("tmux", ["-L", socketName, "show-options", "-wg", "-v", "scroll-on-clear"], { encoding: "utf8", timeout: 3_000 });
            ...
            const showWindow = spawnSync("tmux", ["-L", socketName, "show-options", "-A", "-w", "-t", "soc"], { encoding: "utf8", timeout: 3_000 });
            assert.equal(showWindow.status, 0, "window options must be readable");
            assert.match(showWindow.stdout ?? "", /scroll-on-clear\*?\s+off/, `managed window must inherit scroll-on-clear off, got:\n${showWindow.stdout}`);
            // A second window created later on the same server also inherits it.
            const nw = spawnSync("tmux", ["-L", socketName, "new-window", "-t", "soc"], { encoding: "utf8", timeout: 3_000 });
            assert.equal(nw.status, 0, `new-window failed: ${nw.stderr}`);
            const showWindow2 = spawnSync("tmux", ["-L", socketName, "show-options", "-A", "-w", "-t", "soc"], { encoding: "utf8", timeout: 3_000 });
            assert.match(showWindow2.stdout ?? "", /scroll-on-clear\*?\s+off/, `later windows must inherit scroll-on-clear off, got:\n${showWindow2.stdout}`);
        }
        finally {
            killServer(socketName);
        }
    ...
    it("rejects when the name is already taken (duplicate session)", async () => {
        ...
        try {
            ...
            await assert.rejects(() => createSession(socketName, "dup"), /duplicate session|tmux new-session failed/i);
        }
        finally {
            killServer(socketName);
        }
    ...
    it("throws CommandError tmux.duplicate-session when a session name is already taken", async () => {
        ...
        try {
            ...
            const err = await createSession(socketName, "dup-test").then(() => null, (e) => e);
            assert.ok(err instanceof Error, `Expected an Error, got: ${String(err)}`);
            assert.equal(err.code, "tmux.duplicate-session", `Expected structured code "tmux.duplicate-session", got: ${JSON.stringify(err.code)}`);
        }
        finally {
            killServer(socketName);
        }
...
describe("tmux-south createSession env capability gate (tc-gjdx.2)", () => {
    ...
    it("throws capability-required when newSessionEnvFlag is false", async () => {
        ...
        assert.equal((err.details ?? {}).capability, "newSessionEnvFlag", `Expected details.capability "newSessionEnvFlag", got: ${JSON.stringify(err.details)}`);
    });
...
describe("tmux-south listSessions (tc-zcqr)", () => {
    it("returns [] when no tmux server is running on the socket", async () => {
        ...
        assert.deepEqual(rows, [], "listSessions on a dead socket must return [] (server has no sessions), NOT null");
    });
    it("returns the live session rows when sessions exist", { skip: !TMUX_AVAILABLE }, async () => {
        const socketName = nextSocketName();
        try {
...
describe("tmux-south listSessions tmux-availability (tc-295a.35)", () => {
    ...
    it("reports binaryMissing=false when tmux is present (no-server socket)", async () => {
        ...
        assert.equal(out.binaryMissing, false, "tmux present (ran, reported no-server) ⇒ binaryMissing must be false");
    });
    it("reports binaryMissing=true when the tmux binary is absent (ENOENT)", async () => {
        ...
        try {
            ...
            assert.equal(out.binaryMissing, true, "ENOENT on the tmux binary ⇒ binaryMissing must be true");
        }
        finally {
...
describe("tmux-south listSessions enriched fields (tc-295a.4)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("tmuxccMarked is true for a session created by createSession (which stamps @tmuxcc 1)", async () => {
        ...
        try {
            ...
            assert.equal(row.tmuxccMarked, true, "createSession stamps @tmuxcc 1 — tmuxccMarked must be true");
        }
        finally {
            killServer(socketName);
        }
    ...
    it("tmuxccMarked is false for a session created outside tmuxcc (no @tmuxcc option set)", async () => {
        ...
        try {
            ...
            const r = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", "foreign-sess"], { encoding: "utf8", timeout: 10_000 });
            ...
            assert.equal(row.tmuxccMarked, false, "Session created without @tmuxcc option — tmuxccMarked must be false");
        }
        finally {
            killServer(socketName);
        }
    ...
    it("tmuxccMarked transitions to true after setSessionMarker is called on a foreign session", async () => {
        ...
        try {
            ...
            const r = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", "adopt-sess"], { encoding: "utf8", timeout: 10_000 });
            assert.equal(r.status, 0, `tmux new-session failed: ${r.stderr}`);
            ...
            assert.equal(rowAfter.tmuxccMarked, true, "setSessionMarker must flip tmuxccMarked to true");
        }
        finally {
            killServer(socketName);
        }
    ...
    it("paneCount is >= 1 for a newly-created session (at least 1 pane from new-session)", async () => {
        ...
        try {
            ...
            assert.ok(row.paneCount >= 1, `paneCount must be >= 1 for a fresh session, got ${String(row.paneCount)}`);
        }
        finally {
            killServer(socketName);
        }
    ...
    it("lastActivity is a positive Unix epoch for a live session", async () => {
        ...
        try {
            ...
            assert.ok(row.lastActivity > beforeEpoch, `lastActivity (${String(row.lastActivity)}) must be a recent Unix epoch > ${beforeEpoch}`);
        }
        finally {
            killServer(socketName);
        }
...
describe("tmux-south probeTmuxLiveness (tc-vw10)", () => {
    it('returns "gone" when the probe RUNS and finds no server (positive evidence)', { skip: !TMUX_AVAILABLE }, async () => {
        ...
        assert.equal(liveness, "gone", 'a ran-and-found-no-server probe must report "gone", not "inconclusive"');
    });
    ...
    it('returns "inconclusive" — NOT "gone" — on a spawn failure (binary unreachable)', async () => {
        ...
        try {
            process.env.PATH = "";
            const liveness = await probeTmuxLiveness(socketName, 5_000);
            assert.equal(liveness, "inconclusive", 'a spawn failure (could not run the probe) must be "inconclusive", never "gone"');
        }
        finally {
            if (savedPath === undefined)
                delete process.env.PATH;
...
describe("tmux-south setGlobalScrollOnClear on externally-created session (tc-w3ir.5)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("sets scroll-on-clear off on a server bootstrapped without createSession", async () => {
        ...
        try {
            ...
            const ns = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", "ext-sess"], { encoding: "utf8", timeout: 5_000 });
            ...
            const showAfter = spawnSync("tmux", ["-L", socketName, "show-options", "-wg", "-v", "scroll-on-clear"], { encoding: "utf8", timeout: 3_000 });
            ...
            const showWindow = spawnSync("tmux", ["-L", socketName, "show-options", "-A", "-w", "-t", "ext-sess"], { encoding: "utf8", timeout: 3_000 });
            assert.equal(showWindow.status, 0, "window options must be readable");
            assert.match(showWindow.stdout ?? "", /scroll-on-clear\*?\s+off/, `externally-created session window must inherit scroll-on-clear off, got:\n${showWindow.stdout}`);
        }
        finally {
            killServer(socketName);
        }
...
//# sourceMappingURL=tmux-south.test.js.map