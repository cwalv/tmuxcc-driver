 ...
 */
async function attachAwaitTopology(endpoint, sessionId, expectedWindows, expectedPanes, timeoutMs) {
    const transport = await connectSocketTransport(endpoint);
    await runClientHandshake(transport, CLIENT_CAPS, "server-proxy.capabilities");
    const mux = new Mux(transport);
    ...
    const converged = new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            if (settled)
                return;
            settled = true;
            reject(new Error(`session-proxy model did not converge to ${expectedWindows}w/${expectedPanes}p ` +
            ...
            switch (m.type) {
                case "snapshot": {
                    ...
                    for (const w of s.windows)
                        windows.add(w.windowId);
                    for (const p of s.panes)
                        panes.add(p.paneId);
                    break;
                }
                case "window.added":
                    windows.add(m.windowId);
                    break;
                case "window.closed":
                    windows.delete(m.windowId);
                    break;
                case "pane.opened":
                    panes.add(m.paneId);
                    break;
                case "pane.closed":
                    panes.delete(m.paneId);
                    break;
            }
            check();
        });
...
describe("apply-at-create (tc-gjdx.3, real tmux)", () => {
    ...
    it("T1: topology + geometry + cwd + env + @tmuxcc-template awareness + session-row surfacing", async (t) => {
        ...
        try {
            ...
            const writeCmd = `pwd > ${dirA}/cwd.txt; printf %s "$TCENV" > ${dirA}/env.txt; sleep 300`;
            const template = {
                name: "devtmpl",
                windows: [
                    {
            ...
            assert.equal(fs.readFileSync(`${dirA}/cwd.txt`, "utf8").trim(), dirA, "pane A cwd (pwd) == the -c cwd");
            // Awareness: @tmuxcc-template set on the session.
            assert.equal(tmuxQ(socketName, ["display-message", "-p", "-t", "t1", "-F", "#{@tmuxcc-template}"]), "devtmpl", "@tmuxcc-template session option is set to the template name");
    ...
    it("T2: exactly-once under racing claims (only the created:true claimant applies)", async (t) => {
        ...
        try {
            const { mux } = await connectCommand(sp.endpoint());
            const template = { name: "race", windows: [{ name: "alpha" }, { name: "beta" }] };
            // Fire 4 concurrent claims for the SAME name WITHOUT awaiting between them.
            const responses = await Promise.all(Array.from({ length: 4 }, () => sendCommand(mux, { kind: "session.claim", name: "shared", template })));
    ...
    it("T3: no apply on reattach — a re-claim of an existing session (created:false) does NOT apply", async (t) => {
        ...
        try {
            const { mux } = await connectCommand(sp.endpoint());
            const first = { name: "orig", windows: [{ name: "a" }, { name: "b" }] };
            ...
            const second = { name: "other", windows: [{ name: "x" }, { name: "y" }, { name: "z" }] };
            ...
            assert.equal(tmuxQ(socketName, ["display-message", "-p", "-t", "reattach", "-F", "#{@tmuxcc-template}"]), "orig");
        }
        finally {
            await sp.shutdown();
            killServer(socketName);
        ...
        try {
            ...
            const template = {
                ...
                windows: [
                    ...
                    {
                        name: "toomany",
                        geometry: { kind: "hsplit", children: Array.from({ length: 16 }, () => ({ kind: "pane" })) },
                    },
                ],
            };
    ...
    it("T5: coalescer absorbs a many-window apply burst — model converges, no refuted-confirmation tripwire", async (t) => {
        ...
        try {
            ...
            const metricsText = infoResp.result.payload.info.metricsText;
 ...
 * not be exposed).
 */
function matchCounter(metricsText, metric, labelSubstr) {
...
//# sourceMappingURL=apply-at-create.test.js.map