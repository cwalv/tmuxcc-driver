...
describe("tc-7xv.24 wedge regression — real-socket backpressure engages tmux pause", () => {
    it("slow consumer triggers fc pause within bounded time", async () => {
        ...
        assert.equal(pauseSeen, true, `flow controller must pause within 200 chunks of slow consumer; ` +
        ...
        assert.ok(pauseCmd !== undefined, `fake host must have received a pause command (got ${writes.length} writes)`);
    });
    it("resuming consumer drains buffered bytes and unpauses tmux", async () => {
        ...
        assert.equal(fc.isPanePaused(paneId), false, `pane must resume within 5s of consumer recovery; ` +
        ...
        assert.ok(continueCmd !== undefined, `fake host must have received a continue command after drain (got ${writes.length} writes)`);
    });
});
// ---------------------------------------------------------------------------
...
describe("tc-2ztp Ctrl-C-after-firehose — drain path resumes after producer stops", () => {
    it("a firehose that pauses, then a producer stop, still resumes on consumer drain", async () => {
        ...
        assert.equal(fc.isPanePaused(paneId), false, `pane must resume after Ctrl-C + consumer drain — a permanent pause here is ` +
        ...
        const continueCmd = writes.find((w) => w.includes(":continue"));
        assert.ok(continueCmd !== undefined, `fake host must receive a continue command after Ctrl-C drain (got writes: ${JSON.stringify(writes)})`);
    });
});
//# sourceMappingURL=wedge-regression.test.js.map