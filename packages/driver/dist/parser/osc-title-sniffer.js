            ...
            switch (this._state) {
                ...
                case "OSC_PARAM": {
                    if (b >= 0x30 && b <= 0x39) {
                        ...
                        const ps = parseInt(this._paramDigits, 10);
                        if (ps !== 0 && ps !== 2) {
                            for (let j = 0; j < this._bufLen; j++)
                                out[outLen++] = this._buf[j];
                            out[outLen++] = ST_BEL;
                        }
                        this._resetOsc();
                    }
 ...
 */
function containsOscOpener(chunk) {
    for (let i = 0; i < chunk.length; i++) {
        if (chunk[i] === ESC)
            return true;
    }
 ...
 */
function extractTitleFromOscBuffer(buf) {
    ...
    if (semiIdx === -1)
        return null; // no semicolon found — malformed
...
//# sourceMappingURL=osc-title-sniffer.js.map