 ...
 */
export function decodeOutputPayload(payload) {
    ...
    while (i < payload.length) {
        ...
        if (i + 3 < payload.length &&
            ...
            const octalValue = ((payload[i + 1] - 0x30) << 6) |
                ((payload[i + 2] - 0x30) << 3) |
                (payload[i + 3] - 0x30);
 ...
 */
export function encodeOutputPayload(raw) {
 ...
 */
export function parseOutputNotification(token) {
    ...
    if (i >= line.length || line[i] !== PERCENT)
        return null;
    i++; // skip '%'
