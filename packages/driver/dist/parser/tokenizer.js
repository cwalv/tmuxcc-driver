...
export class ControlTokenizer {
    ...
    // ---------------------------------------------------------------------------
    _processChunk(chunk, out) {
        let i = 0;
        while (i < chunk.length) {
            switch (this._dcsState) {
                case 0 /* DcsState.BEFORE_DCS */:
                    i = this._processBefore(chunk, i, out);
                    break;
                case 1 /* DcsState.MATCHING_DCS_INTRO */:
                    i = this._matchDcsIntro(chunk, i, out);
                    break;
                case 2 /* DcsState.INSIDE */:
                    i = this._processInside(chunk, i, out);
                    break;
                case 3 /* DcsState.MAYBE_ST */:
                    i = this._maybeSt(chunk, i, out);
                    break;
                case 4 /* DcsState.AFTER_DCS */:
     ...
     */
    _processInside(chunk, i, out) {
        // ST detection is suppressed while inside a block body (opaque bytes).
        const scanForSt = this._blockState === 0 /* BlockState.OUTSIDE */;
        ...
        while (i < chunk.length) {
            ...
            if (scanForSt && b === 0x1b) {
                ...
                this._dcsState = 3 /* DcsState.MAYBE_ST */;
    ...
    _maybeSt(chunk, i, out) {
        ...
        if (b === ST_BYTES[1]) {
            ...
            this._dcsState = 4 /* DcsState.AFTER_DCS */;
            return i + 1;
        }
    ...
    _flushLine(out) {
        ...
        if (this._blockState === 1 /* BlockState.INSIDE */) {
    ...
    _tryParseGuardLine(line, out) {
        ...
        if (keyword === "end" || keyword === "error") {
            ...
            if (guard !== null) {
                ...
                this._blockState = 0 /* BlockState.OUTSIDE */;
                return true;
            }
        }
        return false;
 ...
 */
export function tokenizeBuffer(buf) {
