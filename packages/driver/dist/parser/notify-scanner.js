...
const BELL_DETECTION = { kind: "bell" };
const EMPTY = [];
/**
 ...
 */
export function parseOscContent(content) {
...
function parseConEmuProgress(args) {
    const parts = args.split(";");
    const state = CONEMU_STATE[parts[0] ?? ""];
    const payload = {};
    if (state !== undefined)
        payload.progressState = state;
    ...
    if (parts.length >= 2 && (state === "set" || state === "error" || state === "paused")) {
        ...
        if (Number.isFinite(pr))
            payload.progress = clampPercent(pr);
    }
...
function parseOsc777(rest) {
    ...
    const payload = { message, source: "osc777" };
    return {
        kind: "osc9",
...
function parseOsc633(rest) {
    ...
    if (parts.length >= 2 && parts[1] !== "") {
        ...
        if (Number.isFinite(code))
            return { kind: "cmd-exit", payload: { exitCode: code } };
    }
...
//# sourceMappingURL=notify-scanner.js.map