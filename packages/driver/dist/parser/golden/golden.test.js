...
import { CommandCorrelator } from "../correlator.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function notificationKinds(tokens) {
    return tokens
...
function countKind(tokens, kind) {
...
describe("golden: tmux34-session.raw (real tmux 3.4 -C capture)", () => {
    ...
    it("parses session-changed notification from real capture", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const sessionChangedToken = tokens.find((t) => t.kind === "notification" && t.keyword === "session-changed");
        assert.ok(sessionChangedToken !== undefined, "%session-changed not found in capture");
        if (sessionChangedToken?.kind !== "notification")
            return;
    ...
    it("parses window-add notification from real capture", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const tok = tokens.find((t) => t.kind === "notification" && t.keyword === "window-add");
        assert.ok(tok !== undefined, "%window-add not found");
        if (tok?.kind !== "notification")
            return;
    ...
    it("parses window-pane-changed from real capture", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const tok = tokens.find((t) => t.kind === "notification" && t.keyword === "window-pane-changed");
    ...
    it("parses session-window-changed from real capture", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const tok = tokens.find((t) => t.kind === "notification" && t.keyword === "session-window-changed");
        assert.ok(tok !== undefined, "%session-window-changed not found");
        if (tok?.kind !== "notification")
            return;
        const ev = parseNotification(tok);
        assert.equal(ev.kind, "session-window-changed");
        if (ev.kind !== "session-window-changed")
            return;
    ...
    it("%layout-change is emitted as unknown notification (not in notifications.ts vocabulary)", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const tok = tokens.find((t) => t.kind === "notification" && t.keyword === "layout-change");
    ...
    it("parses %exit notification from real capture", () => {
        const tokens = tokenizeBuffer(rawBuf);
        const tok = tokens.find((t) => t.kind === "notification" && t.keyword === "exit");
    ...
    it("decodes %output notifications with all-ASCII payload", () => {
        ...
        const outputToken = tokens.find((t) => t.kind === "notification" && t.keyword === "output");
        assert.ok(outputToken !== undefined, "expected at least one %output notification");
        if (outputToken?.kind !== "notification")
            return;
    ...
    it("NON-UTF-8: %output payload with \\xc0\\xfe\\xff bytes preserved byte-exact", () => {
        ...
        const tokens = tokenizeBuffer(rawBuf);
        const outputTokens = tokens.filter((t) => t.kind === "notification" && t.keyword === "output");
        let found = false;
    ...
    it("STREAMING: byte-by-byte feed yields same tokens as one-shot on real capture", () => {
        ...
        assert.equal(streamTokens.length, oneShotTokens.length, `one-shot: ${oneShotTokens.length} tokens, byte-by-byte: ${streamTokens.length} tokens`);
        // Compare kinds
        for (let i = 0; i < oneShotTokens.length; i++) {
            ...
            if (a.kind === "notification" && b.kind === "notification") {
                ...
                assert.deepEqual(Array.from(b.rawLine), Array.from(a.rawLine), `token[${i}] rawLine mismatch`);
            }
            if (a.kind === "block-body" && b.kind === "block-body") {
                assert.deepEqual(Array.from(b.bytes), Array.from(a.bytes), `token[${i}] body bytes mismatch`);
            }
        }
    });
    // -------------------------------------------------------------------------
    ...
    it("CommandCorrelator resolves command blocks from real capture", async () => {
        ...
        const beginCount = tokens.filter((t) => t.kind === "block-begin" && t.flags !== 0).length;
        const promises = [];
...
describe("golden: non-UTF-8 %output payload (byte-exact preservation)", () => {
    ...
    it("parseOutputNotification extracts paneId and decodes bytes", () => {
        ...
        const tok = { kind: "notification", keyword: "output", rawLine };
    ...
    it("STREAMING: byte-by-byte feed preserves non-UTF-8 bytes in rawLine", () => {
        ...
        const oneShotOut = oneShotTokens.find((t) => t.kind === "notification" && t.keyword === "output");
        const streamedOut = streamedTokens.find((t) => t.kind === "notification" && t.keyword === "output");
        assert.ok(oneShotOut !== undefined && streamedOut !== undefined);
        if (oneShotOut?.kind !== "notification" || streamedOut?.kind !== "notification")
            return;
        assert.deepEqual(Array.from(streamedOut.rawLine), Array.from(oneShotOut.rawLine), "rawLine must be identical between one-shot and byte-by-byte");
    });
});
// ---------------------------------------------------------------------------
...
function assertRealCapture(rawBuf, exp) {
    const tokens = tokenizeBuffer(rawBuf);
    assert.ok(tokens.length > 0, "expected at least one token");
    ...
    if (exp.dcs) {
        ...
        assert.equal(tokens[tokens.length - 1].kind, "dcs-close", "last token must be dcs-close");
    }
    else {
    ...
    const userBlocks = tokens.filter((t) => t.kind === "block-begin" && t.flags !== 0).length;
    assert.ok(userBlocks >= exp.minBeginCount, `expected ≥${exp.minBeginCount} user-command blocks, got ${userBlocks}`);
    // Required notifications, parsed with expected fields.
    const findNotif = (kw) => tokens.find((t) => t.kind === "notification" && t.keyword === kw);
    ...
    if (lcTok.kind === "notification") {
        ...
        assert.equal(ev.kind, "unknown", "%layout-change should be parsed as unknown");
        ...
        assert.ok(rl.startsWith("%layout-change @1 "), `%layout-change rawLine should start with "%layout-change @1 ", got ${JSON.stringify(rl.slice(0, 32))}`);
    }
    ...
    for (const tok of tokens) {
        if (tok.kind !== "notification" || tok.keyword !== "output")
            continue;
        const parsed = parseOutputNotification(tok);
        if (parsed === null)
            continue;
    ...
    assert.ok(nonUtf8Found, "expected one %output token to contain raw bytes 0xc0 0xfe 0xff");
    // Streaming invariance.
    const streamedTokens = byteByByte(rawBuf);
    assert.equal(streamedTokens.length, tokens.length, "byte-by-byte must yield same token count as one-shot");
...
async function assertCorrelatorResolvesAll(tokens) {
    const corr = new CommandCorrelator({ onNotification: () => { } });
    const beginCount = tokens.filter((t) => t.kind === "block-begin" && t.flags !== 0).length;
    const promises = [];
    for (let i = 0; i < beginCount; i++)
...
describe("golden: tmux35a-C.raw (real tmux 3.5a, post-3.4, -C, debian:trixie)", () => {
    ...
    it("%window-renamed is parsed (3.5a emits it on shell auto-rename)", () => {
        ...
        if (!tokens)
            tokens = tokenizeBuffer(rawBuf);
        const wrTok = tokens.find((t) => t.kind === "notification" && t.keyword === "window-renamed");
...
describe("golden: tmux34-CC.raw (real tmux 3.4, -CC DCS-wrapped, host)", () => {
    ...
    it("inner lines use CRLF (\\r\\n) — control-mode over a real pty", () => {
        ...
        if (!tokens)
            tokens = tokenizeBuffer(rawBuf);
        const exitTok = tokens.find((t) => t.kind === "notification" && t.keyword === "exit");
        ...
        if (exitTok.kind === "notification") {
            ...
            assert.ok(!rl.includes("\r"), `%exit rawLine must not contain CR, got ${JSON.stringify(rl)}`);
        }
    });
});
