...
import { sendKeysHex, refreshClientSize, refreshClientWindowSize, refreshClientFlow, refreshClientSubscribeWindows, refreshClientSubscribePanes, listWindows, listPanes, capturePane, displayMessagePane, PANE_GRID_FACTS_FORMAT, newWindow, splitWindow, breakPane, parseEffectIds, EFFECT_IDS_FORMAT, setOption, setOptionForWindow, showOptionsForWindow, 
...
describe("sendKeysHex", () => {
    ...
    it("uses lowercase hex digits", () => {
        ...
        const cmd = sendKeysHex(1, bytes);
        assert.ok(cmd.includes("ab cd ef"), `Expected lowercase hex, got: ${cmd}`);
    });
...
describe("refreshClientWindowSize", () => {
    it("emits @<window>:<W>x<H> form", () => {
        assert.equal(refreshClientWindowSize(2, 159, 48), "refresh-client -C @2:159x48");
    });
});
// ---------------------------------------------------------------------------
...
describe("listWindows", () => {
    it("uses the default format when none is provided", () => {
        ...
        assert.ok(cmd.startsWith("list-windows -F "), `Expected 'list-windows -F ...', got: ${cmd}`);
        assert.ok(cmd.includes("window_id"), "Default format should include window_id");
    });
...
describe("listPanes", () => {
    it("uses the default format when none is provided", () => {
        const cmd = listPanes();
        assert.ok(cmd.startsWith("list-panes"), `Expected 'list-panes ...', got: ${cmd}`);
    ...
    it("session-scoped form targets the immutable session id (tc-0v59)", () => {
        ...
        assert.ok(cmd.startsWith("list-panes -s -t $3 -F "), `Expected 'list-panes -s -t $3 -F ...', got: ${cmd}`);
...
describe("capturePane", () => {
    ...
    it("adds -S flag for startLine", () => {
        assert.equal(capturePane(3, { startLine: -100 }), "capture-pane -t %3 -p -S -100");
    });
    it("adds -E flag for endLine", () => {
        assert.equal(capturePane(3, { endLine: 0 }), "capture-pane -t %3 -p -E 0");
    });
    it("combines all options", () => {
        assert.equal(capturePane(7, { escapes: true, startLine: -200, endLine: -1 }), "capture-pane -t %7 -p -e -S -200 -E -1");
    });
    it("accepts the '-' sentinel for full-history start/end (tc-5quo)", () => {
        assert.equal(capturePane(4, { escapes: true, startLine: "-", endLine: "-" }), "capture-pane -t %4 -p -e -S - -E -");
    });
    it("adds -J flag when joinWrapped=true (tc-0ghi: hydration form)", () => {
        assert.equal(capturePane(4, { escapes: true, joinWrapped: true, startLine: "-", endLine: "-" }), "capture-pane -t %4 -p -e -J -S - -E -");
    });
...
describe("displayMessagePane", () => {
    it("emits display-message -p -t %N -F '<format>'", () => {
        assert.equal(displayMessagePane(3, "#{cursor_x}"), "display-message -p -t %3 -F '#{cursor_x}'");
    });
    it("single-quotes the grid-facts format (contains #{ } and , )", () => {
        assert.equal(displayMessagePane(7, PANE_GRID_FACTS_FORMAT), "display-message -p -t %7 -F '#{cursor_x},#{cursor_y},#{history_size},#{pane_height}'");
    });
});
// ---------------------------------------------------------------------------
// new-window
...
describe("newWindow", () => {
    ...
    it("quotes window name with a space", () => {
        assert.equal(newWindow({ name: "my window" }), "new-window -n 'my window'");
    });
    it("quotes window name with single quote inside", () => {
        const cmd = newWindow({ name: "it's mine" });
        // Must be properly shell-escaped.
        assert.ok(cmd.includes("it'\\''s"), `Expected escaped single quote, got: ${cmd}`);
    });
    it("includes -c for startDirectory", () => {
        assert.equal(newWindow({ startDirectory: "/tmp" }), "new-window -c /tmp");
    });
...
describe("splitWindow", () => {
    ...
    it("includes -p percentage", () => {
        assert.equal(splitWindow(3, "vertical", { percent: 30 }), "split-window -v -t %3 -p 30");
    });
    it("includes -c startDirectory", () => {
        assert.equal(splitWindow(1, "horizontal", { startDirectory: "/home/user" }), "split-window -h -t %1 -c /home/user");
    });
    it("includes shell command", () => {
        assert.equal(splitWindow(2, "vertical", { shellCommand: "htop" }), "split-window -v -t %2 htop");
    });
    ...
    it("quotes shellCommand with spaces even when paneId is undefined", () => {
        assert.equal(splitWindow(undefined, "horizontal", { shellCommand: "npm test --watch" }), "split-window -h 'npm test --watch'");
    });
    it("quotes startDirectory even when paneId is undefined", () => {
        assert.equal(splitWindow(undefined, "vertical", { startDirectory: "/path with spaces" }), "split-window -v -c '/path with spaces'");
    });
...
describe("breakPane", () => {
    ...
    it("printIds adds -P -F EFFECT_IDS_FORMAT before -s", () => {
        assert.equal(breakPane(3, { printIds: true }), "break-pane -d -P -F '#{pane_id} #{window_id}' -s %3");
    });
});
// ---------------------------------------------------------------------------
...
describe("newWindow printIds (tc-ozk.1)", () => {
    it("bare printIds", () => {
        assert.equal(newWindow({ printIds: true }), "new-window -P -F '#{pane_id} #{window_id}'");
    });
    it("printIds is placed before -n name", () => {
        assert.equal(newWindow({ printIds: true, name: "editor" }), "new-window -P -F '#{pane_id} #{window_id}' -n editor");
    });
});
// ---------------------------------------------------------------------------
...
describe("setOption", () => {
    it("session scope: no flags", () => {
        assert.equal(setOption("session", "monitor-activity", "on"), "set-option monitor-activity on");
    });
    it("window scope: -w flag", () => {
        assert.equal(setOption("window", "monitor-activity", "on"), "set-option -w monitor-activity on");
    });
    it("session-global scope: -g flag", () => {
        assert.equal(setOption("session-global", "@tmuxcc", "1"), "set-option -g @tmuxcc 1");
    });
    it("window-global scope: -wg flags (used for monitor-activity)", () => {
        assert.equal(setOption("window-global", "monitor-activity", "on"), "set-option -wg monitor-activity on");
    });
    it("window-global monitor-activity off", () => {
        assert.equal(setOption("window-global", "monitor-activity", "off"), "set-option -wg monitor-activity off");
    });
...
describe("setOptionForWindow", () => {
    it("sets synchronize-panes on for window @3", () => {
        assert.equal(setOptionForWindow(3, "synchronize-panes", "on"), "set-option -wt @3 synchronize-panes on");
    });
    it("sets synchronize-panes off for window @5", () => {
        assert.equal(setOptionForWindow(5, "synchronize-panes", "off"), "set-option -wt @5 synchronize-panes off");
    });
    it("window @0 edge case", () => {
        assert.equal(setOptionForWindow(0, "monitor-activity", "off"), "set-option -wt @0 monitor-activity off");
    });
    it("quotes option value with spaces", () => {
        assert.equal(setOptionForWindow(1, "some-option", "a value"), "set-option -wt @1 some-option 'a value'");
    });
});
// ---------------------------------------------------------------------------
...
describe("showOptionsForWindow", () => {
    it("emits show-options -wvt @3 synchronize-panes", () => {
        assert.equal(showOptionsForWindow(3, "synchronize-panes"), "show-options -wvt @3 synchronize-panes");
    });
    it("works for window @0", () => {
        assert.equal(showOptionsForWindow(0, "monitor-activity"), "show-options -wvt @0 monitor-activity");
    });
});
// ---------------------------------------------------------------------------
...
describe("setOptionForPane", () => {
    it("sets @tmuxcc_label on pane %5", () => {
        assert.equal(setOptionForPane(5, "@tmuxcc_label", "build"), "set-option -pt %5 @tmuxcc_label build");
    });
    it("quotes a name with spaces", () => {
        assert.equal(setOptionForPane(3, "@tmuxcc_label", "my server"), "set-option -pt %3 @tmuxcc_label 'my server'");
    });
    it("escapes embedded single quotes", () => {
        assert.equal(setOptionForPane(2, "@tmuxcc_label", "it's"), "set-option -pt %2 @tmuxcc_label 'it'\\''s'");
    });
    it("empty value serializes as '' (clears the durable name)", () => {
        assert.equal(setOptionForPane(1, "@tmuxcc_label", ""), "set-option -pt %1 @tmuxcc_label ''");
    });
    it("pane %0 edge case", () => {
        assert.equal(setOptionForPane(0, "@tmuxcc_label", "x"), "set-option -pt %0 @tmuxcc_label x");
    });
});
describe("showOptionsForPane", () => {
    it("emits show-options -pvt %5 @tmuxcc_label", () => {
        assert.equal(showOptionsForPane(5, "@tmuxcc_label"), "show-options -pvt %5 @tmuxcc_label");
    });
});
...
describe("refreshClientSubscribeWindows", () => {
    it("emits refresh-client -B with @* scope for synchronize-panes watch", () => {
        assert.equal(refreshClientSubscribeWindows("sync-watch", "#{?synchronize-panes,1,0}"), "refresh-client -B 'sync-watch:@*:#{?synchronize-panes,1,0}'");
    });
    it("works with a simple name and format", () => {
        assert.equal(refreshClientSubscribeWindows("my-sub", "#{window_name}"), "refresh-client -B 'my-sub:@*:#{window_name}'");
    });
    it("escapes embedded single quotes in name via the \\' idiom", () => {
        // name contains a single-quote — verify escaping
        assert.equal(refreshClientSubscribeWindows("it's", "#{window_name}"), "refresh-client -B 'it'\\''s:@*:#{window_name}'");
    });
});
...
describe("refreshClientSubscribePanes", () => {
    it("emits refresh-client -B with %* (all-panes) scope for the pane_title watch", () => {
        assert.equal(refreshClientSubscribePanes("title-watch", "#{pane_title}"), "refresh-client -B 'title-watch:%*:#{pane_title}'");
    });
    it("works with a simple name and format", () => {
        assert.equal(refreshClientSubscribePanes("my-sub", "#{pane_current_command}"), "refresh-client -B 'my-sub:%*:#{pane_current_command}'");
    });
    it("escapes embedded single quotes in name via the \\' idiom", () => {
        assert.equal(refreshClientSubscribePanes("it's", "#{pane_title}"), "refresh-client -B 'it'\\''s:%*:#{pane_title}'");
    });
});
// ---------------------------------------------------------------------------
// tc-zna.3: managed-window resize builders
