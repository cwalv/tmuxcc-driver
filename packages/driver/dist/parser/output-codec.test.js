...
import { decodeOutputPayload, encodeOutputPayload, parseOutputNotification, } from "./output-codec.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function notifToken(keyword, rawLine) {
...
describe("all 256 byte values round-trip", () => {
    it("encode→decode preserves every byte 0x00–0xFF", () => {
        ...
        for (let i = 0; i < 256; i++) {
            assert.strictEqual(decoded[i], i, `byte 0x${i.toString(16).padStart(2, "0")} should round-trip`);
        }
    });
    it("each byte individually round-trips", () => {
        for (let b = 0; b <= 0xff; b++) {
            ...
            assert.strictEqual(decoded[0], b, `byte 0x${b.toString(16).padStart(2, "0")} should round-trip individually`);
        }
    });
});
// ---------------------------------------------------------------------------
...
describe("fuzz: pseudo-random buffers round-trip", () => {
    ...
    it("buffers with all control chars and backslash clusters round-trip", () => {
        ...
        const escapedBytes = new Uint8Array(Array.from({ length: 32 }, (_, i) => i) // 0x00–0x1F
