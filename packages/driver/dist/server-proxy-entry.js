 ...
 * @internal
 */
export function _parseIdleExitMs(raw) {
    if (raw === undefined)
        return undefined;
 ...
 * @internal
 */
export function _parseEntryConfig(argv, env) {
    let socketName = "";
    let runtimeDir;
    let idleExitMs;
...
function parseArgs() {
    ...
    if (cfg.socketName === null) {
        process.stderr.write("Usage: server-proxy-entry --socket-name <name> [--runtime-dir <path>] " +
...
async function main() {
    ...
    const log = openServerProxyLog(serverProxyLogPath(socketName, runtimeDir !== undefined ? { runtimeDir } : {}));
    if (log !== null) {
    ...
    if (PHASE_TIMING_ENABLED) {
        ...
        const lagTimer = setInterval(() => {
            ...
            process.stderr.write(`[tc-is5w] metrics nodejs_eventloop_lag_seconds_p99=${p99s.toFixed(6)}\n`);
            lagHistogram.reset();
        }, 5_000);
    ...
    try {
        ...
        if (err instanceof ServerProxyAlreadyRunningError ||
            err?.code === "server-proxy.already-running") {
            process.stderr.write(`serverProxy: socket ${socketName} already owned by a live broker — ` +
    ...
    process.on("SIGUSR2", () => {
        void serverProxy
            .toggleMetricsHttp()
            .then((state) => {
            process.stderr.write(state.enabled
...
//# sourceMappingURL=server-proxy-entry.js.map