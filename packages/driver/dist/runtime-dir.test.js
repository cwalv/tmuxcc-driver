...
function makeSubDir(baseDir, socketName) {
 ...
 */
function backdateDir(dir, offsetMs = 5_000) {
 ...
 */
function startListeningServer(sockPath) {
    return new Promise((resolve, reject) => {
        const server = net.createServer();
        server.once("error", reject);
        server.listen(sockPath, () => {
            const stop = () => new Promise((res, rej) => server.close((err) => (err ? rej(err) : res())));
            resolve(stop);
        });
    });
}
...
describe("gcStaleRuntimeDirs (tc-s1sm)", () => {
    it("removes a stale dir whose server-proxy.sock is not answering", async () => {
        ...
        try {
            ...
            assert.equal(fs.existsSync(staleDir), false, "stale dir (dead socket) must be removed by GC");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
    ...
    it("removes an orphan dir with no server-proxy.sock at all", async () => {
        ...
        try {
            ...
            const currentName = nextSocketName();
            await gcStaleRuntimeDirs(baseDir, currentName, { probeTimeoutMs: 200, minAgeMs: 0 });
            assert.equal(fs.existsSync(orphanDir), false, "orphan dir (no server-proxy.sock) must be removed by GC");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
    ...
    it("keeps a live-socket dir (server-proxy.sock accepting connections)", async () => {
        ...
        try {
            ...
            assert.equal(fs.existsSync(liveDir), true, "live-socket dir must NOT be removed by GC");
        }
        finally {
            if (stop)
                await stop();
    ...
    it("never removes the current dir even if it has no socket yet", async () => {
        ...
        try {
            ...
            await gcStaleRuntimeDirs(baseDir, currentName, { probeTimeoutMs: 200, minAgeMs: 0 });
            assert.equal(fs.existsSync(currentDir), true, "current dir must never be removed by GC");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
    ...
    it("tolerates ENOENT races: does not throw if a dir disappears during sweep", async () => {
        ...
        try {
            ...
            const currentName = nextSocketName();
            // Must not throw.
            await assert.doesNotReject(() => gcStaleRuntimeDirs(baseDir, currentName, { probeTimeoutMs: 200, minAgeMs: 0 }), "gcStaleRuntimeDirs must tolerate ENOENT races without throwing");
            // stale2 (still present and empty = orphan) should be removed.
            assert.equal(fs.existsSync(stale2Dir), false, "stale2 orphan dir must be removed even after stale1 disappeared");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
    ...
    it("does not throw when baseDir does not exist", async () => {
        ...
        await assert.doesNotReject(() => gcStaleRuntimeDirs(nonExistent, "any-socket-name", { probeTimeoutMs: 200, minAgeMs: 0 }), "gcStaleRuntimeDirs must not throw when the base dir does not exist");
    });
    it("skips non-directory entries in baseDir", async () => {
        ...
        try {
            ...
            assert.equal(fs.existsSync(filePath), true, "plain files in baseDir must not be removed by GC");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
    ...
    it("age guard: keeps a fresh sock-less dir (sibling broker mid-startup)", async () => {
        ...
        try {
            ...
            assert.equal(fs.existsSync(youngDir), true, "a dir younger than minAgeMs must not be swept even without a socket");
            ...
            await gcStaleRuntimeDirs(baseDir, currentName, { probeTimeoutMs: 200, minAgeMs: 0 });
            assert.equal(fs.existsSync(youngDir), false, "with minAgeMs=0 the sock-less orphan is removed");
        }
        finally {
            fs.rmSync(baseDir, { recursive: true, force: true });
        }
