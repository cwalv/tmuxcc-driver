...
async function waitFor(predicate, timeoutMs, what, intervalMs = 25) {
 ...
 */
function findWatcherPids(socketName) {
...
async function sendServerProxyCommand(mux, command, outgoingSeq) {
 ...
 */
async function attachToSession(endpoint, sessionId, primaryPaneId) {
    const transport = await connectSocketTransport(endpoint);
    await runClientHandshake(transport, CLIENT_CAPS, "server-proxy.capabilities");
    const mux = new TransportMux(transport);
    const snapshotPromise = new Promise((resolve) => {
...
describe("server-proxy – unit (no tmux)", () => {
    ...
    it("U3 (tc-k6v): server-proxy.info returns identity fields without tmux", async () => {
        ...
        try {
            ...
            const info = resp.result.payload.info;
            ...
            assert.ok(info.connectedClientCount >= 1, `connectedClientCount must include the requesting client, got ${info.connectedClientCount}`);
    ...
    it("U3b (tc-bn7d): rpc_round_trip_seconds is observed in the live metricsText via _handleCommand", async () => {
        ...
        try {
            ...
            assert.ok(resp.result.ok, `Expected ok=true, got: ${JSON.stringify(resp.result)}`);
            const info = resp.result.payload.info;
            assert.ok(info.metricsText !== null, "server-proxy.info must carry the metricsText exposition");
            const text = info.metricsText;
            assert.ok(text.includes("rpc_round_trip_seconds"), `metricsText must contain rpc_round_trip_seconds (tc-bn7d); got:\n${text}`);
            // The application-leg RTT is attributed to the command kind that drove it.
            assert.ok(text.includes('kind="server-proxy.info"') &&
                /rpc_round_trip_seconds_count\{kind="server-proxy\.info"\}/.test(text), `rpc_round_trip_seconds must be labelled by command kind; got:\n${text}`);
            mux.transport.close();
        }
        finally {
            await serverProxy.shutdown();
    ...
    it("U5 (tc-7aqb.2): server-proxy.info echoes spawnInfo.buildId when opts.spawnInfo is set", async () => {
        ...
        try {
            ...
            assert.ok(resp.result.ok, `Expected ok=true, got: ${JSON.stringify(resp.result)}`);
            const info = resp.result.payload.info;
            assert.deepEqual(info.spawnInfo, spawnInfo, `spawnInfo round-trip: expected ${JSON.stringify(spawnInfo)}, got ${JSON.stringify(info.spawnInfo)}`);
            mux.transport.close();
        }
        finally {
            await serverProxy.shutdown();
    ...
    it("U6 (tc-7aqb.2): server-proxy.info omits spawnInfo when opts.spawnInfo is not set (backward-compat)", async () => {
        ...
        try {
            ...
            assert.ok(resp.result.ok, `Expected ok=true, got: ${JSON.stringify(resp.result)}`);
            const info = resp.result.payload.info;
            assert.equal(info.spawnInfo, undefined, `spawnInfo must be absent when not configured, got ${JSON.stringify(info.spawnInfo)}`);
            mux.transport.close();
        }
        finally {
            await serverProxy.shutdown();
    ...
    it("U2: serverProxy.endpoint() equals serverProxySocketPath(socketName) — well-known path", async () => {
        ...
        try {
            ...
            assert.equal(serverProxy.endpoint(), expected, `serverProxy.endpoint() must equal serverProxySocketPath(socketName): got ${serverProxy.endpoint()}, want ${expected}`);
        }
        finally {
            await serverProxy.shutdown();
        }
    ...
    it("U5 (tc-295a.35): snapshot reports tmuxAvailable=true when tmux is present", async () => {
        ...
        try {
            ...
            assert.equal(snapshot.tmuxAvailable, true, `tmux present ⇒ snapshot.tmuxAvailable must be true; got ${String(snapshot.tmuxAvailable)}`);
        }
        finally {
            await serverProxy.shutdown();
        }
    ...
    it("U6 (tc-295a.35): broker stays up and snapshot reports tmuxAvailable=false when tmux is absent", async () => {
        ...
        try {
            ...
            assert.equal(snapshot.tmuxAvailable, false, `tmux absent ⇒ snapshot.tmuxAvailable must be false; got ${String(snapshot.tmuxAvailable)}`);
        }
        finally {
...
describe("server-proxy – integration (requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("I2: session.claim creates a session and sessionProxy, returns sessionId + created=true", async () => {
        ...
        assert.equal(payload.endpoint, undefined, "D5: claim must NOT return an endpoint");
    ...
    it("I3: session.claim on same name returns same sessionId, created=false", async () => {
        ...
        const p1 = resp1.result.payload;
        const p2 = resp2.result.payload;
    ...
    it("I4: session.create fails if name is already taken", async () => {
        ...
        assert.equal(r1.result.payload.created, true, "session.create must report created=true on success");
        const r2 = await sendServerProxyCommand(mux, { kind: "session.create", name: "unique-sess" }, seq);
        assert.equal(r2.result.ok, false);
        assert.equal(r2.result.code, "session.name-taken");
        mux.transport.close();
    });
    ...
    it("I4a (tc-295a.5): two sequential session.createUnique calls with the same baseName yield distinct names + sessionIds", async () => {
        const { mux } = await connectToServerProxy(serverProxy.endpoint());
        const seq = { value: 1 };
        const r1 = await sendServerProxyCommand(mux, { kind: "session.createUnique", baseName: "myws" }, seq);
        assert.ok(r1.result.ok, `First createUnique failed: ${JSON.stringify(r1.result)}`);
        const p1 = r1.result.payload;
        ...
        const r2 = await sendServerProxyCommand(mux, { kind: "session.createUnique", baseName: "myws" }, seq);
        assert.ok(r2.result.ok, `Second createUnique failed: ${JSON.stringify(r2.result)}`);
        const p2 = r2.result.payload;
    ...
    it("I5: session.destroy kills session and reaps session-proxy", async () => {
        ...
        const sessionId = claimResp.result.payload.sessionId;
    ...
    it("I8 (D5, tc-4b6k.4): session.attach with a primaryPaneId binds the connection and delivers the session snapshot", async () => {
        ...
        assert.equal(attached.snapshot.type, "snapshot");
        const sess = attached.snapshot.session;
    ...
    it("I10 (tc-k6v): server-proxy.info reports tmux server pid + per-session session-proxy pid and pane count", async () => {
        ...
        assert.ok(typeof info.tmuxServerPid === "number" && info.tmuxServerPid > 0, `tmuxServerPid must be a live pid, got ${String(info.tmuxServerPid)}`);
        ...
        assert.ok(typeof row.sessionProxyPid === "number" && row.sessionProxyPid > 0, `sessionProxyPid must be a live pid, got ${String(row.sessionProxyPid)}`);
        assert.equal(row.sessionProxyPid, process.pid, "tc-2x3.3: in-process session-proxy reports the server-proxy's own pid");
        // kill -0 probes liveness without sending a signal.
        assert.doesNotThrow(() => process.kill(row.sessionProxyPid, 0), "sessionProxyPid must be running");
        ...
        assert.ok(typeof row.attachedClientCount === "number" && row.attachedClientCount >= 0, `attachedClientCount must be a number, got ${String(row.attachedClientCount)}`);
        mux.transport.close();
    });
    it("I11 (tc-3y8.7): attachedClientCount is external-only — session-proxy+watcher do not inflate the count", async () => {
        ...
        await waitFor(() => findWatcherPids(socketName).length > 0, 5_000, "watcher to attach to tmux server");
        ...
        assert.ok(sessionInfo, `sessions.snapshot must include 'own-clients-test': ${JSON.stringify(snapshot.sessions)}`);
        // With session-proxy + watcher attached but NO real human clients, external count = 0.
        assert.equal(sessionInfo.attachedClientCount, 0, `attachedClientCount must be 0 (session-proxy and watcher are tmuxcc-owned, not external); got ${sessionInfo.attachedClientCount}`);
        ...
        assert.ok(infoRow, `server-proxy.info sessions must include 'own-clients-test': ${JSON.stringify(info.sessions)}`);
        assert.equal(infoRow.attachedClientCount, 0, `server-proxy.info attachedClientCount must be 0 for external-only semantics; got ${infoRow.attachedClientCount}`);
        mux.transport.close();
    });
    it("I12 (tc-i9aq.2): session.topology returns windows and panes for a known session", async () => {
        ...
        const topology = topResp.result.payload.topology;
    ...
    it("I13 (tc-i9aq.2): session.topology returns empty topology for unknown sessionId", async () => {
        const { mux } = await connectToServerProxy(serverProxy.endpoint());
        const seq = { value: 1 };
        const resp = await sendServerProxyCommand(mux, { kind: "session.topology", sessionId: "s999-nonexistent" }, seq);
        ...
        const topology = resp.result.payload.topology;
    ...
    it("I7 (D5, tc-4b6k.4): claim then session.attach on the broker socket runs the session snapshot round-trip", async () => {
        ...
        const { mux: sessionProxyMux, snapshot: snapshot2 } = await attachToSession(serverProxy.endpoint(), sessionId);
        // SessionProxy SnapshotMessage (wire v3, tc-j9c.2) has singular `session: SnapshotSession`
        assert.equal(snapshot2.type, "snapshot");
        const sess2 = snapshot2.session;
...
describe("server-proxy — capability-required wire shape (tc-u4ny.2)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("I-cap (tc-u4ny.2): session.create with env against below-3.2 caps → wire CommandFailure with details.capability", async () => {
        const socketName = nextSocketName();
        ...
        try {
            ...
            const resp = await sendServerProxyCommand(mux, { kind: "session.create", name: "cap-check", env: { TMUXCC_TEST_CAP: "1" } }, seq);
            ...
            const failure = resp.result;
            assert.equal(failure.code, "tmux.capability-required", `Expected code "tmux.capability-required", got: ${JSON.stringify(failure.code)}`);
            // details.capability must arrive on the wire (the epic's critical link).
            assert.ok(failure.details !== undefined && failure.details !== null, `Expected details to be present on the wire, got: ${JSON.stringify(failure.details)}`);
            assert.equal(failure.details.capability, "newSessionEnvFlag", `Expected details.capability "newSessionEnvFlag", got: ${JSON.stringify(failure.details)}`);
            mux.transport.close();
        }
        finally {
...
describe("tc-w61: @tmuxcc 1 marker set on spawn / claim", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("M1: session.claim sets @tmuxcc 1 on the spawned tmux session", async () => {
        ...
        const markerResult = spawnSync("tmux", ["-L", socketName, "show-options", "-t", sessionName, "-v", "@tmuxcc"], { encoding: "utf8", timeout: 3_000, stdio: ["ignore", "pipe", "ignore"] });
        assert.equal(markerResult.status, 0, "show-options must exit 0 (option is set)");
        assert.equal((markerResult.stdout ?? "").trim(), "1", "@tmuxcc must be exactly '1' after session.claim");
        mux.transport.close();
    });
    it("M2: session.create sets @tmuxcc 1 on the spawned tmux session", async () => {
        ...
        const markerResult = spawnSync("tmux", ["-L", socketName, "show-options", "-t", sessionName, "-v", "@tmuxcc"], { encoding: "utf8", timeout: 3_000, stdio: ["ignore", "pipe", "ignore"] });
        assert.equal(markerResult.status, 0, "show-options must exit 0 (option is set)");
        assert.equal((markerResult.stdout ?? "").trim(), "1", "@tmuxcc must be exactly '1' after session.create");
        mux.transport.close();
    });
    it("M3: mark-on-attach — claiming a pre-existing unmarked session sets @tmuxcc 1", async () => {
        ...
        const created = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", sessionName], { encoding: "utf8", timeout: 5_000 });
        ...
        const before = spawnSync("tmux", ["-L", socketName, "show-options", "-t", sessionName, "-v", "@tmuxcc"], { encoding: "utf8", timeout: 3_000, stdio: ["ignore", "pipe", "ignore"] });
        // show-options exits non-zero or returns empty when the option is unset.
        const beforeValue = (before.stdout ?? "").trim();
        assert.ok(before.status !== 0 || beforeValue !== "1", "Pre-existing session must NOT have @tmuxcc 1 before claiming");
        ...
        const after = spawnSync("tmux", ["-L", socketName, "show-options", "-t", sessionName, "-v", "@tmuxcc"], { encoding: "utf8", timeout: 3_000, stdio: ["ignore", "pipe", "ignore"] });
        assert.equal(after.status, 0, "show-options must exit 0 after claim");
        assert.equal((after.stdout ?? "").trim(), "1", "@tmuxcc must be exactly '1' on pre-existing session after claim (mark-on-attach)");
        mux.transport.close();
    });
});
// ---------------------------------------------------------------------------
...
describe("server-proxy – race test (requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("R1: 10 concurrent claims of the same name all get the same sessionId", async () => {
        ...
        const connections = await Promise.all(Array.from({ length: N }, () => connectToServerProxy(endpoint)));
        // Issue all 10 session.claim requests concurrently
        const responses = await Promise.all(connections.map(({ mux }, i) => sendServerProxyCommand(mux, { kind: "session.claim", name: "race-session" }, { value: i * 100 + 1 })));
        // All must succeed
        for (const resp of responses) {
            assert.ok(resp.result.ok, `Expected ok=true, got: ${JSON.stringify(resp.result)}`);
        }
        // All must resolve to the same sessionId (D5: no endpoint on the wire).
        const sessionIds = new Set(responses.map((r) => r.result.payload.sessionId));
        ...
        const createdCount = responses.filter((r) => r.result.payload.created === true).length;
    ...
    it("R2 (tc-zcqr): 25 sequential single-claim cycles on a fresh server-proxy all succeed", async () => {
        ...
        for (let i = 0; i < N; i++) {
            ...
            const seq = { value: 1 };
            const resp = await sendServerProxyCommand(mux, { kind: "session.claim", name: `r2-cycle-${i}` }, seq);
            assert.ok(resp.result.ok, `cycle ${i}: session.claim must succeed; got: ${JSON.stringify(resp.result)}`);
            const payload = resp.result.payload;
...
describe("server-proxy – self-exit: idle hysteresis (tc-3iv)", () => {
    it("S1: zero IPC clients past hysteresis → self-exit, socket unlinked, respawn works", async () => {
        ...
        try {
            ...
            assert.equal(fs.existsSync(endpoint), false, "socket file must be unlinked on self-exit (next launcher probe must not connect-then-reset)");
...
describe("server-proxy – single-flight bind: double-spawn loser backs off (tc-kyq4.1)", () => {
    it("D1: a second broker on a LIVE socket loses the bind; the winner's socket survives the loser's idle window", async () => {
        ...
        try {
            ...
            await assert.rejects(loser.start(), (err) => err instanceof ServerProxyAlreadyRunningError &&
                err.code === "server-proxy.already-running", "a second broker on a live socket must lose the bind (single-flight), not clobber it");
            ...
            assert.equal(fs.existsSync(endpoint), true, "winner's socket must STILL exist after the loser's idle window (no orphaned-loser unlink)");
            ...
            await loser.shutdown().catch(() => { });
            await winner.shutdown().catch(() => { });
            fs.rmSync(runtimeDir, { recursive: true, force: true });
        }
    });
    it("D2: a DEFINITIVELY-stale leftover socket file is cleaned up + rebound (legit respawn still works)", async () => {
        ...
        const ghost = await import("node:net").then((net) => new Promise((resolve, reject) => {
...
describe("server-proxy – self-exit: detached session does not block idle (tc-7aqb.1, requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
    it("S5: claimed+detached session with zero IPC clients → idle-exit fires after the grace window", { timeout: 30_000 }, async () => {
        ...
        try {
            ...
            assert.deepEqual(exits, ["idle"]);
            assert.equal(fs.existsSync(endpoint), false, "socket file must be unlinked on idle exit (next launcher probe must not connect-then-reset)");
        }
        finally {
            await serverProxy.shutdown(); // idempotent after self-exit
            spawnSync("tmux", ["-L", socketName, "kill-server"], { stdio: "ignore", timeout: 5_000 });
...
describe("server-proxy – self-exit: tmux death & watcher respawn (tc-3iv, requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
    it("S3: tmux kill-server → server-proxy self-exits within 2s and unlinks its socket", async () => {
        ...
        try {
            ...
            assert.ok(exitedAt - killedAt <= 2_000, `self-exit took ${exitedAt - killedAt}ms — must be within 2s of kill-server`);
    ...
    it("S4: watcher SIGKILLed while tmux alive → watcher respawned, server-proxy stays up", async () => {
        ...
        try {
            ...
            // and must NOT exit.
            await waitFor(() => findWatcherPids(socketName).some((p) => p !== pid1), 5_000, "watcher respawn with a new pid");
    ...
    it("S5 (tc-hfxb.22): watcher EOF with an INCONCLUSIVE probe (could not run) → server-proxy STAYS UP, never self-exits", { skip: !TMUX_AVAILABLE }, async () => {
        ...
        try {
            ...
            assert.equal(exits.length, 0, "server-proxy must NOT self-exit on an INCONCLUSIVE watcher-EOF probe (presume-gone-on-a-guess is the tc-hfxb.22 defect)");
            ...
            await waitFor(() => findWatcherPids(socketName).length > 0, 8_000, "watcher re-attaches after the inconclusive EOF (broker still alive)");
...
describe("server-proxy – tc-xnay / tc-ymxe: designed-exit announcement", () => {
    ...
    it("X2 (requires tmux): tmux-gone self-exit broadcasts `server-proxy.exiting` with reason=tmux-gone to connected clients", { skip: !TMUX_AVAILABLE }, async () => {
        ...
        try {
            const { mux } = await connectToServerProxy(endpoint);
            const exitingP = new Promise((resolve) => {
                const unsub = mux.subscribe((msg) => {
                    if (msg.type === "server-proxy.exiting") {
                        unsub();
                        resolve(msg);
    ...
    it("X3: explicit `shutdown()` (SIGTERM equivalent) does NOT broadcast — the launcher already knows it disposed", async () => {
        ...
        try {
            ...
            mux.subscribe((m) => {
                if (m.type === "server-proxy.exiting")
                    exitingSeen = true;
            });
            ...
            assert.equal(exitingSeen, false, "shutdown() must NOT broadcast `server-proxy.exiting` — only designed _selfExit does");
        }
        finally {
...
describe("server-proxy – enriched session fields (tc-295a.4)", { skip: !TMUX_AVAILABLE }, () => {
    ...
    it("E1: snapshot carries tmuxccMarked=true, paneCount>=1, lastActivity>0 after session.claim", async () => {
        ...
        assert.equal(info.tmuxccMarked, true, `tmuxccMarked must be true for a session claimed by tmuxcc; got ${String(info.tmuxccMarked)}`);
        assert.ok(info.paneCount >= 1, `paneCount must be >= 1 in snapshot; got ${String(info.paneCount)}`);
        const nowEpoch = Math.floor(Date.now() / 1_000);
        assert.ok(info.lastActivity > 0 && info.lastActivity <= nowEpoch + 5, `lastActivity must be a positive recent Unix epoch; got ${String(info.lastActivity)}`);
    });
    it("E2: snapshot carries tmuxccMarked=false for a foreign session (no @tmuxcc option)", async () => {
        ...
        const foreignAddedPromise = new Promise((resolve) => {
            const unsub = mux.subscribe((msg) => {
                if (msg.type === "sessions.added" && msg.name === "e2-foreign") {
                    unsub();
                    resolve(msg);
                }
            });
        ...
        const foreignResult = spawnSync("tmux", ["-L", socketName, "new-session", "-d", "-s", "e2-foreign", "-P", "-F", "#{session_id}"], { encoding: "utf8", timeout: 10_000 });
        ...
        assert.equal(added.tmuxccMarked, false, `sessions.added for a foreign session must carry tmuxccMarked=false; got ${String(added.tmuxccMarked)}`);
        ...
        assert.equal(info.tmuxccMarked, false, `tmuxccMarked must be false for a foreign session in snapshot; got ${String(info.tmuxccMarked)}`);
        mux.transport.close();
    });
    it("E3: sessions.added delta carries tmuxccMarked, paneCount, lastActivity for a claimed session", async () => {
        ...
        const addedPromise = new Promise((resolve) => {
            const unsub = mux.subscribe((msg) => {
                if (msg.type === "sessions.added" && msg.name === "e3-delta") {
                    unsub();
                    resolve(msg);
                }
            });
        ...
        assert.equal(added.tmuxccMarked, true, `sessions.added must carry tmuxccMarked=true for a claimed session; got ${String(added.tmuxccMarked)}`);
        assert.ok(added.paneCount >= 1, `sessions.added must carry paneCount>=1; got ${String(added.paneCount)}`);
        const nowEpoch = Math.floor(Date.now() / 1_000);
        const la = added.lastActivity;
        assert.ok(la > 0 && la <= nowEpoch + 5, `sessions.added must carry a positive recent lastActivity; got ${String(la)}`);
        mux.transport.close();
    });
});
// ---------------------------------------------------------------------------
...
describe("server-proxy – shutdown drain: handshake timeout + late-connect guard (tc-i1pg, tc-9r2y)", () => {
    it("tc-i1pg: wedge connection (no handshake) must not block shutdown() — fail-before/pass-after", async () => {
        ...
        const [timeoutP, clearTimeoutFn] = rejectAfter(3_000, "shutdown() hung — tc-i1pg not fixed: wedge connection blocked server.close()");
        try {
    ...
    it("tc-9r2y: late handshake completing after shutdown is rejected — no snapshot, transport closed", async () => {
        ...
        const serverCapsP = new Promise((resolve) => {
            lateTransport.onControl((msg) => {
                if (msg.type === "server-proxy.capabilities")
                    resolve();
            });
        });
        ...
        const [shutdownTimeoutP, clearShutdownTimeout] = rejectAfter(3_000, "shutdown() hung — tc-9r2y: late-connect guard may not be closing the transport");
        try {
...
//# sourceMappingURL=server-proxy.test.js.map