 ...
 */
export function parseMetricsHttpBind(spec, socketName, rtOpts = {}) {
    const raw = (spec ?? "").trim();
    if (raw === "" || raw === "unix") {
    ...
    if (!LOOPBACK_HOSTS.has(host)) {
        throw bindError(`refusing non-loopback TCP bind host "${host}" — only 127.0.0.1 / ::1 / localhost are allowed ` +
 ...
 * socket behind.
 */
export function bindMetricsHttp(bind, providers) {
                ...
                close() {
                    return new Promise((res) => {
                        server.close(() => {
                            ...
                            if (bind.kind === "unix" && bind.managed)
...
async function handleRequest(req, res, providers) {
...
//# sourceMappingURL=metrics-http.js.map