...
import type { PaneNotifyKind } from "@tmuxcc/protocol";

/** Token-bucket budget for one kind: sustained `ratePerSec`, burst `capacity`. */
interface Budget {
 ...
 */
const BUDGETS: Record<PaneNotifyKind, Budget> = {
...
export class PaneNotifyRateLimiter {
  private readonly _buckets = new Map<PaneNotifyKind, Bucket>();

  /**
   * @param nowFn - clock, defaults to `Date.now`. Only used when `allow` is
   *   called without an explicit `nowMs`.
   */
  constructor(private readonly _nowFn: () => number = Date.now) {}
   ...
   */
  allow(kind: PaneNotifyKind, nowMs: number = this._nowFn()): boolean {
 ...
 * expected and silent.
 */
export function isTier1NotifyKind(kind: PaneNotifyKind): boolean {
