 ...
 */
async function attachClient(sessionProxy: SessionProxy): Promise<{
  sessionProxySide: ReturnType<typeof createInMemoryTransportPair>["sessionProxy"];
  ...
  const controlMsgs: Array<{ type: string }> = [];
  const dataChunks: Uint8Array[] = [];
  const addP = sessionProxy.addClient(sessionProxySide);
  await runClientHandshake(client, CLIENT_CAPS);
  client.onControl((m) => controlMsgs.push(m as { type: string }));
  client.onData((_pid, bytes) => dataChunks.push(new Uint8Array(bytes)));
  await addP;
  await waitFor(
    () => (controlMsgs.some((m) => m.type === "pane.hydration.end") ? true : undefined),
    10_000,
    () => `hydration never completed; saw: ${controlMsgs.map((m) => m.type).join(",")}`,
  );
...
describe(
  "tc-76m8.32: a zero-client flood must not wedge the pane (real tmux)",
  ...
  () => {
    ...
    // -----------------------------------------------------------------------
    it(
      "Z1: pane floods while no client is attached; a fresh client attaches and output flows",
      { timeout: 60_000 },
      async () => {
        ...
        try {
          ...
          const tmuxN = (pid as string).slice(1);
          ...
          await waitFor(
            ...
            10_000,
            () => `client 1 never detached; clientCount=${sessionProxy.server.clientCount()}`,
          );
          ...
          await sessionProxy.send(
            `send-keys -t %${tmuxN} 'yes | head -c 200000; echo "ZC-FLOOD-DO""NE"' Enter`,
          );
          ...
          await waitFor(
            ...
            30_000,
            () =>
              `flood neither paused the pane nor completed; ` +
          ...
          assert.equal(
            fc.isPanePaused(pid),
            false,
            `pane must not remain paused after a fresh client attaches; ` +
              `bufferedBytes=${fc.bufferedBytes(pid)} is pinned by a ledger no transport drains ` +
              `(the tc-76m8.32 zero-client wedge)`,
          );
          assert.ok(
            fc.bufferedBytes(pid) <= 16_384,
            `no stale sub-ledger may pin the max above low-water after reattach; ` +
          ...
          await waitFor(
            () => (c2.received().includes("ZC-MARKER-RESUME") ? true : undefined),
            15_000,
            () =>
              `marker never reached the reattached client — the demux gate is still closed ` +
