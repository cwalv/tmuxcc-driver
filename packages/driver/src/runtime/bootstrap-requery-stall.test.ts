...
import type {
  TmuxHost,
  DataHandler as HostDataHandler,
  ExitHandler as HostExitHandler,
  ErrorHandler as HostErrorHandler,
} from "./tmux-host.js";
 ...
 */
class DelayingWriteHost implements TmuxHost {
  private _writesSeen = 0;
  private readonly _queued: Array<string | Uint8Array | Buffer> = [];
  private _released = false;

  constructor(
    private readonly _inner: TmuxHost,
    private readonly _delayWrites: number,
    private readonly _releaseAfterMs: number,
  ) {}
...
describe(
  ...
  () => {
    it("recovers from a stalled first bootstrap requery without a budget blowout or a slot mis-bind", async () => {
      ...
      assert.match(
        stderrLog,
        /bootstrap requery STALLED/,
        "the bounded bootstrap-requery timeout must fire and log the stall + slot cancellation",
      );
      assert.match(
        stderrLog,
        /cancelled 2 correlator slot\(s\)/,
        "both stalled list-* correlator slots must be cancelled before re-issue",
      );
      ...
      assert.ok(
        model.windows.size >= 1,
        `recovered model must have >=1 window (got ${model.windows.size})`,
      );
      assert.ok(
        model.panes.size >= 1,
        `recovered model must have >=1 pane (got ${model.panes.size})`,
      );

      // (c) Recovery landed within a small budget — NOT an unbounded hang.
      assert.ok(
        elapsedMs < RECOVERY_BUDGET_MS,
        `recovery must land within ${RECOVERY_BUDGET_MS} ms (took ${elapsedMs} ms)`,
      );
      ...
      assert.match(
        body,
        /PROBE-OK/,
        `subsequent command's reply must be its OWN reply, not a mis-bound late list-* reply (got: ${JSON.stringify(body)})`,
      );
    ...
    it("does not time out when tmux responds promptly (no false-positive cancel)", async () => {
      ...
      assert.doesNotMatch(
        stderrLog,
        /bootstrap requery STALLED/,
        "a prompt bootstrap must NOT trip the stall timeout (no false positive)",
      );
