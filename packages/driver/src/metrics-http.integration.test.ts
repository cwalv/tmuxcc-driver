...
function getUnix(socketPath: string, reqPath: string): Promise<{ status: number; body: string }> {
...
function sendCommand(
  ...
  return new Promise((resolve, reject) => {
    ...
    const unsub = client.mux.subscribe((msg) => {
      const m = msg as unknown as ServerProxyCommandResponseMessage;
    ...
    client.mux.transport.sendControl({
      ...
      correlationId,
      command,
    } as unknown as Parameters<typeof client.mux.transport.sendControl>[0]);
  });
}

describe("server-proxy metrics-HTTP exposition (tc-44u4.4)", () => {
  let serverProxy: ServerProxyHandle | null = null;
  ...
  afterEach(async () => {
    if (serverProxy !== null) {
      try { await serverProxy.shutdown(); } catch { /* ignore */ }
      serverProxy = null;
    }
    if (runtimeDir !== null) {
  ...
  it("M2: runtime wire toggle binds then unbinds (PRIMARY no-restart path)", async () => {
    ...
    const onPayload = onResp.result.ok ? (onResp.result.payload?.metricsHttp as MetricsHttpStatePayload) : undefined;
    ...
    const offPayload = offResp.result.ok ? (offResp.result.payload?.metricsHttp as MetricsHttpStatePayload) : undefined;
  ...
  it("M4: unix socket is 0600; non-loopback TCP refused; loopback TCP accepted", async () => {
    ...
    await assert.rejects(
      () => serverProxy!.setMetricsHttp(true, "0.0.0.0:0"),
  ...
  it("M5: /metrics includes a session-namespaced registry metric (tmux)", { skip: !tmuxAvailable() }, async () => {
    ...
    assert.match(
      res.body,
      new RegExp(`correlator_pending_slot_max_age_seconds\\{session="${sessionId}"\\}`),
    );
