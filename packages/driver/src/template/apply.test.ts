...
import type { TemplateApplySend } from "./apply.js";
...
function ok(body = ""): CommandResult {
  return { ok: true, commandNumber: 0, body: enc.encode(body) };
}
function err(body = ""): CommandResult {
  return { ok: false, commandNumber: 0, body: enc.encode(body) };
}

interface FakeOptions {
  /** Return a CommandResult to OVERRIDE the default for a matching command, or null to use the default. */
  readonly override?: (cmd: string, callIndex: number) => CommandResult | null;
  /** The initial window id returned by the `list-windows` query. Default "@0". */
 ...
 */
function makeFake(opts: FakeOptions = {}): { send: TemplateApplySend; commands: string[] } {
  ...
  const send: TemplateApplySend = async (cmd: string): Promise<CommandResult> => {
    ...
    if (cmd.startsWith("list-windows")) return ok(initialWindow);
    ...
    if (cmd.startsWith("display-message")) return ok("200x50");
...
describe("applyCompiledTemplate (tc-gjdx.3, fake send)", () => {
  it("single-pane named-template window: awareness FIRST, new-window, kill-initial LAST", async () => {
    ...
    assert.equal(
      commands[1],
      `set-option -t sess ${TEMPLATE_SESSION_OPTION} dev`,
      "awareness option is set FIRST (before any window)",
    );
  ...
  it("unnamed (inline) template does NOT stamp @tmuxcc-template", async () => {
    ...
    assert.equal(
      commands.some((c) => c.includes(TEMPLATE_SESSION_OPTION)),
      false,
      "no awareness stamp for an unnamed template",
    );
  });
  ...
  it("FAIL-LOUD: a rejected split stops the transaction, reports created-so-far, and does NOT roll back", async () => {
    ...
    await assert.rejects(
      () => applyCompiledTemplate(send, plan, "sess"),
      (e: unknown) => {
