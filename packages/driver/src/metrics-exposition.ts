...
/**
 * A single parsed metric family: its `# HELP` / `# TYPE` header lines and the
 * sample lines that follow, keyed by the family (metric base) name.
 */
interface ParsedFamily {
  /** The `# HELP <name> ...` line, if present. */
  help: string | null;
  /** The `# TYPE <name> <type>` line, if present. */
  type: string | null;
  /** Sample lines (no leading `#`), in original order. */
  samples: string[];
}
 ...
 */
function sampleMetricName(line: string): string {
...
function parseExposition(text: string): Map<string, ParsedFamily> {
  ...
  const ensure = (name: string): ParsedFamily => {
  ...
  for (const rawLine of text.split("\n")) {
    ...
    if (line.startsWith("#")) continue; // stray comment — ignore
 ...
 */
function injectSessionLabel(line: string, sessionId: string): string {
  ...
  const spaceIdx = line.indexOf(" ");
  if (spaceIdx < 0) return line; // malformed; pass through untouched
 ...
 */
export function mergeMetricsText(
  serverProxyText: string,
  sessions: readonly SessionExposition[],
): string {
  ...
  for (const fam of serverFamilies.values()) {
    if (fam.help !== null) out.push(fam.help);
    if (fam.type !== null) out.push(fam.type);
    for (const s of fam.samples) out.push(s);
  }
  ...
  const emittedHeader = new Set<string>();
  ...
  for (const ps of parsedSessions) {
    for (const name of ps.families.keys()) {
      if (serverFamilies.has(name)) continue; // owned by server-proxy
