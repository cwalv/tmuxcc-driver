...
describe("tc-295a.6 L1: sessions.removed ordering (requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
  ...
  it("L1: session.destroy broadcasts sessions.removed THEN session-proxy UDS is removed", { timeout: 30_000 }, async () => {
    ...
    assert.ok(
      await probeSessionReachable(serverProxy.endpoint(), sessionId),
      "session must be reachable via session.attach before destroy",
    );
    ...
    assert.ok(
      removedSeenAt !== null,
      "sessions.removed must arrive on C1 after destroy",
    );
    ...
    assert.equal(
      await probeSessionReachable(serverProxy.endpoint(), sessionId),
      false,
      "session must be unreachable via session.attach after destroy + sessions.removed",
    );
...
describe("tc-295a.6 L2: post-destroy C4 reconnect fails (requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
  ...
  it("L2: session.destroy → sessions.removed → UDS gone; post-removal connect fails (protocol tripwire)", { timeout: 30_000 }, async () => {
    ...
    assert.equal(
      reconnectSucceeded,
      false,
      "post-removal C4 reconnect must fail: session.attach must be rejected after sessions.removed " +
...
describe("tc-295a.6 L3: kill-session-under-load ordering (requires tmux)", { skip: !TMUX_AVAILABLE }, () => {
  ...
  it("L3: external kill-session while active → sessions.removed then C4 UDS close in correct order", { timeout: 60_000 }, async () => {
    ...
    c4Transport.sendControl({
      ...
      seq: 1,
      sessionId,
    } as unknown as Parameters<typeof c4Transport.sendControl>[0]);
    ...
    assert.equal(
      await probeSessionReachable(serverProxy.endpoint(), sessionId, 2_000),
      false,
      "session must be unreachable via session.attach after external kill + sessions.removed",
    );
