...
describe("tc-2x3.4: per-session error boundary", { skip: !TMUX_AVAILABLE }, () => {
  ...
  it("EB8 (tc-fah2): last-pane exit → farewell carries cause:pane-exit", { timeout: 30_000 }, async () => {
    ...
    assert.ok(
      farewell !== undefined,
      `EB8-F1: client must receive session.unavailable farewell; got: ${JSON.stringify(received)}`,
    );
    ...
    assert.equal(
      farewell.cause,
      "pane-exit",
      `EB8-F2: farewell cause must be "pane-exit" for a last-pane exit; got: ${JSON.stringify(farewell)}`,
    );
  ...
  it("DS3 (tc-fah2): partial pane exit (not last) → flag not armed → external kill gives cause external", { timeout: 30_000 }, async () => {
    ...
    await waitFor(
      () => proxy.pipeline.getModel().panes.size >= 2,
      8_000,
      "DS3: model must show 2+ panes after split-window",
    );

    // Flag must start false (no pane exit yet).
    assert.equal(
      (proxy as unknown as { _paneExitHeadedCascadeForTesting: boolean })._paneExitHeadedCascadeForTesting,
      false,
      "DS3: _paneExitHeadedCascade must start false",
    );
    ...
    assert.ok(
      farewell !== undefined,
      `DS3-F1: client must receive session.unavailable farewell; got: ${JSON.stringify(received)}`,
    );
    ...
    assert.equal(
      farewell.cause,
      "external",
      `DS3-F2: cause must be "external" when kill-server fires without a preceding last-pane exit; got: ${JSON.stringify(farewell)}`,
    );
  });
