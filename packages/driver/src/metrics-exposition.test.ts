...
function makeSessionReg(seed: { age: number; events: number }): InstanceType<typeof Registry> {
...
describe("mergeMetricsText (tc-44u4.4)", () => {
  it("namespaces same-named session families by session label under one header", async () => {
    ...
    assert.equal(
      (merged.match(/^# HELP correlator_pending_slot_max_age_seconds /gm) ?? []).length,
      1,
      "exactly one HELP line for the wedge gauge",
    );
    assert.equal(
      (merged.match(/^# TYPE correlator_pending_slot_max_age_seconds /gm) ?? []).length,
      1,
    );
  ...
  it("keeps the server-proxy default families singleton (drops the session copies)", async () => {
    ...
    assert.equal(
      (merged.match(/^# HELP process_resident_memory_bytes /gm) ?? []).length,
      1,
    );
