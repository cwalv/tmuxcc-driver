...
describe("newWindow", () => {
  ...
  it("env: single variable → -e NAME=value before shellCommand", () => {
    assert.equal(
      newWindow({ env: { MY_VAR: "hello" } }),
      "new-window -e MY_VAR=hello",
    );
  });
  ...
  it("env: value with spaces is quoted", () => {
    ...
    assert.ok(
      cmd.includes("-e 'MSG=hello world'"),
  ...
  it("env: combined with printIds and name", () => {
    assert.equal(
      newWindow({ printIds: true, name: "win", env: { K: "v" }, shellCommand: "sh" }),
      "new-window -P -F '#{pane_id} #{window_id}' -n win -e K=v sh",
    );
  });
...
describe("splitWindow", () => {
  ...
  it("env: single variable → -e NAME=value before shellCommand", () => {
    assert.equal(
      splitWindow(3, "horizontal", { env: { MY_VAR: "hello" } }),
      "split-window -h -t %3 -e MY_VAR=hello",
    );
  });
  ...
  it("env: combined with printIds and startDirectory", () => {
    assert.equal(
      splitWindow(5, "horizontal", { printIds: true, startDirectory: "/tmp", env: { K: "v" } }),
      "split-window -h -t %5 -P -F '#{pane_id} #{window_id}' -c /tmp -e K=v",
    );
  });
...
describe("selectLayout (tc-gjdx.3)", () => {
  it("emits select-layout -t @<N> with a single-quoted layout string", () => {
    assert.equal(
      selectLayout(2, "6d61,200x50,0,0[200x16,0,0,200x16,0,17,200x16,0,34]"),
      "select-layout -t @2 '6d61,200x50,0,0[200x16,0,0,200x16,0,17,200x16,0,34]'",
    );
  });

  it("single-quotes a layout string containing `{`/`,` (never bare)", () => {
    assert.equal(
      selectLayout(0, "1f24,200x50,0,0{100x50,0,0,49x50,101,0}"),
      "select-layout -t @0 '1f24,200x50,0,0{100x50,0,0,49x50,101,0}'",
    );
  });
});
