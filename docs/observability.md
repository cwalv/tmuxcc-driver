...
| `correlator_pending_slot_max_age_seconds` | Same site as the depth gauge. | Sub-second in steady state. | `> 1 s` = a wedge precursor. Combined with a non-zero `correlator_pending_slots`, this is the canonical "tmux stopped answering" indicator (cf. tc-gek). | None directly — pair with the storm-alarm and `command_round_trip_seconds` tail to triage. |
