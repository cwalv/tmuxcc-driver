...
| Client activity / focus | `input`; explicit foreground (`client.focus`) | **No sizing effect** (tc-cvny): with per-window reporting there is no session-size owner to elect, so neither `input` nor `client.focus` changes any window's size. `client.focus` stays on the wire (accepted and ignored) for the extension's focus tracking. |
