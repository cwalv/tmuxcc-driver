...
| Per-client size | `refresh-client -C @<win>:WxH` (per-window, tmux ≥ 3.4); `window-size latest\|largest\|smallest` | **Per-window report implemented** (tc-cvny): the driver reports each window's box truthfully via `refresh-client -C @<win>:WxH` (change-gated) and lets tmux's native per-window arbitration decide the size — no owner election, no `window-size manual`, no driver-side sizing policy. `resize.request` reports one window; the driver reports every window it knows so none snaps to the global size. `ignoreSize` clients never report. |
| Client activity / focus | `input` (no explicit focus message) | **No sizing effect** (tc-cvny): with per-window reporting there is no session-size owner to elect, so client focus changes no window's size. The `client.focus` wire message + the `size-ownership-activity` feature are retired (tc-cvny.4) — nothing on the wire tracks focus for sizing. |
